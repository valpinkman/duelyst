...
describe('basic route checks', () => {
  ...
  describe('/session', () => {
    it('returns 400 if no client version set', () => new Promise((done) => {
      request
        .get('/session')
        .expect(400, done);
    }));

    it('returns 400 if wrong client version set', () => new Promise((done) => {
      request
        .get('/session')
    ...
    it('returns 401 unauthorized if no token provided', () => new Promise((done) => {
      request
        .get('/session')
