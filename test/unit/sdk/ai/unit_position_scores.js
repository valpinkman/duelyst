...
const positionBackstabAvoidance = require('apps/server/ai/scoring/position/position_backstabAvoidance');
const positionObjectiveBackstab = require('apps/server/ai/scoring/position/position_objective_backstab');
const positionObjectiveDistanceFromBestObjective = require('apps/server/ai/scoring/position/position_objective_distanceFromBestObjective');
const positionObjectiveFrenzy = require('apps/server/ai/scoring/position/position_objective_frenzy');
const positionObjectiveProvoke = require('apps/server/ai/scoring/position/position_objective_provoke');
const positionProximityToEnemies = require('apps/server/ai/scoring/position/position_proximityToEnemies');
const positionProximityToGenerals = require('apps/server/ai/scoring/position/position_proximityToGenerals');
const positionShadowTileAvoidance = require('apps/server/ai/scoring/position/position_shadowTileAvoidance');
const positionZeal = require('apps/server/ai/scoring/position/position_zeal');
...
describe('unit position scoring', () => {
  describe('positioning tests', () => {
    ...
    it('Backstab Avoidance', () => {
      ...
      const backstabber = UtilsSDK.applyCardToBoard(
      ...
      const scoreForPositionBehindTargetedBackstabber = positionBackstabAvoidance(
        gameSession,
        golem,
        positionBehindBackstabber,
        bestObjective,
      );
      if (scoreForPositionBehindTargetedBackstabber != null) {
        console.log(
          `Score for ${golem.getName()} at position`,
          positionBehindBackstabber,
          `with bestObjective ${bestObjective.getName()} at position`,
          bestObjective.getPosition(),
          '=',
          scoreForPositionBehindTargetedBackstabber,
        );
      }
      // test 2
      const positionAdjacentNotBehindBackstabber = { x: 2, y: 1 };
      const scoreForPositionAdjacentNotBehindTargetedBackstabber = positionBackstabAvoidance(
        gameSession,
        golem,
        positionAdjacentNotBehindBackstabber,
        bestObjective,
      );
      if (scoreForPositionAdjacentNotBehindTargetedBackstabber != null) {
        console.log(
          ...
          bestObjective.getPosition(),
          '=',
          scoreForPositionAdjacentNotBehindTargetedBackstabber,
        );
      }
      // test 3
      const positionAwayFromBackstabber = { x: 0, y: 1 };
      const scoreForPositionAwayFromTargetedBackstabber = positionBackstabAvoidance(
        gameSession,
        golem,
        positionAwayFromBackstabber,
        bestObjective,
      );
      if (scoreForPositionAwayFromTargetedBackstabber != null) {
        console.log(
          ...
          bestObjective.getPosition(),
          '=',
          scoreForPositionAwayFromTargetedBackstabber,
        );
      }
      // expect
      expect(scoreForPositionBehindTargetedBackstabber)
        .to.be.below(scoreForPositionAdjacentNotBehindTargetedBackstabber)
    ...
    it('Objective Backstab', () => {
      ...
      const scoreForPositionBehindObjective = positionObjectiveBackstab(
        gameSession,
        backstabber,
        positionBehindObjective,
        bestObjective,
      );
      if (scoreForPositionBehindObjective != null) {
        console.log(
          `Score for ${backstabber.getName()} at position`,
          positionBehindObjective,
          `with bestObjective ${bestObjective.getName()} at position`,
          bestObjective.getPosition(),
          '=',
          scoreForPositionBehindObjective,
        );
      }
      // test 2
      const positionAdjacentNotBehindObjective = { x: 2, y: 1 };
      const scoreForPositionAdjacentNotBehindObjective = positionObjectiveBackstab(
        gameSession,
        backstabber,
        positionAdjacentNotBehindObjective,
        bestObjective,
      );
      if (scoreForPositionAdjacentNotBehindObjective != null) {
        console.log(
          ...
          bestObjective.getPosition(),
          '=',
          scoreForPositionAdjacentNotBehindObjective,
        );
      }
      // test 3
      let positionAwayFromObjective = { x: 0, y: 1 };
      const scoreForPositionAwayFromObjective = positionObjectiveBackstab(
        gameSession,
        backstabber,
        positionAwayFromObjective,
        bestObjective,
      );
      if (scoreForPositionAwayFromObjective != null) {
        console.log(
          ...
          bestObjective.getPosition(),
          '=',
          scoreForPositionAwayFromObjective,
        );
      }
      // test 4 - non-backstabber
      positionAwayFromObjective = { x: 0, y: 1 };
      const scoreForPositionNonBackstabber = positionObjectiveBackstab(
        gameSession,
        golem,
        positionAwayFromObjective,
        backstabber,
      ...
      if (scoreForPositionNonBackstabber != null) {
        console.log(
          `Score for ${golem.getName()} at position`,
          positionAwayFromObjective,
          `with bestObjective ${backstabber.getName()} at position`,
          backstabber.getPosition(),
          '=',
          scoreForPositionNonBackstabber,
        );
      }
      // expect
      expect(scoreForPositionBehindObjective)
        .to.be.above(scoreForPositionAdjacentNotBehindObjective)
        .and.to.be.above(scoreForPositionAwayFromObjective);
      expect(scoreForPositionAdjacentNotBehindObjective).to.be.above(
        scoreForPositionAwayFromObjective,
      );
    ...
    it('Distance From Best Objective', () => {
      ...
      const scoreForPositionAdjacentToObjective = positionObjectiveDistanceFromBestObjective(
        gameSession,
        kaidoAssassin,
        positionAdjacentToObjective,
        bestObjective,
      );
      if (scoreForPositionAdjacentToObjective != null) {
        console.log(
          `Score for ${kaidoAssassin.getName()} at position`,
          positionAdjacentToObjective,
          `(distance 1) with bestObjective ${bestObjective.getName()} at position`,
          bestObjective.getPosition(),
          '=',
          scoreForPositionAdjacentToObjective,
        );
      }
      // test 2 - distance 2
      const position2DistanceFromObjective = { x: 2, y: 1 };
      const scoreForPosition2DistanceFromObjective = positionObjectiveDistanceFromBestObjective(
        gameSession,
        kaidoAssassin,
        position2DistanceFromObjective,
        bestObjective,
      );
      if (scoreForPosition2DistanceFromObjective != null) {
        console.log(
          `Score for ${kaidoAssassin.getName()} at position`,
          position2DistanceFromObjective,
          `(distance 2) with bestObjective ${bestObjective.getName()} at position`,
          bestObjective.getPosition(),
          '=',
          scoreForPosition2DistanceFromObjective,
        );
      }
      // test 3 - distance 3
      const position3DistanceFromObjective = { x: 0, y: 4 };
      const scoreForPosition3DistanceFromObjective = positionObjectiveDistanceFromBestObjective(
        gameSession,
        kaidoAssassin,
        position3DistanceFromObjective,
        bestObjective,
      );
      if (scoreForPosition3DistanceFromObjective != null) {
        console.log(
          `Score for ${kaidoAssassin.getName()} at position`,
          position3DistanceFromObjective,
          `(distance 3) with bestObjective ${bestObjective.getName()} at position`,
          bestObjective.getPosition(),
          '=',
          scoreForPosition3DistanceFromObjective,
        );
      }
      // expect
      expect(scoreForPositionAdjacentToObjective)
        .to.be.above(scoreForPosition2DistanceFromObjective)
        .and.to.be.above(scoreForPosition3DistanceFromObjective);
      expect(scoreForPosition2DistanceFromObjective).to.be.above(
        scoreForPosition3DistanceFromObjective,
      );
      ...
      // test 1 - adjacent (distance 1)
      const scoreForPositionRangedAdjacentToObjective = positionObjectiveDistanceFromBestObjective(
        gameSession,
        heartSeeker,
        positionAdjacentToObjective,
        bestObjective,
      ...
      if (scoreForPositionRangedAdjacentToObjective != null) {
        console.log(
          `Score for ${heartSeeker.getName()} at position`,
          positionAdjacentToObjective,
          `(d=1) with bestObjective ${bestObjective.getName()} at position`,
          bestObjective.getPosition(),
          '=',
          scoreForPositionRangedAdjacentToObjective,
        );
      }
      // test 2 - distance 2
      const scoreForPositionRanged2DistanceFromObjective =
        positionObjectiveDistanceFromBestObjective(
          gameSession,
          heartSeeker,
          position2DistanceFromObjective,
          bestObjective,
      ...
      if (scoreForPositionRanged2DistanceFromObjective != null) {
        console.log(
          `Score for ${heartSeeker.getName()} at position`,
          position2DistanceFromObjective,
          `(d=2) with bestObjective ${bestObjective.getName()} at position`,
          bestObjective.getPosition(),
          '=',
          scoreForPositionRanged2DistanceFromObjective,
        );
      }
      // test 3 - distance 3
      const scoreForPositionRanged3DistanceFromObjective =
        positionObjectiveDistanceFromBestObjective(
          gameSession,
          heartSeeker,
          position3DistanceFromObjective,
      ...
      if (scoreForPositionRanged3DistanceFromObjective != null) {
        console.log(
          `Score for ${heartSeeker.getName()} at position`,
          position3DistanceFromObjective,
          `(d=3) with bestObjective ${bestObjective.getName()} at position`,
          bestObjective.getPosition(),
          '=',
          scoreForPositionRanged3DistanceFromObjective,
        );
      }
      // expect
      expect(scoreForPositionRangedAdjacentToObjective)
        .to.be.below(scoreForPositionRanged2DistanceFromObjective)
        .and.to.be.below(scoreForPositionRanged3DistanceFromObjective);
      expect(scoreForPositionRanged2DistanceFromObjective).to.be.below(
        scoreForPositionRanged3DistanceFromObjective,
      );
    });
    ...
    it('Objective Frenzy', () => {
      ...
      const frenzyUnit = UtilsSDK.applyCardToBoard(
      ...
      const scoreForPositionAdjacentToObjectiveAndEnemy = positionObjectiveFrenzy(
        gameSession,
        frenzyUnit,
        positionAdjacentToObjectiveAndEnemy,
        bestObjective,
      );
      if (scoreForPositionAdjacentToObjectiveAndEnemy != null) {
        console.log(
          `Score for ${frenzyUnit.getName()} adjacent to best objective and one other enemey unit =`,
          scoreForPositionAdjacentToObjectiveAndEnemy,
        );
      }
      // test 2 - adjacent to best target only, no others
      const positionAdjacentToObjectiveOnly = { x: 2, y: 1 };
      const scoreForPositionAdjacentToObjectiveOnly = positionObjectiveFrenzy(
        gameSession,
        frenzyUnit,
        positionAdjacentToObjectiveOnly,
        bestObjective,
      );
      if (scoreForPositionAdjacentToObjectiveOnly != null) {
        console.log(
          `Score for ${frenzyUnit.getName()} adjacent to best objective only =`,
          scoreForPositionAdjacentToObjectiveOnly,
        );
      }
      ...
      const scoreForPositionNotAdjacentToObjectiveAdjacentToOther = positionObjectiveFrenzy(
        gameSession,
        frenzyUnit,
        positionNotAdjacentToObjectiveAdjacentToOther,
        bestObjective,
      );
      if (scoreForPositionNotAdjacentToObjectiveAdjacentToOther != null) {
        console.log(
          `Score for ${frenzyUnit.getName()} adjacent to NON-objective enemy unit only =`,
          scoreForPositionNotAdjacentToObjectiveAdjacentToOther,
        );
      }
      // expect
      expect(scoreForPositionAdjacentToObjectiveAndEnemy)
        .to.be.above(scoreForPositionAdjacentToObjectiveOnly)
    ...
    it('Objective Provoke', () => {
      ...
      const provokeUnit = UtilsSDK.applyCardToBoard(
      ...
      // test 1 - adjacent to best target and 1 additional adjacent enemy
      const positionAdjacentToObjectiveAndEnemy = { x: 3, y: 2 };
      const scoreForPositionAdjacentToObjectiveAndEnemy = positionObjectiveProvoke(
        gameSession,
        provokeUnit,
        positionAdjacentToObjectiveAndEnemy,
        bestObjective,
      ...
      if (scoreForPositionAdjacentToObjectiveAndEnemy != null) {
        console.log(
          `Score for ${provokeUnit.getName()} adjacent to best objective and one other enemey unit =`,
          scoreForPositionAdjacentToObjectiveAndEnemy,
        );
      }
      // test 2 - adjacent to best target only, no others
      const positionAdjacentToObjectiveOnly = { x: 2, y: 1 };
      const scoreForPositionAdjacentToObjectiveOnly = positionObjectiveProvoke(
        gameSession,
        provokeUnit,
        positionAdjacentToObjectiveOnly,
        bestObjective,
      ...
      if (scoreForPositionAdjacentToObjectiveOnly != null) {
        console.log(
          `Score for ${provokeUnit.getName()} adjacent to best objective only =`,
          scoreForPositionAdjacentToObjectiveOnly,
        );
      }
      // test 3 - not adjacent to objective, but adjacent to non-objective enemy
      const positionNotAdjacentToObjectiveAdjacentToOther = { x: 4, y: 4 };
      const scoreForPositionNotAdjacentToObjectiveAdjacentToOther = positionObjectiveProvoke(
        gameSession,
        provokeUnit,
        positionNotAdjacentToObjectiveAdjacentToOther,
        bestObjective,
      ...
      if (scoreForPositionNotAdjacentToObjectiveAdjacentToOther != null) {
        console.log(
          `Score for ${provokeUnit.getName()} adjacent to NON-objective enemy unit, 3 distance from objective =`,
          scoreForPositionNotAdjacentToObjectiveAdjacentToOther,
        );
      }
      // expect
    ...
    it('Proximity to Enemies', () => {
      ...
      const RockPulverizerDamaged = UtilsSDK.applyCardToBoard(
        { id: SDK.Cards.Neutral.RockPulverizer },
        0,
        1,
        gameSession.getPlayer1Id(),
      ...
      let scoreForPositionAdjacentToObjectiveAndEnemy = positionProximityToEnemies(
        gameSession,
        RockPulverizer,
        positionAdjacentToObjectiveAndEnemy,
        bestObjective,
      ...
      if (scoreForPositionAdjacentToObjectiveAndEnemy != null) {
        console.log(
          `Score for ${RockPulverizer.getName()} adjacent to best objective and one other enemey unit =`,
          scoreForPositionAdjacentToObjectiveAndEnemy,
        );
      }
      // test 2 - adjacent to best target only, no others
      let positionAdjacentToObjectiveOnly = { x: 2, y: 1 };
      let scoreForPositionAdjacentToObjectiveOnly = positionProximityToEnemies(
        gameSession,
        RockPulverizer,
        positionAdjacentToObjectiveOnly,
        bestObjective,
      ...
      if (scoreForPositionAdjacentToObjectiveOnly != null) {
        console.log(
          `Score for ${RockPulverizer.getName()} adjacent to best objective only =`,
          scoreForPositionAdjacentToObjectiveOnly,
        );
      }
      // test 3 - not adjacent to anything
      let positionNotAdjacentToObjectiveAdjacentToOther = { x: 0, y: 4 };
      let scoreForPositionNotAdjacentToObjectiveAdjacentToOther = positionProximityToEnemies(
        gameSession,
        RockPulverizer,
        positionNotAdjacentToObjectiveAdjacentToOther,
        bestObjective,
      ...
      if (scoreForPositionNotAdjacentToObjectiveAdjacentToOther != null) {
        console.log(
          `Score for ${RockPulverizer.getName()} adjacent to nothing =`,
          scoreForPositionNotAdjacentToObjectiveAdjacentToOther,
        );
      }
      // expect
      expect(scoreForPositionAdjacentToObjectiveAndEnemy).to.be.above(
        scoreForPositionAdjacentToObjectiveOnly,
      );
      expect(scoreForPositionNotAdjacentToObjectiveAdjacentToOther).to.be.equal(0);
      //+++++++++++++++++++
      ...
      scoreForPositionAdjacentToObjectiveAndEnemy = positionProximityToEnemies(
        gameSession,
        RockPulverizerDamaged,
        positionAdjacentToObjectiveAndEnemy,
        bestObjective,
      );
      if (scoreForPositionAdjacentToObjectiveAndEnemy != null) {
        console.log(
          `Score for ${RockPulverizerDamaged.getName()} adjacent to best objective and one other enemey unit =`,
          scoreForPositionAdjacentToObjectiveAndEnemy,
        );
      }
      // test 2 - adjacent to best target only, no others
      positionAdjacentToObjectiveOnly = { x: 2, y: 1 };
      scoreForPositionAdjacentToObjectiveOnly = positionProximityToEnemies(
        gameSession,
        RockPulverizerDamaged,
        positionAdjacentToObjectiveOnly,
        bestObjective,
      ...
      if (scoreForPositionAdjacentToObjectiveOnly != null) {
        console.log(
          `Score for ${RockPulverizerDamaged.getName()} adjacent to best objective only =`,
          scoreForPositionAdjacentToObjectiveOnly,
        );
      }
      // test 3 - not adjacent to objective, but adjacent to non-objective enemy
      positionNotAdjacentToObjectiveAdjacentToOther = { x: 0, y: 4 };
      scoreForPositionNotAdjacentToObjectiveAdjacentToOther = positionProximityToEnemies(
        gameSession,
        RockPulverizerDamaged,
        positionNotAdjacentToObjectiveAdjacentToOther,
        bestObjective,
      ...
      if (scoreForPositionNotAdjacentToObjectiveAdjacentToOther != null) {
        console.log(
          `Score for ${RockPulverizerDamaged.getName()} adjacent to NON-objective enemy unit, 3 distance from objective =`,
          scoreForPositionNotAdjacentToObjectiveAdjacentToOther,
        );
      }
      // expect
      expect(scoreForPositionAdjacentToObjectiveOnly)
        .to.be.above(scoreForPositionAdjacentToObjectiveAndEnemy)
    ...
    it('Proximity to Generals', () => {
      ...
      let scoreForPositionAdjacentToEnemyGeneral = positionProximityToGenerals(
        gameSession,
        RockPulverizer,
        positionAdjacentToEnemyGeneral,
      );
      if (scoreForPositionAdjacentToEnemyGeneral != null) {
        console.log(
          `Score for ${RockPulverizer.getName()} adjacent to enemy general, 7 from friendly general =`,
          scoreForPositionAdjacentToEnemyGeneral,
        );
      }
      // test 2 - distance 4 from enemy general, adjacent to friendly general
      const position4AwayFromEnemyGeneral = { x: 1, y: 2 };
      let scoreForPosition4AwayFromEnemyGeneral = positionProximityToGenerals(
        gameSession,
        RockPulverizer,
        position4AwayFromEnemyGeneral,
      );
      if (scoreForPosition4AwayFromEnemyGeneral != null) {
        console.log(
          `Score for ${RockPulverizer.getName()} 7 distance from enemy general, adjacent to friendly general =`,
          scoreForPosition4AwayFromEnemyGeneral,
        );
      }
      // expect
      expect(scoreForPositionAdjacentToEnemyGeneral).to.be.above(
        scoreForPosition4AwayFromEnemyGeneral,
      );
      ...
      scoreForPositionAdjacentToEnemyGeneral = positionProximityToGenerals(
        gameSession,
        RockPulverizer,
        positionAdjacentToEnemyGeneral,
      );
      ...
      // test 2 - distance 4 from enemy general, adjacent to friendly general
      scoreForPosition4AwayFromEnemyGeneral = positionProximityToGenerals(
        gameSession,
        RockPulverizer,
        position4AwayFromEnemyGeneral,
      );
    ...
    it('Shadow Tile Avoidance', () => {
      ...
      const SilverguardKnightDamaged = UtilsSDK.applyCardToBoard(
        { id: SDK.Cards.Faction1.SilverguardKnight },
        3,
        3,
        gameSession.getPlayer2Id(),
      ...
      UtilsSDK.executeActionWithoutValidation(
        new SDK.PlayCardFromHandAction(gameSession, gameSession.getPlayer1Id(), 3, 3, 0),
      );
      // test 1 - 4 shadow creep dmg to a 5 hp unit
      const positionInsideShadowCreep = { x: 3, y: 3 };
      const positionOutsideShadowCreep = { x: 7, y: 4 };
      const scoreForPositionInsideShadowCreep5hp = positionShadowTileAvoidance(
        gameSession,
        SilverguardKnight,
        positionInsideShadowCreep,
      );
      if (scoreForPositionInsideShadowCreep5hp != null) {
        console.log(
          `Score for 4 shadow creep dmg to ${SilverguardKnight.getName()} with 5 hp =`,
          scoreForPositionInsideShadowCreep5hp,
        );
      }
      const scoreForPositionOutsideShadowCreep5hp = positionShadowTileAvoidance(
        gameSession,
        SilverguardKnight,
        positionOutsideShadowCreep,
      );
      if (scoreForPositionOutsideShadowCreep5hp != null) {
        console.log(
          `Score for 0 shadow creep dmg to ${SilverguardKnight.getName()} with 5 hp =`,
          scoreForPositionOutsideShadowCreep5hp,
        );
      }
      const scoreForPositionInsideShadowCreep4hp = positionShadowTileAvoidance(
        gameSession,
        SilverguardKnightDamaged,
        positionInsideShadowCreep,
      );
      if (scoreForPositionInsideShadowCreep4hp != null) {
        console.log(
          `Score for 4 shadow creep dmg to ${SilverguardKnightDamaged.getName()} with 4 hp =`,
          scoreForPositionInsideShadowCreep4hp,
        );
      }
      // expect
    ...
    it('Zeal', () => {
      ...
      const scoreForPositionAdjacentToGeneral = positionZeal(
        gameSession,
        WindbladeAdept,
        positionAdjacentToGeneral,
      );
      if (scoreForPositionAdjacentToGeneral != null) {
        console.log(
          `Score for ${WindbladeAdept.getName()} adjacent to general =`,
          scoreForPositionAdjacentToGeneral,
        );
      }
      // test 2 - not
      const positionNotAdjacentToGeneral = { x: 7, y: 2 };
      const scoreForPositionNotAdjacentToGeneral = positionZeal(
        gameSession,
        WindbladeAdept,
        positionNotAdjacentToGeneral,
      );
      if (scoreForPositionNotAdjacentToGeneral != null) {
        console.log(
          `Score for ${WindbladeAdept.getName()} not adjacent to general =`,
          scoreForPositionNotAdjacentToGeneral,
        );
      }
      // expect
