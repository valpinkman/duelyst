...
const AchievementsModule = require('apps/server/lib/data_access/achievements');
...
describe('referrals module', () => {
  ...
  describe('referral program', () => {
    ...
    beforeAll(() => {
      ...
      return Promise.all([
        UsersModule.createNewUser(username1, 'testpassword', 'kumite14'),
        UsersModule.createNewUser(username2, 'testpassword', 'kumite14'),
        UsersModule.createNewUser(username3, 'testpassword', 'kumite14'),
        UsersModule.createNewUser(username4, 'testpassword', 'kumite14'),
    ...
    describe('markUserAsReferredByFriend()', () => {
      it('expect to fail to mark a user as referred with invalid user id', () =>
        ReferralsModule.markUserAsReferredByFriend(newUserId, 'invalidUserId-C$N#Qrnv')
          .then((response) => {
      ...
      it('expect to be able to mark a user as referred by a friend and gain 100 starting GOLD', () =>
        ReferralsModule.markUserAsReferredByFriend(newUserId, userId)
          .then((response) => {
      ...
      it('expect to NOT be able to set a referrer for a user that registered over 30 days ago', () =>
        ReferralsModule.markUserAsReferredByFriend(oldUserId, userId)
          .then((response) => {
            expect(response).to.not.exist;
          })
          .catch((error) => {
      ...
      it('expect to NOT be able to set a referrer for a user that has played a ranked game', () =>
        ReferralsModule.markUserAsReferredByFriend(userWithRankedGame, userId)
          .then((response) => {
            expect(response).to.not.exist;
          })
          .catch((error) => {
      ...
      it("expect to be fail to change a users referrer after it's been set", () =>
        ReferralsModule.markUserAsReferredByFriend(newUserId, userId)
          .then((response) => {
            expect(response).to.not.exist;
          })
      ...
      it('expect to backfill referral events for users who have already achieved certain milestones but marked late', () =>
        ReferralsModule.markUserAsReferredByFriend(oldUserWithEventsId, newUserId)
          .then((response) => {
            expect(response).to.exist;
            return Promise.all([
      ...
      it('expect to NOT be able to mark self as referred by someone you referred', () =>
        ReferralsModule.markUserAsReferredByFriend(userId, newUserId)
          .then((response) => {
            expect(response).to.not.exist;
          })
          .catch((error) => {
      ...
      it('expect to NOT be able to mark self as referred by self', () =>
        ReferralsModule.markUserAsReferredByFriend(userId, userId)
          .then((response) => {
            expect(response).to.not.exist;
          })
          .catch((error) => {
    ...
    describe('processReferralEventForUser()', () => {
      it('expect to fail to process anything for an user with no referrer', () =>
        ReferralsModule.processReferralEventForUser(userId)
          .then((response) => {
            expect(response).to.not.exist;
          })
          .catch((error) => {
      ...
      it('expect to update referral event for a user with a referrer', () =>
        ReferralsModule.processReferralEventForUser(newUserId, userId, 'silver')
          .then((response) => {
            expect(response).to.exist;
            return DuelystFirebase.connect().getRootRef();
          })
      ...
      it('expect referral row record "level" to iterate upwards with events', () =>
        ReferralsModule.processReferralEventForUser(newUserId, userId, 'gold')
          .then((response) => {
            expect(response).to.exist;
            return DuelystFirebase.connect().getRootRef();
          })
      ...
      it('expect off master flow events (such as "purchase") to not change referral record level', () =>
        ReferralsModule.processReferralEventForUser(newUserId, userId, 'purchase')
          .then((response) => {
            expect(response).to.exist;
            return DuelystFirebase.connect().getRootRef();
          })
          ...
          .then(([referralRows, referralEventRows]) => {
            ...
            expect(_.pluck(referralEventRows, 'event_type')).to.have.members([
              'silver',
              'gold',
              'purchase',
            ]);
    ...
    describe('claimReferralRewards()', () => {
      ...
      beforeAll(() => {
        ...
        return UsersModule.createNewUser(username, 'testpassword', 'kumite14').then((userId) => {
          noReferralUserId = userId;
        });
      });

      it("expect to fail to claim anything if you don't have any rewards waiting", () =>
        ReferralsModule.claimReferralRewards(noReferralUserId)
          .then((response) => {
            expect(response).to.not.exist;
          })
          .catch((error) => {
      ...
      it('expect to be able to claim referral rewards and receive rewards', () =>
        ReferralsModule.claimReferralRewards(userId)
          .then((response) => {
            expect(response).to.exist;
            return Promise.all([
              ...
              knex('user_rewards')
                ...
                .andWhere('reward_category', 'referral')
                .select(),
            ]);
          })
      ...
      it('expect claiming again with no new rewards added to ERROR out', () =>
        ReferralsModule.claimReferralRewards(userId)
          .then((response) => {
            expect(response).to.not.exist;
          })
      ...
      it('expect first purchase by a referral achievement to have awarded ROOK emote', () =>
        ...
        AchievementsModule.updateAchievementsProgressWithReferralEvent(userId, 'purchase')
          ...
          .then(([achievementRows, emoteRows]) => {
            ...
            expect(parseInt(emoteRows[0].cosmetic_id, 10)).to.equal(
              SDK.CosmeticsLookup.Emote.OtherRook,
            );
          }));
    });
  });
