...
const CardSetLookup = require('@duelyst/sdk/cards/cardSetLookup');
...
const MigrationsModule = require('apps/server/lib/data_access/migrations');
const GauntletModule = require('apps/server/lib/data_access/gauntlet');
...
describe('migrations module', () => {
  ...
  describe('checkIfUserNeedsMigrateEmotes20160708()', () => {
    it('expect a player to need emote migration if their last session was last year and they have no cosmetics', () =>
      SyncModule.wipeUserData(user2Id)
        .then(() =>
          knex('users')
            .where('id', user2Id)
            .update('last_session_at', moment.utc('2015-02-02').toDate()),
        )
    ...
    it('expect a player to not need emote migration if their last session is after the deadline and they have no cosmetics', () =>
      SyncModule.wipeUserData(user2Id)
        .then(() =>
          knex('users')
            .where('id', user2Id)
            .update('last_session_at', moment.utc('2017-02-02').toDate()),
        )
        .then(() => knex('users').first().where('id', user2Id))
        .then((userRow) => MigrationsModule.checkIfUserNeedsMigrateEmotes20160708(userRow))
        .then((userNeedsMigrateEmotes) => {
    ...
    it('expect a player to not need emote migration if their last session was last year and they have cosmetics', () =>
      SyncModule.wipeUserData(user2Id)
        .then(() =>
          knex('users')
            .where('id', user2Id)
  ...
  describe('userMigrateEmotes20160708()', () => {
    ...
    it('expect a user to have all original general emotes after migration', () =>
      SyncModule.wipeUserData(user2Id)
    ...
    it('expect a user to have all original general emotes after migration and any others they already had', () =>
      SyncModule.wipeUserData(user2Id)
        .then(() => MigrationsModule.userMigrateEmotes20160708(user2Id))
        .then(() => {
          const txPromise = knex.transaction((tx) =>
        ...
        .then((userCosmeticRows) => {
          ...
          const expectedEmoteIds = MigrationsModule.EMOTE_IDS_PRE_COSMETICS_20160708.concat([
            SDK.CosmeticsLookup.Emote.OtherSnowChaserHoliday2015,
          ]);
  ...
  describe('checkIfUserNeedsPrismaticBackfillReward()', () => {
    it('expect a player to need prismatic backfill they have no last_session_version', () =>
      SyncModule.wipeUserData(userId)
        .then(() => knex('users').where('id', userId).update('last_session_version', null))
    ...
    it('expect a player to need prismatic backfill if their last session is before the version deadline', () =>
      SyncModule.wipeUserData(userId)
        .then(() => knex('users').where('id', userId).update('last_session_version', '1.65.12'))
        .then(() => knex('users').first().where('id', userId))
        .then((userRow) => MigrationsModule.checkIfUserNeedsPrismaticBackfillReward(userRow))
        .then((userNeedsPrismaticBackfill) => {
          expect(userNeedsPrismaticBackfill).to.equal(true);
    ...
    it('expect a player to not need prismatic backfill if their last session hotfix version is after 1.73.0', () =>
      SyncModule.wipeUserData(userId)
        .then(() => knex('users').where('id', userId).update('last_session_version', '1.73.1'))
        .then(() => knex('users').first().where('id', userId))
        .then((userRow) => MigrationsModule.checkIfUserNeedsPrismaticBackfillReward(userRow))
        .then((userNeedsPrismaticBackfill) => {
    ...
    it('expect a player to not need prismatic backfill if their last session minor version is after 1.73.0', () =>
      SyncModule.wipeUserData(userId)
        .then(() => knex('users').where('id', userId).update('last_session_version', '1.74.0'))
        .then(() => knex('users').first().where('id', userId))
        .then((userRow) => MigrationsModule.checkIfUserNeedsPrismaticBackfillReward(userRow))
        .then((userNeedsPrismaticBackfill) => {
          expect(userNeedsPrismaticBackfill).to.equal(false);
  ...
  describe('checkIfUserNeedsChargeCountsMigration()', () => {
    it('expect a player to need charge counts migrated if they have no last_session_version', () =>
      SyncModule.wipeUserData(userId)
        .then(() =>
        ...
        .then(() => knex('users').first().where('id', userId))
        .then((userRow) => MigrationsModule.checkIfUserNeedsChargeCountsMigration(userRow))
        .then((needsMigration) => {
          expect(needsMigration).to.equal(true);
        }));

    it('expect a player to need charge counts migrated if their last session is far before the version deadline', () =>
      SyncModule.wipeUserData(userId)
        .then(() =>
          knex('users').where('id', userId).update({
    ...
    it('expect a player to not need charge counts migrated if their last session hotfix version is before 1.74.11', () =>
      SyncModule.wipeUserData(userId)
        .then(() =>
          knex('users').where('id', userId).update({
    ...
    it('expect a player to not need charge counts migrated if their last session hotfix version is after 1.74.11', () =>
      SyncModule.wipeUserData(userId)
        .then(() =>
          knex('users').where('id', userId).update({
    ...
    it('expect a player to not need prismatic backfill if their last session version is at least 1.75.0', () =>
      SyncModule.wipeUserData(userId)
        .then(() =>
          knex('users').where('id', userId).update({
    ...
    it('expect a player to not need prismatic backfill if their last session version is before 1.74.0 but they have no purchases', () =>
      SyncModule.wipeUserData(userId)
        .then(() =>
          knex('users').where('id', userId).update({
  ...
  describe('userCreateChargeCountsMigration()', () => {
    it('expect a player that did not purchase a starter bundle to have nothing in their firebase purchase count tree', () =>
      SyncModule.wipeUserData(userId)
        .then(() =>
          knex('users').where('id', userId).update({
            last_session_version: null,
        ...
        .then(() => MigrationsModule.userCreateChargeCountsMigration(userId))
    ...
    it('expect a player that purchased a starter bundle to correctly mark it into purchase count tree', () =>
      SyncModule.wipeUserData(userId)
        .then(() =>
          knex('users').where('id', userId).update({
            last_session_version: null,
  ...
  describe('checkIfUserNeedsIncompleteGauntletRefund()', () => {
    it('expect a player with an old gauntlet run that is complete to not need a refund', () =>
      SyncModule.wipeUserData(userId)
        .then(() =>
          knex('users').where('id', userId).update({
            last_session_version: null,
        ...
        .then((ticketId) =>
          GauntletModule.startRun(userId, ticketId, moment.utc('2017-04-02 13:00')),
        )
        .then(() =>
          knex('user_gauntlet_run').where('user_id', userId).update({ is_complete: true }),
        )
        .then(() => knex('users').first().where('id', userId))
    ...
    it('expect a player with a gauntlet run that is not complete and has no faction choices to not need a refund', () =>
      SyncModule.wipeUserData(userId)
        .then(() =>
          knex('users').where('id', userId).update({
            last_session_version: null,
    ...
    it('expect a player with an old gauntlet run that is not complete and has faction choices to need a refund', () =>
      SyncModule.wipeUserData(userId)
        .then(() =>
          knex('users').where('id', userId).update({
            last_session_version: null,
  ...
  describe('userIncompleteGauntletRefund()', () => {
    it('expect a player with an old gauntlet run that is not complete to no longer have a run and to now have an arena ticket', () =>
      SyncModule.wipeUserData(userId)
        .then(() =>
          knex('users').where('id', userId).update({
            last_session_version: null,
