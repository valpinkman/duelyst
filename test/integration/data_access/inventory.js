...
describe('inventory module', () => {
      ...
      sunSister =
        ...
        _.find(
          SDK.GameSession.getCardCaches()
            ...
            .getIsCollectible(true)
            .getCards(),
          (c) => c.getId() === SDK.Cards.Faction1.SunSister,
        );
      expect(sunSister).to.exist;
    });

    it('expect to contain a prismatic Seven Sisters cards in Legendary group', () => {
      let prismaticSunSister = _.find(
        SDK.GameSession.getCardCaches()
          .getRarity(SDK.Rarity.Legendary)
          .getIsCollectible(true)
          .getCardIds(),
        (c) => c === SDK.Cards.Faction1.SunSister + SDK.Cards.Prismatic,
      );
      prismaticSunSister =
        prismaticSunSister ||
        _.find(
          SDK.GameSession.getCardCaches()
            ...
            .getIsCollectible(true)
            .getCards(),
          (c) => c.getId() === SDK.Cards.Faction1.SunSister + SDK.Cards.Prismatic,
        );
  ...
  describe('getAllNonUnlockableCollectibleSdkCards()', () => {
    it('expect NOT to contain any ACHIEVEMENT cards', () => {
      const anyAchievementCard = _.find(
        SDK.GameSession.getCardCaches().getIsCollectible(true).getIsUnlockable(false).getCards(),
        (c) => c.getIsUnlockableWithAchievement(),
      );
    ...
    it('expect NOT to contain prismatic ACHIEVEMENT cards', () => {
      const anyPrismaticAchievementCard = _.find(
        SDK.GameSession.getCardCaches().getIsCollectible(true).getIsUnlockable(false).getCards(),
        (c) => c.getIsUnlockableWithAchievement() && c.getIsUnlockablePrismaticWithAchievement(),
      );
  ...
  describe('buyBoosterPacksWithGold()', () => {
    ...
    it('expect NOT to be able to buy booster packs with NO gold', () =>
      InventoryModule.buyBoosterPacksWithGold(userId, 1, SDK.CardSet.Core)
        .then((result) => {
      ...
      knex('users')
        ...
        .then((rootRef) =>
          Promise.all([
            ...
            knex
              ...
              .where({ user_id: userId, memo: `spirit orb ${_chainState.spiritOrbId}` }),
        ...
        .then(
          ([
            ...
            spiritOrbRows,
            currencyLogRow,
            firebaseWalletSnapshot,
            firebaseBoostersSnapshot,
          ]) => {
        ...
        .then((rootRef) =>
          Promise.all([
            ...
            knex
              ...
              .where({
                user_id: userId,
                memo: `spirit orb ${_chainState.boosterIds[_chainState.boosterIds.length - 1]}`,
              }),
            FirebasePromises.once(
              rootRef.child('user-inventory').child(userId).child('wallet'),
              'value',
    ...
    it('expect NOT to be able to buy booster packs with INSUFFICIENT gold', () =>
      knex('users')
        .where('id', userId)
        .update({ wallet_gold: 20 })
  ...
  */
  ...
  describe('addRemainingOrbsForCardSetToUser', () => {
    it('expect to be able to add a full set of orbs to user with no orbs for that set, expect no gold to be awarded', () =>
    ...
    it('expect to be able to add a full set of orbs to user with 3 orbs for that set, expect 900 gold to be awarded', () =>
      SyncModule.wipeUserData(userId)
        .then(() => {
          const txPromise = knex.transaction((tx) =>
            Promise.all([
    ...
    it('expect to be not fail when trying to add remaining set of orbs to user with max orbs for that set, expect no gold to be awarded', () =>
      SyncModule.wipeUserData(userId)
        .then(() => {
          const txPromise = knex.transaction((tx) =>
            Promise.all([
  ...
  describe('buyRemainingSpiritOrbsWithSpirit', () => {
    it('expect to fail to buy remaining orbs with spirit with insufficient spirit', () =>
      SyncModule.wipeUserData(userId)
        .then(() => {
          const txPromise = knex.transaction((tx) => {
    ...
    it('expect to fail to buy remaining orbs for set with no spirit cost', () =>
      SyncModule.wipeUserData(userId)
        .then(() => {
          const txPromise = knex.transaction((tx) =>
            InventoryModule.giveUserSpirit(txPromise, tx, userId, 4000, 'qa gift'),
          );
          return txPromise;
        })
        .then(() => {
    ...
    it('expect to be able to buy entire set of spirit orbs with sufficient spirit', () => {
      const fullSetSpiritCost =
        SDK.CardSetFactory.cardSetForIdentifier(orbTestCardSet).fullSetSpiritCost;
      return SyncModule.wipeUserData(userId)
        .then(() => {
          const txPromise = knex.transaction((tx) =>
        ...
        .then(() => {
          const txPromise = knex.transaction((tx) =>
            InventoryModule.buyRemainingSpiritOrbsWithSpirit(userId, orbTestCardSet),
          );
          return txPromise;
        })
        .then((result) => {
  ...
  */
  ...
  describe('unlockBoosterPack()', () => {
    ...
    it('expect NOT to be able to unlock a booster that does not belong to you', () =>
      knex('user_spirit_orbs')
        .insert({
          id: 'valid-pack-id',
          user_id: 'some-other-test-user',
        })
        .then(() => InventoryModule.unlockBoosterPack(userId, 'valid-pack-id'))
        .then((result) => {
          expect(result).to.not.exist;
        })
        .catch((error) => {
    ...
    it('expect cards you received from the booster to be marked as NEW and UNREAD', () =>
    ...
    it('expect unlocking a booster to mark it as used', () =>
      DuelystFirebase.connect()
        .getRootRef()
        .then((rootRef) =>
          Promise.all([
    ...
    it('expect that unlocking 5 packs concurrently works', () =>
      knex('users')
        .where('id', userId)
        .update({ wallet_gold: 500 })
  ...
  describe('giveUserCosmeticId()', () => {
    ...
    it('expect a user to be able to receive a cosmetic by id', () =>
    ...
    it('expect a user to receive spirit the second time they receive the same cosmetic by id', () =>
      knex('user_cosmetic_inventory')
        .where('user_id', userId)
        .delete()
  ...
  describe('craftCard()', () => {
    ...
    describe('when ALL_CARDS_AVAILABLE is FALSE', () => {
          ...
          .then((fbRootRef) =>
            Promise.all([
              FirebasePromises.remove(
                fbRootRef.child('user-inventory').child(userId).child('card-collection'),
              ),
      ...
      it('expect NOT to be able to craft a COMMON card with 0 spirit', () =>
      ...
      it('expect to be able to craft a COMMON card with 40 spirit in wallet', () =>
        knex('users')
          .where('id', userId)
          .update({
            wallet_spirit: 40,
      ...
      it('expect the crafted card to appear in your inventory as new/unread', () =>
        DuelystFirebase.connect()
          .getRootRef()
          .then((rootRef) =>
            Promise.all([
              knex
                .select()
                .from('user_cards')
                .where({ user_id: userId, card_id: SDK.Cards.Faction1.Lightchaser }),
              knex
                .select()
                .from('user_card_log')
                .where({ user_id: userId, card_id: SDK.Cards.Faction1.Lightchaser }),
      ...
      it('expect to be left with 0 spirit in wallet', () =>
        knex('users')
          .first()
          .where('id', userId)
          .then((userRow) => {
      ...
      it('expect NOT to be able to craft a RARE card with 40 spirit in wallet', () =>
        knex('users')
          .where('id', userId)
          .update({
            wallet_spirit: 40,
      ...
      it('expect to be able to craft a RARE card with 100 spirit in wallet', () =>
        knex('users')
          .where('id', userId)
          .update({
            wallet_spirit: 100,
          ...
          .then((rootRef) =>
            Promise.all([
              knex
                .select()
                .from('user_cards')
                .where({ user_id: userId, card_id: SDK.Cards.Faction1.IroncliffeGuardian }),
              knex
                .select()
                .from('user_card_log')
                .where({ user_id: userId, card_id: SDK.Cards.Faction1.IroncliffeGuardian }),
          ...
          .then(([cardCountRows, cardLogRows, cardCollection, fbCardCollection]) => {
            ...
            expect(cardCollection.cards[SDK.Cards.Faction1.IroncliffeGuardian].is_unread).to.equal(
              true,
            );
            expect(cardCollection.cards[SDK.Cards.Faction1.IroncliffeGuardian].is_new).to.equal(
              true,
            );
            ...
            expect(
              fbCardCollection.val()[SDK.Cards.Faction1.IroncliffeGuardian].is_unread,
            ).to.equal(true);
            expect(fbCardCollection.val()[SDK.Cards.Faction1.IroncliffeGuardian].is_new).to.equal(
              true,
            );
          }));

      it('expect NOT to be able to craft a BASIC card', () =>
        knex('users')
          .where('id', userId)
          .update({
            wallet_spirit: 100,
      ...
      it('expect NOT to be able to craft a normal ACHIEVEMENT card', () =>
        knex('users')
          .where('id', userId)
          .update({
            wallet_spirit: 900,
      ...
      it('expect NOT to be able to craft a card that becomes available in the future', () => {
        const sunstoneTemplar = _.find(
          SDK.GameSession.getCardCaches().getIsCollectible(true).getCards(),
          (card) => card.id === SDK.Cards.Faction1.SunstoneTemplar,
        );
        ...
        return knex('users')
          ...
          .catch((error) => {
            ...
            expect(error.message).to.equal(
              `Could not craft card ${SDK.Cards.Faction1.SunstoneTemplar}. It's not yet available.`,
            );
          });
      });
      ...
      it('expect to be able to craft a card that is NOW available', () => {
        ...
        return knex('users')
          ...
          .then((rootRef) =>
            Promise.all([
              knex
                .select()
                .from('user_cards')
                .where({ user_id: userId, card_id: SDK.Cards.Faction1.SunstoneTemplar }),
              knex
                .select()
                .from('user_card_log')
                .where({ user_id: userId, card_id: SDK.Cards.Faction1.SunstoneTemplar }),
          ...
          .then(([cardCountRows, cardLogRows, cardCollection, fbCardCollection]) => {
            ...
            expect(cardCollection.cards[SDK.Cards.Faction1.SunstoneTemplar].is_unread).to.equal(
              true,
            );
            ...
            expect(fbCardCollection.val()[SDK.Cards.Faction1.SunstoneTemplar].is_unread).to.equal(
              true,
            );
            expect(fbCardCollection.val()[SDK.Cards.Faction1.SunstoneTemplar].is_new).to.equal(
              true,
            );
          });
      });

      it('expect to not be able to craft a card with a skin applied', () =>
        knex('users')
          .where('id', userId)
          .update({
            wallet_spirit: 10000,
          ...
          .then(() =>
            InventoryModule.craftCard(
              userId,
              SDK.Cards.getSkinnedCardId(SDK.Cards.Faction2.General, 1),
            ),
          )
          .then((result) => {
            expect(result).to.not.exist;
      ...
      it('expect to not be able to craft a prismatic card with a skin applied', () =>
        knex('users')
          .where('id', userId)
          .update({
            wallet_spirit: 10000,
          ...
          .then(() =>
            InventoryModule.craftCard(
              userId,
              SDK.Cards.getSkinnedCardId(SDK.Cards.Faction2.GeneralPrismatic, 1),
            ),
          )
          .then((result) => {
            expect(result).to.not.exist;
  ...
  describe('disenchantCard()', () => {
    ...
    it('expect NOT to be able to disenchant a COMMON card you do not own', () =>
      InventoryModule.disenchantCards(userId, [SDK.Cards.Faction1.Lightchaser])
        .then((result) => {
          expect(result).to.not.exist;
        })
        .catch((error) => {
    ...
    it('expect to be able to disenchant a COMMON card and receive its spirit reward', () =>
      knex('users')
        .where('id', userId)
        .update({
        ...
        .then(
          ([userRow, cardCountRows, cardLogRows, cardCollection, fbCardCollection, fbWallet]) => {
    ...
    it('expect to be able to disenchant a set of COMMON,RARE,EPIC, and LEGENDARY cards for 480 spirit', () =>
      knex('users')
        ...
        .then(() =>
          InventoryModule.disenchantCards(userId, [
    ...
    it('expect NOT to be able to disenchant a RARE card you do not own with a COMMON in your inventory', () =>
      knex('users')
        .where('id', userId)
        .update({
          wallet_spirit: 40,
        ...
        .then(() =>
          InventoryModule.disenchantCards(userId, [SDK.Cards.Faction1.IroncliffeGuardian]),
        )
        .then((result) => {
          expect(result).to.not.exist;
        })
    ...
    it('expect inventory to look right after 1/3 copies of a card are disenchanted', () =>
      knex('users')
        .where('id', userId)
        .update({
          wallet_spirit: 200,
    ...
    it('expect inventory to look right after a single card from a larger collection is disechanted', () =>
      InventoryModule.disenchantCards(userId, [SDK.Cards.Faction1.Lightchaser])
        .then(() => DuelystFirebase.connect().getRootRef())
        .then((rootRef) =>
          Promise.all([
    ...
    it('expect to be able to disenchant DUPLICATE cards', () =>
      knex('users')
        .where('id', userId)
        .update({
          wallet_spirit: 160,
    ...
    it('expect LOCKING when disenchant DUPLICATES to work to avoid race conditions', () =>
      knex('users')
        .where('id', userId)
        .update({
          wallet_spirit: 40,
    ...
    it('expect NOT to be able to disenchant an BASIC card', () => {
      const txPromise = knex
        .transaction((tx) =>
          InventoryModule.giveUserCards(txPromise, tx, userId, [SDK.Cards.Spell.Tempest]),
        )
        .then(() => InventoryModule.disenchantCards(userId, [SDK.Cards.Spell.Tempest]))
        .then((result) => {
          expect(result).to.not.exist;
        })
        .catch((error) => {
    ...
    it('expect NOT to be able to disenchant an ACHIEVEMENT card', () => {
      const txPromise = knex
        .transaction((tx) =>
          InventoryModule.giveUserCards(txPromise, tx, userId, [SDK.Cards.Neutral.SwornSister]),
        )
        .then(() => InventoryModule.disenchantCards(userId, [SDK.Cards.Neutral.SwornSister]))
        .then((result) => {
          expect(result).to.not.exist;
        })
        .catch((error) => {
    ...
    it('expect to NOT be able to disenchant a card with a skin applied', () => {
      ...
      const txPromise = knex
        .transaction((tx) => InventoryModule.giveUserCosmeticId(txPromise, tx, userId, skinId))
  ...
  describe('giveUserNewPurchasableCosmetic()', () => {
    it('to be able to give each permutation of rarity and/or cosmetic type', () => {
      ...
      permutations = _.filter(
        permutations,
        (permutation) =>
          _.find(
            emptyPermutations,
            (emptyPermutation) =>
              emptyPermutation[0] === permutation[0] && emptyPermutation[1] === permutation[1],
          ) == null,
      );
      ...
      return PromiseUtils.each(
        permutations,
        (params) => {
          const cosmeticsMatchingParams = _.filter(
            SDK.CosmeticsFactory.getAllCosmetics(),
            (cosmeticData) => {
          ...
          return SyncModule.wipeUserData(userId)
            ...
            .then(() => knex('user_cosmetic_inventory').where('user_id', userId))
            .then((userCosmeticsRows) => {
              expect(userCosmeticsRows).to.exist;
              expect(userCosmeticsRows.length).to.equal(1);

              const cosmeticRow = userCosmeticsRows[0];
              const cosmeticData = SDK.CosmeticsFactory.cosmeticForIdentifier(
                cosmeticRow.cosmetic_id,
              );
        ...
        PromiseUtils.each(
          ...
          (cosmeticData) => {
            ...
            const retPromise = txPromise.then((rewardData) => {
              const cosmeticsData = SDK.CosmeticsFactory.cosmeticForIdentifier(
  ...
  describe('giveUserCards()', () => {
    ...
    it('expect giving [] empty array of cards to do nothing', () => {
      const txPromise = knex
        .transaction((tx) => InventoryModule.giveUserCards(txPromise, tx, userId, []))
        .then(() => DuelystFirebase.connect().getRootRef())
        .then((rootRef) =>
          Promise.all([
            knex.select().from('user_cards').where({ user_id: userId }),
      ...
      return txPromise
        ...
        .then((rootRef) =>
          Promise.all([
            ...
            FirebasePromises.once(
              rootRef.child('user-inventory').child(userId).child('codex'),
              'value',
            ),
          ]),
        )
  ...
  describe('markCardAsReadInUserCollection()', () => {
      ...
      DuelystFirebase.connect()
        ...
        .then((fbRootRef) =>
          Promise.all([
            ...
            FirebasePromises.remove(
              fbRootRef.child('user-inventory').child(userId).child('card-lore'),
            ),
            knex('user_cards').where('user_id', userId).delete(),
            knex('user_card_log').where('user_id', userId).delete(),
            knex('user_card_collection').where('user_id', userId).delete(),
        ...
        .then(() => {
          const txPromise = knex.transaction((tx) =>
            InventoryModule.giveUserCards(txPromise, tx, userId, [20157, 10974]),
          );
          return txPromise;
        }),
    );

    it('to mark a card as read', () =>
      InventoryModule.markCardAsReadInUserCollection(userId, 20157)
        .then(() => DuelystFirebase.connect().getRootRef())
        .then((rootRef) =>
          Promise.all([
    ...
    it("to mark a card's lore as read", () =>
      InventoryModule.markCardLoreAsReadInUserCollection(userId, 20157)
        .then(() => DuelystFirebase.connect().getRootRef())
        .then((rootRef) =>
          Promise.all([
            knex
              ...
              .where({ user_id: userId, card_id: 20157 }),
            FirebasePromises.once(
              rootRef.child('user-inventory').child(userId).child('card-lore'),
              'value',
            ),
          ]),
        )
    ...
    it('to debit spirit correctly', () => {
      ...
      return knex
        ...
        .then((userRow) => {
          ...
          const txPromise = knex.transaction((tx) => {
            InventoryModule.giveUserSpirit(txPromise, tx, userId, spiritToCredit)
              .then(() =>
                InventoryModule.debitSpiritFromUser(txPromise, tx, userId, -1 * spiritToDebit),
              )
              .then(tx.commit)
              .catch(tx.rollback);
          });
        ...
        .then(([userRow, walletRef]) => {
          ...
          expect(walletRef.val().spirit_amount).to.equal(
            spiritBefore + spiritToCredit - spiritToDebit,
          ); // expected 85 to equal 95//95 is correct
        });
    });
  ...
  describe('softWipeUserCardInventory()', () => {
    ...
    describe('if a user attempts to soft wipe after a wipe period has expired', () => {
      it('should throw a BadRequestError', () =>
        InventoryModule.softWipeUserCardInventory(
          userId,
          moment(InventoryModule.SOFTWIPE_AVAILABLE_UNTIL).add(1, 'day'),
        )
          .then((r) => {
            expect(r).to.not.exist;
          })
    ...
    describe('if a user has not opened any orbs and has no cards', () => {
      ...
      it('should not make any changes to the account and throw a BadRequestError', () =>
        InventoryModule.softWipeUserCardInventory(userId)
          .then((r) => {
            expect(r).to.not.exist;
          })
          .catch((e) => {
    ...
    describe('if a user has only opened spirit orbs and has no other cards', () => {
      beforeAll(() =>
        SyncModule.wipeUserData(userId)
          .then(() => knex('users').where('id', userId).update({ wallet_gold: 200 }))
      ...
      it('should wipe the collection entirely', () =>
        Promise.all([
      ...
      it('should give user # of orbs equal to number opened and mark old orbs with "wiped_at" time', () =>
        Promise.all([
      ...
      it('should have created card log ledger items for the wipe', () =>
        knex('user_card_log')
          .where('user_id', userId)
          .then((cardLogRows) => {
            // count all debits
            const debits = _.reduce(
              cardLogRows,
              (memo, cardLogItem) => {
    ...
    describe('if a user has not opened any orbs and has some BASIC cards', () => {
      ...
      it('should still have BASIC cards in the inventory', () =>
        Promise.all([
      ...
      it('should leave card log ledger items unchanged', () =>
        knex('user_card_log')
          .where('user_id', userId)
          .then((cardLogRows) => {
            // count all debits
    ...
    describe('if a user has opened some orbs and has some BASIC cards and ACHIEVEMENT cards', () => {
      ...
      it('should still have BASIC cards in the inventory and card ledger count should be accurate', () =>
        Promise.all([
          knex('user_cards').where('user_id', userId),
          knex('user_card_collection').first().where('user_id', userId),
        ]).then(([cardCountRows, cardCollectionRow]) => {
    ...
    describe('if a user has opened some orbs and has some BASIC cards and ACHIEVEMENT cards and disenchanted some orb cards', () => {
      ...
      it('should still have BASIC and ACHIEVEMENT cards in the inventory and card ledger count should be accurate', () =>
        Promise.all([
          knex('user_cards').where('user_id', userId),
          knex('user_card_collection').first().where('user_id', userId),
        ]).then(([cardCountRows, cardCollectionRow]) => {
      ...
      it('should set users wallet spirit to 0 and create a currency ledger item for it', () =>
        Promise.all([
          knex('users').where('id', userId).first(),
    ...
    describe('if a user has opened some orbs and has some BASIC cards and ACHIEVEMENT cards and disenchanted their ACHIEVEMENT cards', () => {
      ...
      it('should restore BASIC and ACHIEVEMENT cards in the inventory', () =>
        Promise.all([
          knex('user_cards').where('user_id', userId),
          knex('user_card_collection').first().where('user_id', userId),
        ]).then(([cardCountRows, cardCollectionRow]) => {
    ...
    describe('if a user has opened some orbs and has some BASIC cards and ACHIEVEMENT cards and disenchanted all non-basic cards and crafted other cards', () => {
      ...
      beforeAll(() =>
        SyncModule.wipeUserData(userId)
          ...
          .then((orbResults) => Promise.all([InventoryModule.craftCard(userId, 10985)]))
          .then(() => InventoryModule.softWipeUserCardInventory(userId)),
      );
  ...
  describe('prismatics', () => {
    ...
    it('expect to be able to craft a prismatic non-basic', () => {
      ...
      const cardToCraft = _.find(
        SDK.GameSession.getCardCaches().getIsCollectible(true).getCards(),
        (card) => card.id === cardIdToCraft,
      );
    ...
    it('expect to be able to disenchant a prismatic non-basic', () => {
      ...
      const cardToDisenchant = _.find(
        SDK.GameSession.getCardCaches().getIsCollectible(true).getCards(),
        (card) => card.id === cardIdToDisenchant,
      );
        ...
        .then((rootRef) =>
          Promise.all([
            knex
              .select()
              .from('user_cards')
              .where({ user_id: userId, card_id: cardIdToDisenchant }),
            knex
              .select()
              .from('user_card_log')
              .where({ user_id: userId, card_id: cardIdToDisenchant })
    ...
    it('expect disenchanting a prismatic non-basic to reward more spirit than the normal version', () => {
      ...
      return knex('users')
        ...
        .then(() =>
          InventoryModule.disenchantCards(userId, [SDK.Cards.getBaseCardId(cardIdToDisenchant)]),
        )
        .then((result) => {
          expect(result).to.exist;
          return DuelystFirebase.connect().getRootRef();
    ...
    it('expect to not be able to disenchant a prismatic basic card', () => {
      ...
      const txPromise = knex
        .transaction((tx) =>
          InventoryModule.giveUserCards(txPromise, tx, userId, [cardIdToDisenchant]),
        )
        .then(() => InventoryModule.disenchantCards(userId, [cardIdToDisenchant]))
        .then((result) => {
          expect(result).to.not.exist;
      ...
      const txPromise = knex
        ...
        .then(() =>
          Promise.all([
            ...
            knex('users')
              .where('id', userId)
              .update({ wallet_spirit: rarityData.spiritCostPrismatic }),
          ]),
        )
        .then(([cardRow]) => {
  ...
  describe('Test cached card methods', () => {
    ...
    it('expect cache to contain cards in each major category', () => {
      ...
      expect(
        SDK.GameSession.getCardCaches().getCardSet(SDK.CardSet.Core).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getCardSet(SDK.CardSet.Shimzar).getCards().length > 0,
      ).to.equal(true);

      expect(
        SDK.GameSession.getCardCaches().getFaction(SDK.Factions.Faction1).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getFaction(SDK.Factions.Faction2).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getFaction(SDK.Factions.Faction3).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getFaction(SDK.Factions.Faction4).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getFaction(SDK.Factions.Faction5).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getFaction(SDK.Factions.Faction6).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getFaction(SDK.Factions.Neutral).getCards().length > 0,
      ).to.equal(true);

      expect(
        SDK.GameSession.getCardCaches().getRarity(SDK.Rarity.Common).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getRarity(SDK.Rarity.Rare).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getRarity(SDK.Rarity.Epic).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getRarity(SDK.Rarity.Legendary).getCards().length > 0,
      ).to.equal(true);

      expect(
        SDK.GameSession.getCardCaches().getType(SDK.CardType.Unit).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getType(SDK.CardType.Spell).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getType(SDK.CardType.Artifact).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getType(SDK.CardType.Tile).getCards().length > 0,
      ).to.equal(true);

      expect(
        SDK.GameSession.getCardCaches().getRace(SDK.Races.Neutral).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getRace(SDK.Races.Golem).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getRace(SDK.Races.Arcanyst).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getRace(SDK.Races.Dervish).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getRace(SDK.Races.Mech).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getRace(SDK.Races.Vespyr).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getRace(SDK.Races.Structure).getCards().length > 0,
      ).to.equal(true);
      expect(
      ...
      expect(
        SDK.GameSession.getCardCaches().getIsCollectible(false).getCards().length > 0,
      ).to.equal(true);

      expect(SDK.GameSession.getCardCaches().getIsUnlockable(true).getCards().length > 0).to.equal(
        true,
      );
      expect(SDK.GameSession.getCardCaches().getIsUnlockable(false).getCards().length > 0).to.equal(
        true,
      );

      expect(SDK.GameSession.getCardCaches().getIsPrismatic(true).getCards().length > 0).to.equal(
        true,
      );
      expect(SDK.GameSession.getCardCaches().getIsPrismatic(false).getCards().length > 0).to.equal(
        true,
      );

      expect(SDK.GameSession.getCardCaches().getIsSkinned(true).getCards().length > 0).to.equal(
        true,
      );
      expect(SDK.GameSession.getCardCaches().getIsSkinned(false).getCards().length > 0).to.equal(
        true,
      );

      expect(
        SDK.GameSession.getCardCaches().getIsHiddenInCollection(true).getCards().length > 0,
      ).to.equal(true);
      expect(
        SDK.GameSession.getCardCaches().getIsHiddenInCollection(false).getCards().length > 0,
      ).to.equal(true);
    });
    ...
    it('expect only common faction 2 cards in all cards filtered by faction 2 commons', () => {
      const cards = SDK.GameSession.getCardCaches()
        .getFaction(SDK.Factions.Faction2)
    ...
    it('expect only core set epic faction 3 cards in all cards filtered by core set faction 3 epics', () => {
      const cards = SDK.GameSession.getCardCaches()
        ...
        .getFaction(SDK.Factions.Faction3)
    ...
    it('expect only shimzar set neutral cards in all cards filtered by shimzar set neutrals', () => {
      const cards = SDK.GameSession.getCardCaches()
        ...
        .getFaction(SDK.Factions.Neutral)
        .getCards();
      expect(cards.length > 0).to.equal(true);
      for (let i = 0, il = cards.length; i < il; i++) {
    ...
    it('expect Sarlac card id to exist in collectible card ids filtered by legendary', () => {
      const cardIds = SDK.GameSession.getCardCaches()
        .getIsCollectible(true)
        .getRarity(SDK.Rarity.Legendary)
        .getCardIds();
  ...
  describe('craftCosmetic()', () => {
    it('expect to be able to craft a COMMON cosmetic with 500 spirit in wallet', () =>
      Promise.all([
        ...
        .then(([cosmeticRow, userRow]) => {
          expect(cosmeticRow).to.exist;
          expect(cosmeticRow.user_id).to.equal(userId);
          expect(parseInt(cosmeticRow.cosmetic_id, 10)).to.equal(
            SDK.CosmeticsLookup.Emote.Faction1Angry,
          );
    ...
    it('expect to NOT be able to craft a COMMON cosmetic they already own', () =>
      Promise.all([
        knex('users').where('id', userId).update({
          wallet_spirit: 200,
        }),
  ...
  describe('claimFreeCardOfTheDay', () => {
    ...
    it('expect to be able to claim a free card of the day', () =>
      InventoryModule.claimFreeCardOfTheDay(userId)
        .then((cardId) => {
        ...
        .then(([userRow, cardCountRows, userSnapshot]) => {
          ...
          expect(userSnapshot.val().free_card_of_the_day_claimed_at).to.equal(
            userRow.free_card_of_the_day_claimed_at.valueOf(),
          );
        }));

    it('expect NOT to be able to claim a free card of the day twice on the same day', () =>
      InventoryModule.claimFreeCardOfTheDay(userId)
        .then((result) => {
          expect(result).to.not.exist;
        })
