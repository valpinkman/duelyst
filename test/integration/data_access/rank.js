...
const RankModule = require('@duelyst/server/lib/data_access/rank');
...
const { SRankManager } = require('@duelyst/server/redis/index');
...
describe('rank module', () => {
  ...
  describe('userNeedsSeasonStartRanking', () => {
    it('expect to be TRUE when there is no rank data', () =>
      RankModule.userNeedsSeasonStartRanking(userId).then((value) => {
  ...
  describe('_isSeasonTimestampExpired', () => {
    ...
    it('expect days last month to be expired', () => {
      ...
      expect(RankModule._isSeasonTimestampExpired(nowLastMonth.endOf('month').valueOf())).to.equal(
        true,
      );
      expect(
        RankModule._isSeasonTimestampExpired(nowLastMonth.startOf('month').valueOf()),
      ).to.equal(true);
    });
  });
  ...
  describe('cycleUserSeasonRanking()', () => {
    ...
    it('expect to update and return rank data', () =>
      RankModule.cycleUserSeasonRanking(userId).then((rankData) => {
        expect(rankData).to.exist;
        currentRankData = rankData;
      }));

    it('expect rank data to be correctly set in DB and Firebase', () =>
        ...
        .then((rootRef) =>
          Promise.all([
            ...
            FirebasePromises.once(
              rootRef.child('user-ranking').child(userId).child('current'),
              'value',
            ),
          ]),
        )
    ...
    it('expect NO update if current season rank cycled again', () =>
      RankModule.cycleUserSeasonRanking(userId).then((rankData) => {
        expect(rankData).to.exist;
        expect(moment.utc(rankData.updated_at).valueOf()).to.equal(
          moment.utc(currentRankData.updated_at).valueOf(),
        );
      }));

    it('expect to update rank data with force update', () =>
      RankModule.cycleUserSeasonRanking(userId, true).then((rankData) => {
        expect(rankData).to.exist;
        expect(moment.utc(rankData.created_at).valueOf()).to.not.equal(
          moment.utc(currentRankData.created_at).valueOf(),
        );
        currentRankData = rankData;
      }));
    ...
    it('expect new rank when a season passes', () => {
      ...
      return RankModule.cycleUserSeasonRanking(userId, false, systemTime).then((rankData) => {
        expect(rankData).to.exist;
        expect(moment.utc(rankData.created_at).valueOf()).to.not.equal(
          moment.utc(currentRankData.created_at).valueOf(),
        );
        expect(moment.utc(rankData.starting_at).valueOf()).to.not.equal(
          moment.utc(currentRankData.starting_at).valueOf(),
        );
      });
    });

    it(`expect the most recent rank in the DB and Firebase history to exist and to be unread and for the ${moment().utc().format('MMMM YYYY')} season`, () =>
      DuelystFirebase.connect()
        .getRootRef()
        .then((rootRef) =>
          Promise.all([
            ...
            FirebasePromises.once(
              rootRef.child('user-ranking').child(userId).child('history').limitToLast(1),
              'child_added',
            ),
          ]),
        )
        .then(([historyRow, historySnapshot]) => {
          ...
          expect(moment.utc(historyRankData.starting_at).valueOf()).to.equal(
            expectedSeasonTimestamp,
          );
    ...
    it(`expect the top rank record in the DB and Firebase to exist and to be unread and for the ${moment().utc().format('MMMM YYYY')} season`, () =>
      DuelystFirebase.connect()
        .getRootRef()
        .then((rootRef) =>
          Promise.all([
            knex.first().from('users').where('id', userId),
            FirebasePromises.once(
              rootRef.child('user-ranking').child(userId).child('top'),
              'value',
            ),
          ]),
        )
        .then(([userRow, topRankSnapshot]) => {
          ...
          expect(moment.utc(userRow.top_rank_starting_at).valueOf()).to.equal(
            expectedSeasonTimestamp,
          );
  ...
  describe('cycleUserSeasonRanking() srank', () => {
    ...
    it('expect players to have accurate ladder position and rating after cycling if (S-Rank and Non-Srank)', () => {
      ...
      return Promise.all([
        createOrWipeUser(
          'unit-test-rating-1@duelyst.local',
          player1UserName,
          0,
          startOfCurrentSeasonDate,
        ),
        createOrWipeUser(
          'unit-test-rating-2@duelyst.local',
          player2UserName,
          0,
          startOfCurrentSeasonDate,
        ),
        createOrWipeUser(
          'unit-test-rating-3@duelyst.local',
          player3UserName,
          5,
          startOfCurrentSeasonDate,
        ),
        createOrWipeUser(
          'unit-test-rating-4@duelyst.local',
          player4UserName,
          2,
          startOfCurrentSeasonDate,
        ),
        ...
        .then(() => {
          ...
          return PromiseUtils.map(matchTuples, (tuple) =>
            RankModule.updateUsersRatingsWithGameOutcome(
              userIdsByUsername[tuple[0]],
              userIdsByUsername[tuple[1]],
              tuple[2],
              generatePushId(),
              false,
              true,
        ...
        .then(() => {
          ...
          return PromiseUtils.map(usersToCycle, (username) =>
            RankModule.cycleUserSeasonRanking(userIdsByUsername[username], false, nextSeasonMoment),
          );
        })
        .then(() =>
          ...
          Promise.all([
            knex('user_rank_history')
              .first()
              .where('user_id', userIdsByUsername[player1UserName])
              .andWhere('starting_at', startOfCurrentSeasonDate),
            knex('user_rank_ratings')
              .first()
              .where('user_id', userIdsByUsername[player1UserName])
              .andWhere('season_starting_at', startOfCurrentSeasonDate),
            knex('user_rank_history')
              .first()
              .where('user_id', userIdsByUsername[player2UserName])
              .andWhere('starting_at', startOfCurrentSeasonDate),
            knex('user_rank_ratings')
              .first()
              .where('user_id', userIdsByUsername[player2UserName])
            ...
            knex('user_rank_history')
              .first()
              .where('user_id', userIdsByUsername[player3UserName])
              .andWhere('starting_at', startOfCurrentSeasonDate),
            knex('user_rank_ratings')
              .first()
              .where('user_id', userIdsByUsername[player3UserName])
            ...
            knex('user_rank_history')
              .first()
              .where('user_id', userIdsByUsername[player4UserName])
              .andWhere('starting_at', startOfCurrentSeasonDate),
            knex('user_rank_ratings')
              .first()
              .where('user_id', userIdsByUsername[player4UserName])
        ...
        .then(
          ([
            user1RankHistoryRow,
            user1RatingHistoryRow,
            user2RankHistoryRow,
            user2RatingHistoryRow,
            user3RankHistoryRow,
            user3RatingHistoryRow,
            user4RankHistoryRow,
            user4RatingHistoryRow,
          ]) => {
  ...
  describe('updateUserRankingWithGameOutcome()', () => {
    ...
    it('expect no change in data in case of a draw (input as win)', () =>
      RankModule.updateUserRankingWithGameOutcome(userId, true, generatePushId(), true).then(
        (rankData) => {
          expect(rankData).to.exist;
          expect(rankData.rank).to.equal(19);
          expect(rankData.stars).to.equal(1);
    ...
    it('expect no change in data in case of a draw (input as loss)', () =>
      RankModule.updateUserRankingWithGameOutcome(userId, false, generatePushId(), true).then(
        (rankData) => {
          expect(rankData).to.exist;
          expect(rankData.rank).to.equal(19);
          expect(rankData.stars).to.equal(1);
    ...
    it('expect to update rank data with an additional loss', () =>
      RankModule.updateUserRankingWithGameOutcome(userId, false, generatePushId()).then(
        (rankData) => {
          expect(rankData).to.exist;
          expect(rankData.rank).to.equal(19);
    ...
    it('expect to update dropping a rank with an additional loss', () =>
      RankModule.updateUserRankingWithGameOutcome(userId, false, generatePushId()).then(
        (rankData) => {
          expect(rankData).to.exist;
    ...
    it('expect rank (20) data to be correctly set in DB and Firebase', () =>
      DuelystFirebase.connect()
        .getRootRef()
        .then((rootRef) =>
          Promise.all([
  ...
  describe('getCurrentSeasonRank()', () => {
    ...
    it('expect expired rank to default to returning 30', () => {
      ...
      return RankModule.getCurrentSeasonRank(userId, systemTime).then((rank) => {
  ...
  describe('updateUsersRatingsWithGameOutcome()', () => {
    ...
    it('expect to have no rating data after two non silver players play a match', () => {
      ...
      return RankModule.updateUsersRatingsWithGameOutcome(
        player1Id,
        player2Id,
        true,
        generatePushId(),
      ...
      return RankModule.updateUsersRatingsWithGameOutcome(
        ...
        Promise.all([
          knex
            .first()
            .from('user_rank_ratings')
            .where('user_id', player3Id)
            .andWhere('season_starting_at', seasonStartingAt),
          knex
            .first()
            .from('user_rank_ratings')
            .where('user_id', player4Id)
            .andWhere('season_starting_at', seasonStartingAt),
        ])
          ...
          .then((rootRef) =>
            Promise.all([
              FirebasePromises.once(
                rootRef.child('users').child(player3Id).child('presence').child('ladder_position'),
                'value',
              ),
              FirebasePromises.once(
                rootRef.child('users').child(player4Id).child('presence').child('ladder_position'),
                'value',
              ),
            ]),
          )
    ...
    it('expect a diamond player playing a diamond player, both get a rating but no ladder position', () => {
      ...
      const startOfSeasonMonth = moment(now).utc().startOf('month');
      const seasonStartingAt = startOfSeasonMonth.toDate();
      return Promise.all([SyncModule.wipeUserData(player1Id), SyncModule.wipeUserData(player2Id)])
        .then(() => Promise.all([updateUserRank(player1Id, 5), updateUserRank(player2Id, 5)]))
        .then(() =>
          RankModule.updateUsersRatingsWithGameOutcome(
            player1Id,
            player2Id,
    ...
    it('expect a diamond player playing a srank player, diamond to get a rating but no ladder position, srank to get a rating and ladder position', () => {
      ...
      const seasonStartingAt = startOfSeasonMonth.toDate();
      return Promise.all([SyncModule.wipeUserData(player1Id), SyncModule.wipeUserData(player2Id)])
        .then(() => Promise.all([updateUserRank(player1Id, 5), updateUserRank(player2Id, 0)]))
        .then(() =>
          RankModule.updateUsersRatingsWithGameOutcome(
            player1Id,
            player2Id,
      ...
      return Promise.all([
        ...
        ([
          ...
          return PromiseUtils.each(
            ...
            (matchUpDescription) =>
              RankModule.updateUsersRatingsWithGameOutcome(
                matchUpDescription[0],
                matchUpDescription[1],
                matchUpDescription[2],
                generatePushId(),
                false,
                true,
                true,
  ...
  describe('claimRewardsForSeasonRank()', () => {
    ...
    beforeAll(() =>
      SyncModule.wipeUserData(userId).then(() => RankModule.cycleUserSeasonRanking(userId)),
    );
      ...
      return RankModule.cycleUserSeasonRanking(userId, false, daySoFar)
        ...
        .then((rootRef) =>
          Promise.all([
            knex('user_rank_history')
              .first()
              .where({ user_id: userId, starting_at: lastSeasonMoment.startOf('month') }),
          ]),
        )
        .then(([historyRow]) => {
          expect(historyRow).to.exist;
    ...
    it("expect to be rank 24 (1 star), earn 90 gold, 10 spirit, and 1 card after last season's rank 20", () => {
      ...
      return knex('users')
        .where('id', userId)
        .update({ rank: 20, rank_top_rank: 20 })
          ...
          Promise.all([
            ...
            knex('user_rewards')
              ...
              .where({
                ...
                source_id: lastSeasonMoment.format('YYYY/MM'),
              }),
          ]),
        )
    ...
    it("expect to be rank 19 (0 star), earn 110 gold, 135 spirit, and 2 cards after last season's rank 10", () => {
      ...
      return knex('users')
        .where('id', userId)
        .update({ rank: 10, rank_top_rank: 10 })
        .then(() => RankModule.cycleUserSeasonRanking(userId, false, daySoFar))
        .then(() => RankModule.claimRewardsForSeasonRank(userId, lastSeasonMoment, daySoFar))
        .then((rewards) => {
          expect(rewards).to.exist;
