...
const GUARD =
  /\bPromise\s*\.\s*(promisifyAll|promisify|defer|join|method|try|attempt|props|each|mapSeries|reduce|some|filter|map|delay|config|longStackTraces|onPossiblyUnhandledRejection|coroutine|using|disposer|settle|any|race|bind|TimeoutError|CancellationError|OperationalError|AggregateError)\b/;
const REQUIRE =
  /^[ \t]*(?:const|let|var)\s+Promise\s*=\s*require\(['"]bluebird['"]\);?[ \t]*\r?\n/m;
...
for (const file of process.argv.slice(2)) {
  ...
  if (!REQUIRE.test(before)) {
    skipped.push([file, 'no bluebird require']);
    continue;
  }

  const after = before.replace(REQUIRE, '');
  if (GUARD.test(stripCommentsAndStrings(after))) {
    skipped.push([file, 'still uses a bluebird-only static']);
    continue;
  }
