...
const rewardsData = [
  {
    spirit_gained: 23,
    shards_gained: [1, 2],
    bonuses_gained: [{ spirit: 2, gold: 10, shards: [1, 2] }],
  },
  {
    spirit_gained: 23,
    shards_gained: [1, 4],
    bonuses_gained: [{ spirit: 2, gold: 5, shards: [2, 3] }],
  },
  {
    spirit_gained: 23,
    shards_gained: [3, 4],
    bonuses_gained: [{ spirit: 0, gold: 10, shards: [3, 4] }],
  },
];
...
const reducedModelRewards = _.reduce(
  modelRewards,
  (memo, reward) => {
    ...
    for (const shardId in initialRewardShardCounts) {
      if (initialRewardShardCounts[shardId] != null) {
        memo.shards_gained[shardId] =
          (memo.shards_gained[shardId] || 0) + initialRewardShardCounts[shardId];
      }
    }
      ...
      if (rewardBonus) {
        ...
        for (const rewardShardId in rewardShardCounts) {
          if (rewardShardCounts[rewardShardId] != null) {
            totalBonus.shards[rewardShardId] =
              (totalBonus.shards[rewardShardId] || 0) + rewardShardCounts[rewardShardId];
          }
        }
      }
    }
