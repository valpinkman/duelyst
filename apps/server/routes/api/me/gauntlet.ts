  ...
  return GauntletModule.startRun(user_id, ticket_id)
    ...
    .catch(function (error) {
      Logger.module('API').error(
        `ERROR starting arena run for user ${user_id.blue}`.red,
        util.inspect(error),
      );
      return next(error);
    });
...
router.delete('/current', function (req, res, next) {
  ...
  return GauntletModule.resignRun(user_id)
    ...
    .catch(function (error) {
      Logger.module('API').error(
        `ERROR resigning arena run for user ${user_id.blue}`.red,
        util.inspect(error),
      );
      return next(error);
    });
  ...
  return GauntletModule.chooseFaction(user_id, faction_id)
    .then(function (data) {
      Logger.module('API').debug(
        `Arena faction ${faction_id} selected for user ${user_id.blue}`.cyan,
      );
      return res.status(200).json(data);
    })
    .catch(function (error) {
      Logger.module('API').error(
        `ERROR choosing arena faction for user ${user_id.blue}`.red,
        util.inspect(error),
      );
      return next(error);
    });
...
router.post('/current/cards', function (req, res, next) {
  ...
  return GauntletModule.chooseCard(user_id, card_id)
    ...
    .catch(function (error) {
      Logger.module('API').error(
        `ERROR choosing arena card for user ${user_id.blue}`.red,
        util.inspect(error),
      );
      return next(error);
    });
...
router.put('/current/rewards_claimed_at', function (req, res, next) {
  ...
  return GauntletModule.claimRewards(user_id)
    ...
    .catch(
      onType(Errors.ArenaRewardsAlreadyClaimedError, function (error) {
        Logger.module('API').error(
          `ERROR: arena rewards already claimed for user ${user_id.blue}`.red,
          util.inspect(error),
        );
        return res
          .status(403)
          .json({ message: 'The rewards for this run have already been claimed previously.' });
      }),
    )
    .catch(function (error) {
      Logger.module('API').error(
        `ERROR claiming arena rewards for user ${user_id.blue}`.red,
        util.inspect(error),
      );
      return next(error);
    });
...
router.get('/decks', function (req, res, next) {
  ...
  const recentGauntletDecksPromise = knex('user_gauntlet_run_complete')
    .select()
  ...
  return Promise.all([currentGauntletDeckPromise, recentGauntletDecksPromise])
    ...
    .catch(function (error) {
      Logger.module('API').error(
        `ERROR getting gauntlet decks for user ${user_id.blue}`.red,
        util.inspect(error),
      );
      return next(error);
    });
