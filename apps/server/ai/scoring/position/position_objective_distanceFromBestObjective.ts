 ...
 * @returns {Number}
 */
const position_objective_distanceFromBestObjective = function (
  gameSession,
  unit,
  position,
  bestObjective,
  ...
  const bounty = isEvasive
    ? BOUNTY.DISTANCE_FROM_BEST_ENEMY_TARGET_EVASIVE
    : BOUNTY.DISTANCE_FROM_BEST_ENEMY_TARGET;
  ...
  let score = isEvasive
    ? distanceFromBestEnemyTarget * bounty - maxDistance
    : distanceFromBestEnemyTarget * bounty;
  ...
  if (unit.hasModifierClass(ModifierBlastAttack)) {
    const distanceFromBestEnemyTargetX = distanceBetweenBoardPositions(
      { x: position.x, y: bestObjective.getPosition().y },
      bestObjective.getPosition(),
    );
