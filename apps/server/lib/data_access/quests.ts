...
class QuestsModule {
  ...
  static needsDailyQuests(userId, systemTime?) {
    ...
    if (!userId) {
      return Promise.reject(
        new Error(`Can not find if user needs daily quests: invalid user ID - ${userId}`),
      );
    }
    ...
    return Promise.all([
      ...
      const currentStage =
        NewPlayerProgressionStageEnum[
          newPlayerProgressionRow != null ? newPlayerProgressionRow.stage : undefined
      ...
      const begginerQuestsInFirstTwoSlots = _.filter(
        questRows,
        (q) =>
          q.quest_slot_index < 2 &&
          __guard__(QuestFactory.questForIdentifier(q.quest_type_id), (x) => x.isBeginner),
      );
      if (
        (begginerQuestsInFirstTwoSlots != null
          ? begginerQuestsInFirstTwoSlots.length
          : undefined) === 2
      ) {
        return Promise.resolve(false);
      }
      ...
      if (questRows != null) {
        for (var quest of Array.from<any>(questRows)) {
          ...
          if (
            _.contains(
              QuestFactory.questForIdentifier(quest.quest_type_id).types,
          ...
          if (
            ...
            quest.quest_slot_index !== QuestsModule.CATCH_UP_QUEST_SLOT
          ) {
            return Promise.resolve(true);
          }
        }
      ...
      if (userRow.daily_quests_generated_at != null) {
        ...
        const daysPassed =
          (now_utc_val - moment(userRow.daily_quests_generated_at).utc().valueOf()) /
          1000 /
          60 /
          60 /
          24;
  ...
  static generateDailyQuests(userId, systemTime?) {
    ...
    var txPromise = knex
      .transaction(function (tx) {
        Promise.all([
          knex('users').transacting(tx).forUpdate().first().where('id', userId),
          knex('user_quests').transacting(tx).forUpdate().select().where({ user_id: userId }),
        ])
          .then(function ([userRow, questRows]) {
            ...
            questRows = _.reduce(
              ...
              (memo, row, i) => {
                ...
                if (sdkQuest == null) {
                  removalQueries.push(
                    tx('user_quests')
                      .where({ user_id: row.user_id, quest_slot_index: row.quest_slot_index })
                      .delete(),
                  );
                  ...
                  _.contains(sdkQuest.types, QuestType.Promotional) &&
                  !sdkQuest.isAvailableOn(MOMENT_NOW_UTC)
                ) {
                  removalQueries.push(
                    tx('user_quests')
                      .where({ user_id: row.user_id, quest_slot_index: row.quest_slot_index })
                  ...
                  _.contains(sdkQuest.types, QuestType.CatchUp) &&
                  row.quest_slot_index !== QuestsModule.CATCH_UP_QUEST_SLOT
                ) {
                  removalQueries.push(
                    tx('user_quests')
                      .where({ user_id: row.user_id, quest_slot_index: row.quest_slot_index })
            ...
            const existingSeasonalQuest = _.find(
              questRows,
              (q) => q.quest_slot_index === QuestsModule.SEASONAL_QUEST_SLOT,
            );
            if (!existingSeasonalQuest && QuestsModule.SEASONAL_QUESTS_ACTIVE) {
              Logger.module('QuestsModule').debug(
                `generateDailyQuests() -> No active seasonal quest found for user ${userId.blue}.`,
              );
              const seasonalQuest = QuestFactory.seasonalQuestForMoment(MOMENT_NOW_UTC);
              if (seasonalQuest != null) {
                Logger.module('QuestsModule').debug(
                  `generateDailyQuests() -> Current season has quest ${seasonalQuest.name}. ${userId.blue}.`,
                );
                ...
                allQueries.push(
                  tx('user_quests_complete')
                    .where('user_id', userId)
                    .andWhere('quest_type_id', seasonalQuest.id)
                    .first()
                    .then((completedQuestRow) => {
                      if (completedQuestRow == null) {
                        Logger.module('QuestsModule').debug(
                          `generateDailyQuests() -> Generating ${seasonalQuest.name} seasonal quest for ${userId.blue}.`,
                        );
                        // update return data
                        ...
                        return Logger.module('QuestsModule').debug(
                          `generateDailyQuests() -> User ${userId.blue} already completed ${seasonalQuest.name} seasonal quest.`,
                        );
                      }
                    }),
                );
            ...
            const existingPromotionalQuest = _.find(
              questRows,
              (q) => q.quest_slot_index === QuestsModule.PROMOTIONAL_QUEST_SLOT,
            );
            const newPromotionalQuest = QuestFactory.promotionalQuestForMoment(MOMENT_NOW_UTC);
            const replaceCurrentPromoQuest =
              existingPromotionalQuest != null &&
              newPromotionalQuest != null &&
              existingPromotionalQuest.id < newPromotionalQuest.id;

            if (
              ((existingPromotionalQuest == null && newPromotionalQuest != null) ||
                replaceCurrentPromoQuest) &&
              QuestsModule.PROMOTIONAL_QUEST_ACTIVE
            ) {
              Logger.module('QuestsModule').debug(
                `generateDailyQuests() -> No active promotional quest found for user ${userId.blue}.`,
              );
              if (newPromotionalQuest != null) {
                Logger.module('QuestsModule').debug(
                  `generateDailyQuests() -> Current promotional quest ${newPromotionalQuest.name}. ${userId.blue}.`,
                );
                ...
                allQueries.push(
                  tx('user_quests_complete')
                    .where('user_id', userId)
                    .andWhere('quest_type_id', newPromotionalQuest.id)
                    .first()
                    .then((completedQuestRow) => {
                      if (completedQuestRow == null) {
                        Logger.module('QuestsModule').debug(
                          `generateDailyQuests() -> Generating ${newPromotionalQuest.name} promotional quest for ${userId.blue}.`,
                        );
                        // update return data
                        ...
                        return Logger.module('QuestsModule').debug(
                          `generateDailyQuests() -> User ${userId.blue} already completed ${newPromotionalQuest.name} promotional quest.`,
                        );
                      }
                    }),
                );
            ...
            let daysSinceGeneration = MOMENT_NOW_UTC.clone()
              .startOf('day')
              .diff(moment.utc(userRow.daily_quests_generated_at), 'days');
            if (daysSinceGeneration > 0 && QuestsModule.CATCH_UP_QUEST_ACTIVE) {
              ...
              numCatchUpChargesGenerated += _.reduce(
                questRows,
                function (memo, questRow) {
                  const sdkQuest = QuestFactory.questForIdentifier(questRow.quest_type_id);
                  if (
                    sdkQuest != null &&
                    !sdkQuest.isCatchUp &&
                    !sdkQuest.isBeginner &&
                    questRow.quest_slot_index < QuestsModule.CATCH_UP_QUEST_SLOT
                  ) {
                    return memo + 1;
                  } else {
                    return memo;
              ...
              daysSinceGeneration = MOMENT_NOW_UTC.clone()
                .startOf('day')
                .diff(moment.utc(userRow.daily_quests_generated_at), 'days');
              ...
              if (numCatchUpChargesGenerated !== 0) {
                allQueries.push(
                  QuestsModule._giveUserCatchUpQuestCharge(
                    ...
                    tx,
                    userId,
                    numCatchUpChargesGenerated,
                    MOMENT_NOW_UTC,
                  ).then((catchupQuestRow) => {
            ...
            for (var i of Array.from<any>(QuestsModule.DAILY_QUEST_SLOTS)) {
              ...
              if (!questAtSlot) {
                Logger.module('QuestsModule').debug(
                  `generateDailyQuests() -> Generating quest for slot ${i}. user ${userId.blue}.`,
                );
                ...
                allQueries.push(knex.insert(quest).into('user_quests').transacting(tx));
          ...
          .then(function () {
            ...
            // if it's a new day
            if (
              start_of_today_utc.valueOf() !==
              (_chainState.userRow.daily_quests_generated_at != null
                ? _chainState.userRow.daily_quests_generated_at.valueOf()
                : undefined)
            ) {
            ...
            if (dirty) {
              Logger.module('QuestsModule').debug(
                `generateDailyQuests() -> Updating user record. user ${userId.blue}.`,
              );
      ...
      .then(function () {
        ...
        for (var q of Array.from<any>(_chainState.updatedQuests)) {
          ...
          if (data.mulliganed_at) {
            data.mulliganed_at = moment.utc(data.mulliganed_at).valueOf();
          }
          if (data.updated_at) {
          ...
          allPromises.push(
            ...
            FirebasePromises.set(
              _chainState.fbRootRef
                ...
                .child('current')
                .child('updated_at'),
              moment().utc(_chainState.userRow.daily_quests_updated_at).valueOf(),
            ),
            FirebasePromises.set(
              _chainState.fbRootRef
                .child('user-quests')
                ...
                .child('current')
                .child('generated_at'),
              moment().utc(_chainState.userRow.daily_quests_generated_at).valueOf(),
            ),
          );
        }
      ...
      .then(function () {
        Logger.module('QuestsModule').timeEnd(
          `generateDailyQuests() -> for user ${userId.blue}.`.green,
        );
  ...
  static needsBeginnerQuests(userId, systemTime?) {
    ...
    Logger.module('QuestsModule').time(
      `generateBeginnerQuests() -> for user ${userId.blue}.`.green,
    );
    ...
    return knex('user_new_player_progression')
      ...
      .then(function (newPlayerCoreStateRow) {
        ...
        const newPlayerStage =
          NewPlayerProgressionStageEnum[
            newPlayerCoreStateRow != null ? newPlayerCoreStateRow.stage : undefined
          ] || NewPlayerProgressionStageEnum.Tutorial;
  ...
  static generateBeginnerQuests(userId, systemTime?) {
    ...
    return knex('user_new_player_progression')
      ...
      .then(function (newPlayerCoreStateRow) {
        // current player stage
        const newPlayerStage =
          NewPlayerProgressionStageEnum[newPlayerCoreStateRow.stage] ||
          NewPlayerProgressionStageEnum.Tutorial;

        Logger.module('QuestsModule').debug(
          `generateBeginnerQuests() -> stage ${newPlayerStage.key} for user ${userId.blue}.`,
        );
            ...
            Promise.all([
              ...
              .then(function ([userRow, questRows, questCompleteRows]) {
                ...
                for (sdkQuest of Array.from<any>(questsToGenerate)) {
                  ...
                  var questCompleted = _.find(
                    questCompleteRows,
                    (q) => q.quest_type_id === sdkQuest.id,
                  );
                  if (questExists || questCompleted) {
                    throw new Errors.NoNeedForNewBeginnerQuestsError(
                      'Beginner quests for this stage have already been generated.',
                    );
                  }
                }
                ...
                for (
                  var i = questRows.length,
                    end = questRows.length + questsToGenerate.length,
                    asc = questRows.length <= end;
                  asc ? i <= end : i >= end;
                  asc ? i++ : i--
                ) {
                  ...
                  if (!questAtSlot) {
                    Logger.module('QuestsModule').debug(
                      `generateBeginnerQuests() -> Generating quest for slot ${i}. user ${userId.blue}.`,
                    );
                    ...
                    allQueries.push(tx.insert(quest).into('user_quests'));
  ...
  static canMulliganDailyQuest(userId, questIndex, systemTime?) {
    ...
    return knex
      .first()
      .from('user_quests')
      .where({ user_id: userId, quest_slot_index: questIndex })
  ...
  static mulliganDailyQuest(userId, questIndex, systemTime, replaceWithQuestId) {
    ...
    if (!userId) {
      return Promise.reject(
        new Error(`Can not mulligan daily quests: invalid user ID - ${userId}`),
      );
    }

    Logger.module('QuestsModule').time(
      `mulliganDailyQuest() -> mulliganed quest slot ${questIndex} by user ${userId.blue}.`.green,
    );
    ...
    return knex
        ...
        knex('user_quests')
          ...
          .then(function (questRows) {
            ...
            if ((questRows != null ? questRows.length : undefined) > 0) {
              ...
              Logger.module('QuestsModule').debug(
                `mulliganDailyQuest() -> About to mulliganing quest type ${questToMulligan.quest_type_id}. user ${userId.blue}.`,
              );
              ...
              if (questToMulligan != null) {
                if (questToMulligan.mulliganed_at) {
                  ...
                  if (duration.asHours() < 23) {
                    Logger.module('QuestsModule').error(
                      `mulliganDailyQuest() -> Can not mulligan quest ${questIndex} because it's only been ${duration.asHours()} hours. user ${userId.blue}.`,
                    );
                    return Promise.reject(
                      new Errors.QuestCantBeMulliganedError(
                        'This quest has already been mulliganed today',
                      ),
                    );
                  }
                }
                ...
                Logger.module('QuestsModule').debug(
                  `mulliganDailyQuest() -> Mulliganing quest ${questIndex}. user ${userId.blue}.`,
                );

                const start_of_today_utc = MOMENT_NOW_UTC.clone().startOf('day').toDate();

                let quest1 = QuestFactory.randomQuestForSlotExcludingIds(
                  questIndex,
                  quest_ids_generated,
                );
      ...
      .then(function () {
        ...
        Logger.module('QuestsModule').timeEnd(
          `mulliganDailyQuest() -> mulliganed quest slot ${questIndex} by user ${userId.blue}.`
            .green,
        );
  ...
  static updateQuestProgressWithGame(userId, gameId, gameSessionData, systemTime?) {
    ...
    if (!userId || !gameId) {
      return Promise.reject(
        new Error(
          `Can not update quest progress : invalid user ID - ${userId} - or game ID - ${gameId}`,
        ),
      );
    }
    if (!gameSessionData) {
    ...
    Logger.module('QuestsModule').time(
      `updateQuestProgressWithGame() -> for game ${gameId} by user ${userId.blue}.`.green,
    );
        ...
        PromiseUtils.withTimeout(
          Promise.resolve(tx('users').where({ id: userId }).first('id').forUpdate())
              ...
              if ((questRows != null ? questRows.length : undefined) > 0) {
                ...
                for (quest of Array.from<any>(questRows)) {
                  try {
                    ...
                    var progressAmount = questModel.progressForGameDataForPlayerId(
                      gameSessionData,
                      userId,
                    );
                    ...
                    if (progressAmount > 0) {
                      ...
                      if (quest.progressed_by_game_ids == null) {
                      ...
                      allQueries.push(
                        QuestsModule._setQuestProgress(
                          txPromise,
                          tx,
                          quest,
                      ...
                      Logger.module('QuestsModule').debug(
                        `updateQuestProgressWithGame() -> quest[${quest.quest_slot_index}] NOT satisfied: ${questModel.getName()} User ${userId.blue}. Game [G:${gameId}].`
                          .yellow,
                      );
                    }
                  } catch (e) {
                    Logger.module('QuestsModule').debug(
                      'updateQuestProgressWithGame() -> caught ERROR processing QUEST DATA'.red,
                      e,
                    );
                  ...
                  if (quest.completed_at) {
                    ...
                    allQueries.push(
                      QuestsModule.updateQuestProgressWithCompletedQuest(
                        txPromise,
                        tx,
                        completedQuest.user_id,
                        gameId,
                        completedQuest.quest_type_id,
                        questRows,
                      ),
                    );
                  }
            ...
            .then(function (rewards) {
              return (_chainState.rewards = _.flatten(_.compact(rewards)));
            })
            .then(() => SyncModule._bumpUserTransactionCounter(tx, userId)),
          10000,
        ).catch(
          onType(PromiseUtils.TimeoutError, function (e) {
            Logger.module('QuestsModule').error(
              `updateQuestProgressWithGame() -> ERROR, operation timeout for u:${userId} g:${gameId}`,
            );
            throw e;
          }),
        ),
   ...
   * @return  {Promise}            Promise that will post { quests:[] rewards:[] } on completion.
   */
  static updateQuestProgressWithCompletedChallenge(
    txPromise,
    tx,
    userId,
    challengeId,
    ...
    if (!userId || !challengeId) {
      return Promise.reject(
        new Error(
          `Can not update quest progress : invalid user ID - ${userId} - or challenge ID - ${challengeId}`,
        ),
      );
    }

    Logger.module('QuestsModule').time(
      `updateQuestProgressWithCompletedChallenge() -> for challenge ${challengeId} by user ${userId.blue}.`
        .green,
    );
          ...
          for (var quest of Array.from<any>(questRows)) {
            try {
              ...
              if (progressAmount > 0) {
                ...
                Logger.module('QuestsModule').debug(
                  `updateQuestProgressWithCompletedChallenge() -> quest[${quest.quest_slot_index}] NOT satisfied: ${questModel.getName()} User ${userId.blue}. Challenge: ${challengeId}.`
                    .yellow,
                );
              }
            } catch (e) {
              Logger.module('QuestsModule').debug(
                'updateQuestProgressWithCompletedChallenge() -> caught ERROR processing QUEST DATA'
                  .red,
                e,
              );
              return Promise.reject(new Error('ERROR PROCESSING QUEST DATA'));
   ...
   * @return  {Promise}            Promise that will post { quests:[] rewards:[] } on completion.
   */
  static updateQuestProgressWithCompletedQuest(
    txPromise,
    tx,
    userId,
    gameId,
    ...
    if (!userId || !questId) {
      return Promise.reject(
        new Error(
          `Can not update quest progress : invalid user ID - ${userId} - or quest ID - ${questId}`,
        ),
      );
    }

    Logger.module('QuestsModule').time(
      `updateQuestProgressWithCompletedQuest() -> for user ${userId.blue}.`.green,
    );
          ...
          for (var quest of Array.from<any>(questRows)) {
            try {
              ...
              if (progressAmount > 0) {
                ...
                Logger.module('QuestsModule').debug(
                  `updateQuestProgressWithCompletedQuest() -> quest[${quest.quest_slot_index}] NOT satisfied: ${questModel.getName()} User ${userId.blue}.`
                    .yellow,
                );
              }
            } catch (e) {
              Logger.module('QuestsModule').debug(
                'updateQuestProgressWithCompletedQuest() -> caught ERROR processing QUEST DATA'.red,
                e,
              );
              return Promise.reject(new Error('ERROR PROCESSING QUEST DATA'));
              break;
   ...
   * @return  {Promise}            Promise that will post { quests:[] rewards:[] } on completion.
   */
  static updateQuestProgressWithProgressedFactionData(
    txPromise,
    tx,
    userId,
    progressedFactionData,
    systemTime?,
  ) {
    const _chainState: Record<string, any> = {};
    // userId or challengeId must be defined
    if (!userId) {
      return Promise.reject(
        new Error(`Can not update quest progress for faction data : invalid user ID - ${userId}`),
      );
    }

    if (!progressedFactionData) {
      return Promise.reject(
        new Error('Can not update quest progress for faction data : invalid faction data'),
      );
    }
    ...
    if (factionId == null) {
      return Promise.reject(
        new Error(
          'Can not update quest progress for faction data : invalid faction data - missing faction id',
        ),
      );
    }

    Logger.module('QuestsModule').time(
      `updateQuestProgressWithProgressedFactionData() -> for faction id ${factionId} by user ${userId.blue}.`
        .green,
    );
        ...
        if ((questRows != null ? questRows.length : undefined) > 0) {
          ...
          for (var quest of Array.from<any>(questRows)) {
            try {
              ...
              // check if game and player satisfied this class of quest
              var progressAmount =
                questModel.progressForProgressedFactionData(progressedFactionData);
              if (progressAmount > 0) {
                if (quest.progress == null) {
                  quest.progress = 0;
                }
                ...
                Logger.module('QuestsModule').debug(
                  `updateQuestProgressWithProgressedFactionData() -> quest[${quest.quest_slot_index}] NOT satisfied: ${questModel.getName()} User ${userId.blue}. Faction id: ${factionId}.`
                    .yellow,
                );
              }
            } catch (e) {
              Logger.module('QuestsModule').debug(
                'updateQuestProgressWithProgressedFactionData() -> caught ERROR processing QUEST DATA'
                  .red,
                e,
              );
              return Promise.reject(new Error('ERROR PROCESSING QUEST DATA'));
  ...
  static _setQuestProgress(
    ...
    if (progressAmount > 0) {
      ...
      if (quest.progress >= quest.params.completionProgress) {
        Logger.module('QuestsModule').debug(
          `_setQuestProgress() -> quest[${quest.quest_slot_index}] COMPLETED at ${quest.progress}/${quest.params.completionProgress}: ${questModel.getName()}. User ${quest.user_id.blue}. For [G:${gameId}].`
            .cyan,
        );
        ...
        allQueries.push(
          tx('user_quests')
            .where({ user_id: quest.user_id, quest_slot_index: quest.quest_slot_index })
            .delete(),
        );
        ...
        if (QuestFactory.questForIdentifier(quest.quest_type_id).giftChests != null) {
          ...
          for (var type of Array.from<any>(
            QuestFactory.questForIdentifier(quest.quest_type_id).giftChests,
          )) {
        ...
        if (QuestFactory.questForIdentifier(quest.quest_type_id).cosmeticKeys != null) {
          ...
          for (var cosmeticKeyType of Array.from<any>(
            QuestFactory.questForIdentifier(quest.quest_type_id).cosmeticKeys,
          )) {
            // allQueries.push(GiftCrateModule.addGiftCrateToUser(txPromise, tx, quest.user_id, type))
            allQueries.push(
        ...
        Logger.module('QuestsModule').debug(
          `_setQuestProgress() -> quest[${quest.quest_slot_index}] progressed to ${quest.progress}/${quest.params.completionProgress}: ${questModel.getName()}. User ${quest.user_id.blue}. Game [G:${gameId}].`
            .cyan,
        );
      ...
      gameSessionData != null &&
      questModel.shouldResetProgress(gameSessionData, progressAmount)
    ) {
      Logger.module('QuestsModule').debug(
        `_setQuestProgress() -> quest[${quest.quest_slot_index}] progress is reset: ${questModel.getName()} User ${quest.user_id.blue}. Game [G:${gameId}].`
          .yellow,
      );
      ...
      Logger.module('QuestsModule').debug(
        `_setQuestProgress() -> quest[${quest.quest_slot_index}] NOT satisfied: ${questModel.getName()} User ${quest.user_id.blue}. Game [G:${gameId}].`
          .yellow,
      );
    }
    ...
    return Promise.all(allQueries)
      .then(function () {
        return (_chainState.rewards = rewards);
      })
      .then(() => DuelystFirebase.connect().getRootRef())
      .then(function (fbRootRef) {
        const allPromises = [];
  ...
  static _giveUserCatchUpQuestCharge(txPromise, tx, userId, numCharges, systemTime?) {
    ...
    Logger.module('QuestsModule').debug(
      `_giveUserCatchUpQuestCharge() -> User ${userId.blue} receiving ${numCharges} quest catch up charges.`
        .cyan,
    );
    ...
    return knex('user_quests')
      ...
      .then(function (userQuestRows) {
        ...
        if (_chainState.userCatchUpQuestRow == null) {
          ...
          _chainState.userCatchUpQuestRow = QuestsModule._questDataForQuest(
            QuestFactory.questForIdentifier(QuestCatchUp.Identifier),
            MOMENT_NOW_UTC,
          );
        ...
        if (_chainState.userCatchUpQuestRow.gold <= QuestsModule.CATCH_UP_MAX_GOLD_VALUE) {
          ...
          _chainState.userCatchUpQuestRow.gold = Math.min(
            _chainState.userCatchUpQuestRow.gold +
              numCharges * QuestsModule.CATCH_UP_CHARGE_GOLD_VALUE,
        ...
        if (_chainState.needsUpdate || _chainState.needsInsert) {
          txPromise
            ...
            .then((fbRootRef) => {
              ...
              fbUserCatchUpQuestData.created_at = moment
                .utc(fbUserCatchUpQuestData.created_at)
                .valueOf();
              fbUserCatchUpQuestData.begin_at = moment
                .utc(fbUserCatchUpQuestData.begin_at)
                .valueOf();
              if (fbUserCatchUpQuestData.mulliganed_at) {
                fbUserCatchUpQuestData.mulliganed_at = moment
                  .utc(fbUserCatchUpQuestData.mulliganed_at)
                  .valueOf();
              }
              if (fbUserCatchUpQuestData.updated_at) {
                fbUserCatchUpQuestData.updated_at = moment
                  .utc(fbUserCatchUpQuestData.updated_at)
                  .valueOf();
              }
        ...
        if (_chainState.needsInsert) {
          Logger.module('QuestsModule').debug(
            `_giveUserCatchUpQuestCharge() -> Inserting new catch up quest for user ${userId.blue}.`
              .cyan,
          );
          return knex.insert(_chainState.userCatchUpQuestRow).into('user_quests').transacting(tx);
        } else if (_chainState.needsUpdate) {
          Logger.module('QuestsModule').debug(
            `_giveUserCatchUpQuestCharge() -> Updating current catch up quest for user ${userId.blue}.`
              .cyan,
          );
          return knex('user_quests')
            .where({ user_id: userId, quest_slot_index: QuestsModule.CATCH_UP_QUEST_SLOT })
            .update(_chainState.userCatchUpQuestRow)
            .transacting(tx);
        } else {
          return Promise.resolve();
        }
