...
class BlurShaderGenerator {
  static compileShader(name, sigma, kernelSize = null) {
    ...
    blurProgram.initWithVertexShaderByteArray(
      glslify('./PosTexVertex.glsl'),
      this.generateFragmentShaderCode(sigma, kernelSize),
    );
  ...
  static generateFragmentShaderCode(sigma, kernelSize = null) {
    ...
    const expectedKernelSize = Math.max(
      MIN_KERNEL_SIZE,
  ...
  static generateKernel(sigma, kernelSize, sampleCount) {
    ...
    const outsideSamplesLeft = this.calcSamplesForRange(
      sigma,
      -5 * sigma,
      kernelLeft - 0.5,
      samplesPerBin,
    );
    const outsideSamplesRight = this.calcSamplesForRange(
      sigma,
      -kernelLeft + 0.5,
      5 * sigma,
      samplesPerBin,
