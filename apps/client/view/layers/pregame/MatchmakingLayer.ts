...
const SoundEffectSequence = require('apps/client/audio/SoundEffectSequence');
...
const MatchmakingLayer = ParallaxLayer.extend({
  ...
  getRequiredResources() {
    return ParallaxLayer.prototype.getRequiredResources
      .call(this)
      .concat(PKGS.getPkgForIdentifier('matchmaking'));
  },
  ...
  onResize() {
    ...
    this.whenRequiredResourcesReady().then((requestId) => {
      ...
      if (this._crestNode != null) {
        this._crestNode.setScale(
          UtilsEngine.getWindowHeightRelativeNodeScale(this._crestNode) * 1.1,
        );
      }

      if (this._myGeneralSprite != null) {
        this._myGeneralSprite.stopActionByTag(CONFIG.MOVE_TAG);
        this._myGeneralSprite.setScale(
          UtilsEngine.getWindowHeightRelativeNodeScale(this._myGeneralSprite) * 1.75,
        );
        if (this._opponentGeneralSprite != null) {
          this.addOrUpdateParallaxedNode(
            this._myGeneralSprite,
            -1,
            this.generalParallaxRatio,
            cc.p(winSize.width * this._myGeneralOffsetX, -winSize.height * 0.2),
          );
        } else {
          this.addOrUpdateParallaxedNode(
            this._myGeneralSprite,
      ...
      if (this._opponentGeneralSprite != null) {
        ...
        this._opponentGeneralSprite.setScale(
          UtilsEngine.getWindowHeightRelativeNodeScale(this._opponentGeneralSprite) * 1.75,
        );
        this.addOrUpdateParallaxedNode(
          this._opponentGeneralSprite,
          -1,
          this.generalParallaxRatio,
          cc.p(winSize.width * this._opponentGeneralOffsetX, -winSize.height * 0.2),
        );
      }
    });
  },
  ...
  showVsForGame(
    ...
    const showOpponentPlayerPromise = this.showOpponentPlayer(
      opponentPlayerFactionId,
      opponentGeneralFactionId,
    );
    ...
    return Promise.all([showMyPlayerPromise, showOpponentPlayerPromise]).then(() => {
      ...
      const moveMyPlayerPromise = this.movePlayer(
        this._myGeneralOffsetX,
        this._myGeneralSprite,
        animationDuration,
      );
      const moveOpponentPlayerPromise = this.movePlayer(
        this._opponentGeneralOffsetX,
        this._opponentGeneralSprite,
        animationDuration,
      );
      ...
      return Promise.all([moveMyPlayerPromise, moveOpponentPlayerPromise]);
    });
  },
  ...
  showNewGame(player1GeneralId, player2GeneralId) {
    if (this._showNewGamePromise == null) {
      this._showNewGamePromise = new Promise<void>((resolve, reject) => {
        ...
        const announcerSecondResource =
          player2General && player2General.getAnnouncerSecondResource();
        if (announcerFirstResource == null || announcerSecondResource == null) {
          ...
          this.addResourceRequest(requestId, null, [
            announcerFirstResource,
            announcerSecondResource,
          ]);
  ...
  showMyPlayer(factionId, generalId) {
    if (this._showMyPlayerPromise == null) {
      ...
      if (generalId == null) {
        generalId = SDK.FactionFactory.generalIdForFactionByOrder(
          factionId,
      ...
      this.getFX().showGradientColorMap(
        ...
        cc.color(194, 203, 240, 255),
        cc.color(20, 25, 60, 255),
      );
      ...
      Promise.all([this.whenRequiredResourcesReady(), this.whenResourcesReady(requestId)]).then(
        ([requiredRequestId, runesRequestId]) => {
          if (
            !this.getAreResourcesValid(requiredRequestId) ||
            !this.getAreResourcesValid(runesRequestId)
          )
          ...
          particles.setPosVar(
            cc.p(UtilsEngine.getGSIWinWidth() * 0.5, UtilsEngine.getGSIWinHeight() * 0.5),
          );
      ...
      this._showMyPlayerPromise = Promise.all([
        ...
        if (
          !this._crestNode.getAreResourcesValid(crestRequestId) ||
          !this._myGeneralSprite.getAreResourcesValid(generalRequestId)
        )
          return; // resources have been invalidated
  ...
  showOpponentPlayer(factionId, generalId) {
    if (this._showOpponentPlayerPromise == null) {
      ...
      this._showOpponentPlayerPromise = this._opponentGeneralSprite
        .whenRequiredResourcesReady()
        .then((generalRequestId) => {
          ...
          this.runAction(
            cc.targetedAction(
              this._opponentGeneralSprite,
              cc.fadeTo(CONFIG.ANIMATE_FAST_DURATION, 255),
            ),
          );
        });
    }
  ...
  _addRuneTracerNode(spriteIdentifier) {
    ...
    if (animAction) {
      ...
      runeGlowSprite.runAction(
        cc.repeatForever(cc.sequence(cc.fadeTo(0.1, 130), cc.fadeTo(0.2, 100))),
      );
    ...
    const movePromise = new Promise<void>((resolve, reject) => {
      ...
      const parallaxPositionUpdate = function (value, key) {
        if (key === 'offset') {
          ...
          parallaxNode.setChildParallaxRatioAndOffset(
            generalSprite,
            generalParallaxRatio,
            tweenedOffset,
          );
      ...
      if (animationDuration != null && animationDuration > 0.0) {
        ...
        const moveAction = cc
          .actionTween(animationDuration, 'offset', 0.0, 1.0)
