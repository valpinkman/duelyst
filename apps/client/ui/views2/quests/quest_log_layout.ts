...
var QuestLogLayout = Backbone.Marionette.LayoutView.extend({
  ...
  serializeModel: function (model) {
    ...
    data.is_replace_instruction_visible = this.collection.find(function (q) {
      return q.get('is_replaceable');
    });
  ...
  isFirstWinOfTheDayAvailable: function () {
    var now = moment().utc();
    var newFirstWinRewardMoment = moment(this.model.get('last_daily_win_at') || 0)
      .utc()
      .add(22, 'hours');
  ...
  showDailyItems: function () {
    ...
    if (this._dailyChallengeModel != null || !newPlayerManager.canSeeDailyChallenge()) {
      ...
      if (
        ...
        this._dailyChallengeModel.get('challenge_id') != null &&
        this._dailyChallengeModel.get('url') != null
      ) {
      ...
      if (newPlayerManager.canSeeFreeCardOfTheDay()) {
        this.freeCardOfTheDayRegion.show(
          new FreeCardOfTheDayItemView({
      ...
      this.questsRegion.show(
        new QuestLogCompositeView({
      ...
      questsManager.fetchDailyChallengeModel().then(
        function (dailyChallengeModel) {
  ...
  onShow: function () {
    ...
    if (!this.isFirstWinOfTheDayAvailable()) {
      ...
      this.firstWinOfDayUpdateInterval = setInterval(
        this.updateFirstWinOfDayCountdown.bind(this),
        1000,
      );
    }
  ...
  updateRolloverCountdown: function () {
    ...
    this.ui.$rollovercountdown.text(
      i18next.t('quests.quest_rollover_instructions', {
        time_until_quests: UtilsJavascript.stringifyHoursMinutesSeconds(
          tillMidnightDate.getUTCHours(),
          tillMidnightDate.getUTCMinutes(),
          tillMidnightDate.getUTCSeconds(),
        ),
      }),
    );
  },
  ...
  updateFirstWinOfDayCountdown: function () {
    var last_win_at = moment(this.model.get('last_daily_win_at') || 0)
      .add(22, 'hours')
      .utc();
    var now = moment().utc();
    ...
    if (duration.hours())
      durationStr +=
        duration.hours() + ' ' + i18next.t('common.time_hour', { count: duration.hours() }) + ' ';

    if (duration.minutes())
      durationStr +=
        duration.minutes() +
        ' ' +
        i18next.t('common.time_minute', { count: duration.minutes() }) +
        ' ';

    durationStr +=
      duration.seconds() +
      ' ' +
      i18next.t('common.time_second', { count: duration.seconds() }) +
      ' ';
