...
var ChangePortraitItemView = require('apps/client/ui/views/item/change_portrait');
var ChangeUsernameItemView = require('apps/client/ui/views/item/change_username');
var ChangePasswordItemView = require('apps/client/ui/views/item/change_password');
var ConfirmDialogItemView = require('apps/client/ui/views/item/confirm_dialog');
// region views
...
var ProfileLayout = Backbone.Marionette.LayoutView.extend({
  ...
  serializeModel: function (model) {
    ...
    if (!data.isViewingBuddyProfile) {
      data.season_count = Math.floor(
        moment.duration(moment().utc().valueOf() - data.created_at).asMonths(),
      );
    }
  ...
  onChangeUsernameClicked: function (e) {
    var changeUsernameItemView = new ChangeUsernameItemView({
      model: ProfileManager.getInstance().profile,
    });
    changeUsernameItemView.listenTo(
      changeUsernameItemView,
      'success',
      function () {
  ...
  onChangePasswordClicked: function (e) {
    var confirmDialogItemView = new ConfirmDialogItemView({
      title: 'Click OK and use the forgot password system to set a password.',
    });
  ...
  showSummary: function () {
    if (this.cache.summary) {
      ...
      currentSeasonGameCounterModel.url =
        this.apiEndpointUrl +
        '/rank/history/' +
        moment().utc().startOf('month').format('YYYY-MM') +
        '/game_counter';
        ...
        .then(
          function () {
            ...
            _.each(ribbonsCollection.toJSON(), function (r) {
              var ribbon = _.find(ribbons, function (ribbon) {
                return ribbon.id == r.ribbon_id;
              });
    ...
    this.ui.$profileTabs.children().removeClass('active');
    this.ui.$profileTabs
      .children("[data-value='" + this.selectedTabValue + "']")
      .addClass('active');
  },
  ...
  showRankHistory: function () {
    if (this.cache.rank_history) {
      this.contentRegion.show(
        new ProfileRankHistoryCollectionView({
  ...
  showMatchHistory: function () {
    ...
    if (this.cache['match_history_' + this.gamesHistoryPage]) {
      ...
      Promise.all([gameHistoryCollection.onSyncOrReady()]).then(
        function () {
  ...
  showRiftSummary: function () {
    if (this.cache.rift_summary) {
      ...
      Promise.all([riftSummaryModel.onSyncOrReady()]).then(
        function () {
  ...
  showFactionLevels: function () {
    if (this.cache.faction_levels) {
      this.contentRegion.show(
        new ProfileFactionLevelCollectionView({
      ...
      Promise.all([factionProgressionModel.onSyncOrReady()]).then(
        function () {
