...
var MainMenuTmpl = require('apps/client/ui/templates/item/main_menu.hbs');
...
var ZodiacSymbolModel = require('apps/client/ui/models/zodiac_symbol');
var audio_engine = require('apps/client/audio/audio_engine');
var CrateManager = require('apps/client/ui/managers/crate_manager');
var StreamManager = require('apps/client/ui/managers/stream_manager');
var ServerStatusManager = require('apps/client/ui/managers/server_status_manager');
var i18next = require('i18next');
var ShopSpiritOrbsModalView = require('apps/client/ui/views2/shop/shop_spirit_orbs_modal');
var PlayLayout = require('apps/client/ui/views/layouts/play');
...
var MainMenuItemView = Backbone.Marionette.ItemView.extend({
  ...
  serializeModel: function (model) {
    ...
    if (
      ProgressionManager.getInstance().getHasActiveBossEvent() &&
      NewPlayerManager.getInstance().canPlayBossBattle()
    ) {
      data.boss_battle = ProgressionManager.getInstance().getCurrentBossEventModels()[0].toJSON();
      data.boss_battle_is_available = true;
      data.boss_battle_is_defeated = ProgressionManager.getInstance().getHasDefeatedBossForEvent(
        data.boss_battle.boss_id,
        data.boss_battle.event_id,
      );
    } else if (
      ProgressionManager.getInstance().getUpcomingBossEventModel() &&
      NewPlayerManager.getInstance().canPlayBossBattle()
    ) {
  ...
  onShow: function () {
    ...
    var showLiveStreamBadgeProbability =
      ServerStatusManager.getInstance().serverStatusModel.get('live_stream_badge_probability') ||
      -1;
    ...
    StreamManager.getInstance().onReady(
      function () {
        ...
        if (
          StreamManager.getInstance().liveStreamCollection.length > 0 &&
          !StreamManager.getInstance().hasDismissedStreams
        ) {
    ...
    if (showPremiumPurchasePromo) {
      ...
      this.ui.$premiumPurchasePromoBtn.html(
        i18next.t('shop.confirm_purchase_dialog_premium_price', {
          price: ShopData.promos.FROSTFIRE_2017_PREMIUM_CRATE.price,
        }),
      );
    ...
    if (showPurchasePromo) {
      ...
      this.ui.$purchasePromoBtn.html(
        i18next.t('shop.confirm_purchase_dialog_premium_price', {
          price: ShopData.promos.FROSTFIRE_2017_CRATE.price,
        }),
      );
  ...
  onRender: function () {
    ...
    if (
      !NewPlayerManager.getInstance().canSeeCrates() &&
      CrateManager.getInstance().getGiftCrateCount() == 0
    ) {
  ...
  bindUnreadCardCount: function () {
    ...
    if (InventoryManager.getInstance().hasUnreadCards()) {
      this.ui.$btnCollection
        .find('.badge-unread-cards')
        .addClass('active')
        .text(InventoryManager.getInstance().getTotalUnreadCardCount());
    } else {
  ...
  bindUnreadLoreCount: function () {
    ...
    if (
      ...
      InventoryManager.getInstance().hasUnreadCardLore()
    ) {
    ...
    if (NewPlayerManager.getInstance().canSeeCrates() || crateManager.getGiftCrateCount() > 0) {
      if (crateManager.getCosmeticChestCountForType(SDK.CosmeticsChestTypeLookup.Boss) > 0) {
        ...
        if (openableCrateType != null) {
          ...
          this.ui.$btnCrateInventory
            .find('.badge')
            .addClass('active')
            .text(
              i18next.t('main_menu.menu_item_crates_can_open_badge', {
                crate_name: CosmeticsFactory.nameForCosmeticChestType(openableCrateType),
              }),
            );
        } else {
          var numUnreadCrates =
            CrateManager.getInstance().getGiftCrateCount() + crateManager.getCosmeticChestCount();
          if (numUnreadCrates > 0) {
            this.ui.$btnCrateInventory
              .find('.badge')
              .addClass('active')
              .text('' + numUnreadCrates);
          } else {
  ...
  onClickNextScene: function () {
    ...
    while (newSceneData == null) {
      ...
      if (
        potentialSceneId === selectedScene ||
  ...
  _showNewPlayerUI: function () {
    if (
      ...
      SDK.NewPlayerProgressionStageEnum.TutorialDone.value
    ) {
    ...
    if (
      NewPlayerManager.getInstance().getCurrentCoreStage().value ==
      SDK.NewPlayerProgressionStageEnum.FirstPracticeDuelDone.value
    ) {
      this.ui.$playLabel.addClass('emphasis-glow');
      this.ui.$playLabel.popover({
    ...
    if (
      NewPlayerManager.getInstance().getCurrentCoreStage().value ==
      SDK.NewPlayerProgressionStageEnum.ExtendedPracticeDone.value
    ) {
      this.ui.$playLabel.addClass('emphasis-glow');
      this.ui.$playLabel.popover({
    ...
    if (
      NewPlayerManager.getInstance().getCurrentCoreStage().value ==
      SDK.NewPlayerProgressionStageEnum.FirstFactionLevelingDone.value
    ) {
    }
  ...
  animateReveal: function () {
    ...
    brandMain[0].animate([{ opacity: 0.0 }, { opacity: 1.0 }], {
      duration: 2000,
      delay: delay,
  ...
  deactivateSymbolMainMenu: function () {
    if (
      this.ui.$symbolMainMenuIcon instanceof $ &&
      this.ui.$symbolMainMenuIcon._animation != null
    ) {
      this.ui.$symbolMainMenuIcon._animation.pause();
    }
    if (
      this.ui.$symbolMainMenuRingInner instanceof $ &&
      this.ui.$symbolMainMenuRingInner._animation != null
    ) {
      this.ui.$symbolMainMenuRingInner._animation.pause();
    }
    if (
      this.ui.$symbolMainMenuRingOuter instanceof $ &&
      this.ui.$symbolMainMenuRingOuter._animation != null
    ) {
