...
export class ChromaApp {
  ...
  public async Instance(create: boolean = true): Promise<ChromaInstance> {
    ...
    if (create) {
      ...
      // An async executor swallows nothing here -- the try/catch only forwarded
      // to reject(), which is what an async function does on its own. Rejection
      // values are unchanged.
      this.activeInstance = (async (): Promise<ChromaInstance> => {
        const response = await fetch('http://localhost:54235/razer/chromasdk', options);
        const json = await response.json();
        if (json.uri !== undefined) {
          return new ChromaInstance(json.uri);
        }
        throw 'Unable to retrieve URI ' + JSON.stringify(json);
      })();
