    ...
    if (
      ...
      if (owner instanceof Player) {
        ...
        if (
          ...
          deck.getCardIndexInHandAtIndex(action.indexOfCardInHand) == null
        ) {
          return this.invalidateAction(
            action,
            action.getTargetPosition(),
            i18next.t('validators.invalid_card_to_replace_message'),
          );
        }
        if (!deck.getCanReplaceCardThisTurn()) {
          return this.invalidateAction(
            action,
            action.getTargetPosition(),
            i18next.t('validators.out_of_replaces_message'),
          );
        }
      }
    }
