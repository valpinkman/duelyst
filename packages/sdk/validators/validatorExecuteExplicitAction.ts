...
const Player = require('@duelyst/sdk/player');
const DrawStartingHandAction = require('@duelyst/sdk/actions/drawStartingHandAction');
...
const RollbackToSnapshotAction = require('@duelyst/sdk/actions/rollbackToSnapshotAction');
const EndFollowupAction = require('@duelyst/sdk/actions/endFollowupAction');
const i18next = require('i18next');

class ValidatorExecuteExplicitAction extends Validator {
  declare type: any;

  static type = 'ValidatorExecuteExplicitAction';
  ...
  onValidateAction(event) {
    ...
    if (
      ...
      action.getType() !== ResignAction.type
    ) {
        ...
        if (gameSession.isNew()) {
          if (action instanceof DrawStartingHandAction) {
            if (owner.getHasStartingHand()) {
              ...
              return this.invalidateAction(
                ...
                i18next.t('validators.cant_draw_starting_hand_again_message'),
              );
            }
            if (action.getMulliganIndices().length > CONFIG.STARTING_HAND_REPLACE_COUNT) {
              ...
              return this.invalidateAction(
                action,
                action.getTargetPosition(),
                i18next.t('validators.max_replaces_reached_message'),
              );
            }
          } else {
            ...
            return this.invalidateAction(
              action,
              action.getTargetPosition(),
              i18next.t('validators.game_hasnt_started_message'),
            );
          }
          ...
          if (gameSession.getCurrentTurn().getEnded()) {
            ...
            return this.invalidateAction(
              action,
              action.getTargetPosition(),
              i18next.t('validators.turn_has_ended_message'),
            );
          }
          if (owner !== gameSession.getCurrentPlayer()) {
            ...
            return this.invalidateAction(
              action,
              action.getTargetPosition(),
              i18next.t('validators.opponents_turn_message'),
            );
          }
          if (
            gameSession.getTurnTimeRemaining() <= 0.0 &&
            action.getType() !== EndTurnAction.type
          ) {
            return this.invalidateAction(
              action,
              action.getTargetPosition(),
              i18next.t('validators.run_out_of_time_message'),
            );
          }
          if (owner.getRemainingMana() < action.getManaCost()) {
            ...
            return this.invalidateAction(
              action,
              action.getTargetPosition(),
              i18next.t('validators.not_enough_mana_message'),
            );
          }
          if (
            !(
              action.getType() === MoveAction.type ||
              action.getType() === AttackAction.type ||
              action.getType() === PlayCardFromHandAction.type ||
              action.getType() === PlaySignatureCardAction.type ||
              action.getType() === ReplaceCardFromHandAction.type ||
              action.getType() === EndTurnAction.type ||
              action.getType() === EndFollowupAction.type ||
              action.getType() === RollbackToSnapshotAction.type ||
              (action instanceof ApplyCardToBoardAction &&
                __guard__(action.getCard(), (x) => x.getIsFollowup()))
            )
          ) {
            // player is attempting to execute an explicit action that is not one of the allowed types
            return this.invalidateAction(
              action,
              action.getTargetPosition(),
              i18next.t('validators.cannot_do_that_message'),
            );
          }
        } else {
          ...
          return this.invalidateAction(
            action,
            action.getTargetPosition(),
            i18next.t('validators.game_is_over_message'),
          );
        }
      }
    }
...
ValidatorExecuteExplicitAction.prototype.type = 'ValidatorExecuteExplicitAction';
