...
class SisterAchievement extends Achievement {
  ...
  static progressForCardCollection(cardCollection, allCards) {
    ...
    const sisterCount =
      (cardCollection[Cards.Faction4.ShadowSister] != null
        ? cardCollection[Cards.Faction4.ShadowSister].count
        : undefined) || 0;
    if (sisterCount >= 3) {
      return 0;
    }
    ...
    const allFactionRares = _.filter(
      allCards,
      (card) =>
        card.getFactionId() === Factions.Faction4 &&
    ...
    for (var card of Array.from<any>(allFactionRares)) {
...
SisterAchievement.title = i18next.t('achievements.shadow_sister_title');
SisterAchievement.description = i18next.t('achievements.shadow_sister_desc');
SisterAchievement.rewards = {
  cards: [Cards.Faction4.ShadowSister, Cards.Faction4.ShadowSister, Cards.Faction4.ShadowSister],
};
