...
class SisterAchievement extends Achievement {
  ...
  static progressForCardCollection(cardCollection, allCards) {
    ...
    const sisterCount =
      (cardCollection[Cards.Faction6.WindSister] != null
        ? cardCollection[Cards.Faction6.WindSister].count
        : undefined) || 0;
    if (sisterCount >= 3) {
      return 0;
    }
    ...
    const allFactionRares = _.filter(
      allCards,
      (card) =>
        card.getFactionId() === Factions.Faction6 &&
...
SisterAchievement.title = i18next.t('achievements.wind_sister_title');
SisterAchievement.description = i18next.t('achievements.wind_sister_desc');
SisterAchievement.rewards = {
  cards: [Cards.Faction6.WindSister, Cards.Faction6.WindSister, Cards.Faction6.WindSister],
};
