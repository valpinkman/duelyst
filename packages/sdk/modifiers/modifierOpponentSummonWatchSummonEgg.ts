  ...
  onSummonWatch(action?) {
    ...
    if (this.getGameSession().getIsRunningAsAuthoritative()) {
      ...
      if (validSpawnLocations.length > 0) {
        const spawnLocation = validSpawnLocations.splice(
        ...
        if (eggToSpawn.additionalInherentModifiersContextObjects == null) {
          eggToSpawn.additionalInherentModifiersContextObjects = [];
        }
        eggToSpawn.additionalInherentModifiersContextObjects.push(
          ModifierEgg.createContextObject(this.cardDataOrIndexToSpawn, this.eggName),
        );
...
ModifierOpponentSummonWatchSummonEgg.prototype.fxResource = [
