...
class SpellJoseki extends Spell {
  onApplyEffectToBoardTile(board, x, y, sourceAction) {
    ...
    const myDrawPile = this.getGameSession()
      .getPlayerById(this.getOwnerId())
      .getDeck()
      .getDrawPile();
    const opponentCard = this.getGameSession().getCardByIndex(
      opponentsDrawPile[
        this.getGameSession().getRandomIntegerForExecution(opponentsDrawPile.length)
      ],
    );
    const myCard = this.getGameSession().getCardByIndex(
      myDrawPile[this.getGameSession().getRandomIntegerForExecution(myDrawPile.length)],
    );
