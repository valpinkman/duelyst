...
const ModifierMirage = require('@duelyst/sdk/modifiers/modifierMirage');
...
class SpellMirage extends SpellApplyEntityToBoard {
  ...
  onApplyEffectToBoardTile(board, x, y, sourceAction) {
    ...
    const targetSpawnPositions = UtilsGameSession.getRandomSmartSpawnPositionsFromPattern(
      this.getGameSession(),
      { x, y },
      CONFIG.PATTERN_3x3,
      card,
    ...
    if (targetSpawnPositions.length > 0) {
      return (() => {
        const result = [];
        for (var position of Array.from<any>(targetSpawnPositions)) {
          ...
          if (spawnAction != null) {
            ...
            result.push(
              this.getGameSession().applyModifierContextObject(
                ModifierMirage.createContextObject(),
                spawnAction.getCard(),
              ),
            );
          } else {
