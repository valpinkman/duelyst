 ...
 *   -> resource packages (tools/generate_packages.js -> packages/data/packages.js)
 *   -> JS bundle (vite build) -> resource copy (non-cdn packages + web assets)
 *
 * Usage: FIREBASE_URL=... node tools/build/build-client.mjs [--skip-packages] [--skip-resources]
...
const PACKAGES_MANIFEST = path.join(rootDir, 'tools/build/packages-manifest.json');
...
function generatePackages(mode) {
  ...
  execFileSync('node', ['tools/generate_packages.js', ...GENERATOR_MODES[mode]], {
    stdio: 'inherit',
  });
  delete require.cache[require.resolve(path.join(rootDir, 'packages/data/packages'))];
  return require(path.join(rootDir, 'packages/data/packages'));
}
