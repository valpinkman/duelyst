...
router.delete('/rank/history/:season_key/rewards', function (req, res, next) {
  ...
  knex('user_rank_history')
    .where({ user_id: user_id, starting_at: season_starting_at.toDate() })
    .first()
    .then((row) => console.log('found: ', row));
  ...
  return knex('users')
    ...
    .then(function (fbRootRef) {
      ...
      FirebasePromises.set(
        _chainState.fbRootRef.child('user-ranking').child(user_id).child('current'),
        {
      ...
      if (_chainState.updateData.top_rank != null) {
        return FirebasePromises.set(
          _chainState.fbRootRef.child('user-ranking').child(user_id).child('top'),
          {
    ...
    .then(function () {
      return res.status(200).json({
        ...
        top_rank:
          _chainState.updateData.rank_top_rank != null
            ? _chainState.updateData.rank_top_rank
            : _chainState.userRow.rank_top_rank,
      });
    });
});
  ...
  return (txPromise = knex
    .transaction(function (tx) {
      tx('user_rank_ratings')
        ...
        .andWhere('season_starting_at', seasonStartingAt)
        .first()
        .then(function (userRankRatingRow) {
          // Update or insert
          if (userRankRatingRow != null) {
            userRatingRowData.ladder_rating = RankModule._ladderRatingForRatingAndWinCount(
              userRatingRowData.rating,
              userRankRatingRow.srank_win_count,
            );
            return tx('user_rank_ratings')
              .where('user_id', user_id)
              .andWhere('season_starting_at', seasonStartingAt)
              .update(userRatingRowData);
          } else {
  ...
  return Promise.all([
    ...
    .then((fbRootRef) =>
      FirebasePromises.remove(
        fbRootRef.child('users').child(user_id).child('presence').child('ladder_position'),
      ),
    )
    .then(() => res.status(200).json({}));
});
  ...
  return (txPromise = knex
    .transaction(function (tx) {
      RankModule.getUserLadderPosition(tx, user_id, startOfSeasonMonth, MOMENT_UTC_NOW)
        .then((userLadderPosition) => (user_ladder_position = userLadderPosition || null))
        .then(tx.commit)
        .catch(tx.rollback);
    })
...
router.delete('/rank/history/last', function (req, res, next) {
  ...
  return Promise.all([
    knex('user_rank_history')
      .where({ user_id: user_id, starting_at: previous_season_starting_at.toDate() })
      .delete(),
    knex('user_rank_ratings')
      .where({ user_id: user_id, season_starting_at: previous_season_starting_at.toDate() })
      .delete(),
  ])
    .then(function () {
      ...
      return Promise.all([
        ...
        knex('user_rank_ratings')
          .where({ user_id: user_id, season_starting_at: current_season_starting_at })
          .update(_chainState.updateRankRatingData),
...
router.post('/inventory/cards', function (req, res, next) {
  ...
  return (txPromise = knex
    .transaction((tx) =>
      InventoryModule.giveUserCards(txPromise, tx, user_id, cardIds, 'QA', 'QA', 'QA gift'),
    )
    .then(() => res.status(200).json({})));
});
...
router.post('/inventory/card_set_with_spirit', function (req, res, next) {
  ...
  return (txPromise = knex
    .transaction((tx) => InventoryModule.buyRemainingSpiritOrbsWithSpirit(user_id, cardSetId))
    .then(() => res.status(200).json({}))
  ...
  return (txPromise = knex
    .transaction((tx) =>
      tx('user_card_collection')
        ...
        .then(function () {
          return PromiseUtils.map(_chainState.ownedUnusedCards, (cardId) =>
            tx('user_cards').where('user_id', user_id).andWhere('card_id', cardId).delete(),
          );
        }),
    )
  ...
  return (txPromise = knex
    .transaction((tx) =>
      tx('user_card_collection')
        ...
        .then(function () {
          return PromiseUtils.map(_chainState.ownedBloodbornCards, (cardId) =>
            tx('user_cards').where('user_id', user_id).andWhere('card_id', cardId).delete(),
          );
        })
        .then(() =>
          Promise.all([
            tx('user_spirit_orbs_opened')
              .where('user_id', user_id)
              .andWhere('card_set', SDK.CardSet.Bloodborn)
              .delete(),
            tx('user_spirit_orbs')
              .where('user_id', user_id)
              .andWhere('card_set', SDK.CardSet.Bloodborn)
  ...
  return (txPromise = knex
    .transaction((tx) =>
      tx('user_card_collection')
        ...
        .then(function () {
          return PromiseUtils.map(_chainState.ownedUnityCards, (cardId) =>
            tx('user_cards').where('user_id', user_id).andWhere('card_id', cardId).delete(),
          );
        })
        .then(() =>
          Promise.all([
            tx('user_spirit_orbs_opened')
              .where('user_id', user_id)
              .andWhere('card_set', SDK.CardSet.Unity)
              .delete(),
            tx('user_spirit_orbs')
              .where('user_id', user_id)
              .andWhere('card_set', SDK.CardSet.Unity)
      ...
      return Promise.all([
        ...
        FirebasePromises.set(
          _chainState.fbRootRef
            .child('users')
            .child(user_id)
            .child('free_card_of_the_day_claimed_at'),
          twoDaysAgoMoment.valueOf(),
        ),
      ]);
    })
...
router.put('/quests/current/progress', function (req, res, next) {
  const user_id = req.user.d.id;
  const { quest_slots } = req.body;
  var txPromise = knex.transaction((tx) =>
    PromiseUtils.each(quest_slots, (quest_slot) =>
      tx('user_quests')
        .first()
        .where({ user_id: user_id, quest_slot_index: quest_slot })
...
router.put('/quests/generated_at', function (req, res, next) {
  ...
  return knex('users')
    .where({ id: user_id })
    .first('daily_quests_generated_at')
    .then(function (row) {
      _chainState.previousGeneratedAt = row.daily_quests_generated_at;
      _chainState.newGeneratedAtMoment = moment
        .utc(row.daily_quests_generated_at)
      ...
      return Promise.all([
        FirebasePromises.set(
          _chainState.fbRootRef
            ...
            .child('current')
            .child('updated_at'),
          _chainState.newGeneratedAtMoment.valueOf(),
        ),
        FirebasePromises.set(
          _chainState.fbRootRef
            .child('user-quests')
...
router.post('/quests/setup_frostfire_2016', function (req, res, next) {
  ...
  return Promise.all([
    ...
    knex('user_quests_complete')
      .where('user_id', user_id)
      .andWhere('quest_type_id', 30001)
      .delete(),
  ])
    .then(() => QuestsModule.generateDailyQuests(user_id, moment.utc('2016-12-02')))
    .then(() =>
      Promise.all([
    ...
    .then(() =>
      Promise.all([
        knex('user_quests')
          .where('quest_slot_index', 0)
          .andWhere('user_id', user_id)
          .update({ progress: 3 }),
        knex('user_quests')
          .where('quest_slot_index', 1)
          .andWhere('user_id', user_id)
          .update({ progress: 3 }),
        knex('user_quests')
          .where('quest_slot_index', QuestsModule.SEASONAL_QUEST_SLOT)
          .andWhere('user_id', user_id)
          .update({ progress: 13 }),
      ]),
    )
...
router.post('/quests/setup_seasonal_quest', function (req, res, next) {
  ...
  if (sdkQuestForGenerationTime == null) {
    res
      .status(403)
      .json({ message: 'No seasonal quest for: ' + generateQuestsAtMoment.toString() });
    return;
  }
  ...
  return Promise.all([
    ...
    knex('user_quests_complete')
      .where('user_id', user_id)
      .andWhere('quest_type_id', sdkQuestForGenerationTime.getId())
      .delete(),
  ])
    .then(() => QuestsModule.generateDailyQuests(user_id, generateQuestsAtMoment))
    .then(() =>
      Promise.all([
...
router.post('/quests/setup_promotional_quest', function (req, res, next) {
  ...
  const sdkQuestForGenerationTime =
    SDK.QuestFactory.promotionalQuestForMoment(generateQuestsAtMoment);
  ...
  return Promise.all([
    ...
    .then(() =>
      Promise.all([
        knex('user_quests')
          .where('quest_slot_index', QuestsModule.PROMOTIONAL_QUEST_SLOT)
          .andWhere('user_id', user_id)
          .update({ progress: progressToStartWith }),
      ]),
    )
    .then(() => SyncModule._syncUserFromSQLToFirebase(user_id))
    .then(() => res.status(200).json({}));
...
router.post('/faction_progression/add_level', function (req, res, next) {
  ...
  if (factionId != null) {
    return knex('user_faction_progression')
      .where({ user_id: user_id, faction_id: factionId })
      .first()
      .then(function (factionProgressionRow) {
        if (factionProgressionRow == null) {
          return UsersModule.createFactionProgressionRecord(
            user_id,
            factionId,
            generatePushId(),
            SDK.GameType.Ranked,
        ...
        if (factionProgressionRow == null) {
          ...
          if (deltaXP <= 0) {
            return Promise.reject(
              `Cannot increase level for faction ${factionId}, currently at ${currentLevel}`,
            );
          } else {
            const winsNeeded = Math.ceil(deltaXP / SDK.FactionProgression.winXP);
            const promises = [];
            for (
              let i = 0, end = winsNeeded, asc = end >= 0;
              asc ? i < end : i > end;
              asc ? i++ : i--
            ) {
              promises.push(
...
router.post('/faction_progression/set_all_levels_to_10', function (req, res, next) {
  ...
  return knex('user_faction_progression')
    ...
    .then(function (factionRows) {
      ...
      return PromiseUtils.each(winsPerFaction, (factionId) =>
        UsersModule.updateUserFactionProgressionWithGameOutcome(
          user_id,
          factionId,
          true,
  ...
  return knex('user_gauntlet_run')
    ...
    .then(function (gauntletData) {
      ...
      if (!sdkCard.getIsGeneral()) {
        return Promise.reject(
          new Error('Current Gauntlet deck does not have general in expected slot'),
        );
      }
...
router.post('/gauntlet/progress', function (req, res, next) {
  ...
  return Redis.GameManager.generateGameId()
    .then((gameId) => GauntletModule.updateArenaRunWithGameOutcome(userId, isWinner, gameId, false))
    .then((runData) => res.status(200).json(runData))
    .catch((error) => res.status(403).json({ message: error.toString() }));
});
  ...
  return knex('user_gauntlet_run')
    ...
    .then(function (gauntletData) {
      var recursiveSelect = function (gauntletData) {
        if (
          gauntletData == null ||
          gauntletData.card_choices == null ||
          gauntletData.card_choices[0] == null
        ) {
          ...
          return GauntletModule.chooseCard(userId, cardIdChoice).then((newGauntletData) =>
  ...
  return (txPromise = knex
    .transaction(function (tx) {
      Promise.all([
        tx('user_emotes')
          .where('user_id', userId)
          .andWhere('emote_id', CosmeticsLookup.Emote.OtherSnowChaserHoliday2015)
          .delete(),
        tx('user_rewards')
          .where('user_id', userId)
          .andWhere('source_id', GiftCrateLookup.WinterHoliday2015)
          .delete(),
        tx('user_gift_crates')
          .where('user_id', userId)
          .andWhere('crate_type', GiftCrateLookup.WinterHoliday2015)
          .delete(),
      ])
        .then(() =>
          GiftCrateModule.addGiftCrateToUser(
            ...
            tx,
            userId,
            GiftCrateLookup.WinterHoliday2015,
          ),
        )
        .then(tx.commit)
        .catch(tx.rollback);
  ...
  return (txPromise = knex
    .transaction(function (tx) {
      Promise.all([
        tx('user_rewards')
          .where('user_id', userId)
          .andWhere('source_id', GiftCrateLookup.FebruaryLag2016)
          .delete(),
        tx('user_gift_crates')
          .where('user_id', userId)
          .andWhere('crate_type', GiftCrateLookup.FebruaryLag2016)
          .delete(),
      ])
        .then(() =>
          GiftCrateModule.addGiftCrateToUser(
            ...
            tx,
            userId,
            GiftCrateLookup.FebruaryLag2016,
          ),
        )
        .then(tx.commit)
        .catch(tx.rollback);
  ...
  return (txPromise = knex
    .transaction((tx) =>
      CosmeticChestsModule.giveUserChest(
        ...
        'soft',
        generatePushId(),
        moment.utc().subtract(hoursBack, 'hours'),
      )
        .then(tx.commit)
        .catch(tx.rollback),
    )
...
router.post(
  '/daily_challenge',
  (req, res, next) => res.status(403).json({}),
  /*
  ...
  return DuelystFirebase.connect()
    ...
    .then(function (fbRootRef) {
      ...
      return FirebasePromises.update(
        _chainState.fbRootRef.child('daily-challenges').child(dateKey),
        {
          isQAReady: true,
        },
      );
...
router.post('/migration/prismatic_backfill', function (req, res, next) {
  ...
  return knex('users')
    ...
    .then(function () {
      ...
      return PromiseUtils.each(__range__(1, numOrbs, true), (index) =>
        knex('user_spirit_orbs_opened').insert({
          id: generatePushId(),
          user_id,
...
router.delete('/boss_event', function (req, res, next) {
  ...
  return DuelystFirebase.connect()
    .getRootRef()
    .then((fbRootRef) => FirebasePromises.remove(fbRootRef.child('boss-events').child(bossEventId)))
    .then(function () {
...
router.delete('/boss_event/rewards', function (req, res, next) {
  ...
  return Promise.all([
    ...
    knex('user_cosmetic_chests')
      .where('user_id', user_id)
      .andWhere('chest_type', SDK.CosmeticsChestTypeLookup.Boss)
      .delete(),
    knex('user_cosmetic_chests_opened')
      .where('user_id', user_id)
      .andWhere('chest_type', SDK.CosmeticsChestTypeLookup.Boss)
      .delete(),
  ])
...
router.put('/boss_event', function (req, res, next) {
  ...
  return DuelystFirebase.connect()
    ...
    .then(function () {
      return FirebasePromises.set(
        _chainState.fbRootRef.child('boss-events').child(bossEventId),
        bossEventData,
      );
    })
    .then(function () {
...
router.put('/rift/duplicates', function (req, res, next) {
  ...
  return knex('user_rift_runs')
    ...
    .where('user_id', user_id)
    .then((riftRuns) =>
      PromiseUtils.each(riftRuns, function (riftRun) {
        if (riftRun.card_choices != null) {
          return knex('user_rift_runs')
            .where('ticket_id', riftRun.ticket_id)
            .update({
