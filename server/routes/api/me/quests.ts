...
router.post('/beginner', function (req, res, next) {
  ...
  return QuestsModule.generateBeginnerQuests(user_id)
    ...
    .catch(onType(Errors.NoNeedForNewBeginnerQuestsError, (e) => res.status(304).json({})))
    .catch(function (error) {
      // oops, looks like we have an error
      Logger.module('API').debug(
        `BEGINNER quests failed to generate for ${user_id.blue}`.red +
          ' ERROR: ' +
          util.inspect(error),
      );
      return next(error);
...
router.post('/daily', function (req, res, next) {
  ...
  return QuestsModule.needsDailyQuests(user_id)
    .then(function (value) {
      ...
      if (!value) {
        Logger.module('API').debug(
          `DAILY quests are still less than a day old for ${user_id.blue}`,
        );
        ...
        return QuestsModule.generateDailyQuests(user_id)
          ...
          .catch(
            onType(Errors.FirebaseTransactionDidNotCommitError, function (error) {
          ...
          .catch(function (error) {
            // oops, looks like we have an error
            Logger.module('API').debug(
              `DAILY quests failed to generate for ${user_id.blue}`.red +
                ' ERROR: ' +
                util.inspect(error),
            );
            return next(error);
    ...
    .catch(function (error) {
      Logger.module('API').debug(
        `Failed to detect if we need daily quests for ${user_id.blue}`.red +
          ' ERROR: ' +
          util.inspect(error),
      );
      return next(error);
...
router.put('/daily/:quest_index', function (req, res, next) {
  ...
  return QuestsModule.mulliganDailyQuest(user_id, quest_index)
    ...
    .catch(
      onType(Errors.QuestCantBeMulliganedError, function (error) {
        Logger.module('API').debug(
          `DAILY quest ${quest_index} can't be MULLIGANED again today for ${user_id.blue}`.yellow,
        );
    ...
    .catch(function (error) {
      // oops, looks like we have an error
      Logger.module('API').debug(
        `DAILY quest ${quest_index} failed to mulligan for ${user_id.blue}`.red +
          ' ERROR: ' +
          util.inspect(error),
      );
      return next(error);
