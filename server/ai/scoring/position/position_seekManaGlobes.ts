...
const position_seekManaGlobes = function (gameSession, unit, position) {
  ...
  const manaGlobes = _.filter(
    gameSession.getBoard().getTiles(true),
    (tile) => tile.getBaseCardId() === SDK.Cards.Tile.BonusMana && !tile.getDepleted(),
  );
  if (manaGlobes.length > 0) {
    ...
    const distanceFromNearestGlobe = distanceBetweenBoardPositions(
      position,
      nearestGlobe.getPosition(),
    );
