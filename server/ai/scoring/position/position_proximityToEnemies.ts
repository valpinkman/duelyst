...
const position_proximityToEnemies = function (gameSession, unit, position) {
  ...
  if (!isUnitEvasive(unit)) {
    ...
    score +=
      _.reject(gameSession.getBoard().getCardsAroundPosition(position, CardType.Unit, 1), (card) =>
        card.getIsSameTeamAs(unit),
      ).length *
      (unit.getHP() < BOUNTY.HIGH_HP_THRESHOLD
        ? BOUNTY.QUANTITY_SURROUNDING_ENEMIES_LOW_HP
        : BOUNTY.QUANTITY_SURROUNDING_ENEMIES_HIGH_HP);
    /// /Logger.module("AI").debug("[G:" + gameSession.gameId + "] scoreForUnit_module_proximityToEnemies() => unit " + unit.getLogName() + ". score = " + score);
  }
