...
class RiftModule {
  ...
  static buyRiftTicketWithGold(userId) {
    ...
    if (!userId) {
      Logger.module('RiftModule').log(
        `buyRiftTicketWithGold() -> invalid user ID - ${userId}.`.red,
      );
      return Promise.reject(
        new Error(`Can not buy rift ticket with gold : invalid user ID - ${userId}`),
      );
    }
    ...
    var txPromise = knex
        ...
        tx.first()
          ...
          .then(function (userRow) {
            ...
            if (userRow.wallet_gold >= CONFIG.RIFT_TICKET_GOLD_PRICE) {
              ...
              const final_wallet_gold = (_chainState.final_wallet_gold =
                userRow.wallet_gold - CONFIG.RIFT_TICKET_GOLD_PRICE);
              ...
              Logger.module('RiftModule').log(
                `buyRiftTicketWithGold() -> Cannot buy ticket because user ${userId.blue} has insufficient funds`
                  .red,
              );
              return Promise.reject(
                new Errors.InsufficientFundsError(
                  `Insufficient funds in wallet to buy rift ticket for ${userId}`,
                ),
              );
            }
          })
      ...
      .then(function () {
        Logger.module('RiftModule').log(
          `buyRiftTicketWithGold() -> User ${userId.blue}`.green +
            ` purchased ticket ${_chainState.ticketId}.`.green,
        );
  ...
  static claimFirstFreeRiftTicket(userId) {
    ...
    if (!userId) {
      Logger.module('RiftModule').log(
        `claimFirstFreeRiftTicket() -> invalid user ID - ${userId}.`.red,
      );
      return Promise.reject(
        new Error(`Can not claim free rift ticket : invalid user ID - ${userId}`),
      );
    }
    ...
    var txPromise = knex
      .transaction(function (tx) {
        Promise.all([
          ...
          .then(function ([userRow, ticketRow, usedTicketRow, existingRun]) {
            ...
            if (ticketRow != null) {
              Logger.module('RiftModule').log(
                `claimFirstFreeRiftTicket() -> user ID - ${userId} already has a ticket.`.red,
              );
              return Promise.reject(
                new Error(
                  `Can not claim free rift ticket : user ID - ${userId} already has a ticket`,
                ),
              );
            }
            ...
            if (usedTicketRow != null) {
              ...
              return Promise.reject(
                new Error(
                  `Can not claim free rift ticket : user ID - ${userId} already has a used ticket`,
                ),
              );
            }
            ...
            if (existingRun != null) {
              Logger.module('RiftModule').log(
                `claimFirstFreeRiftTicket() -> user ID - ${userId} already has an active run.`.red,
              );
              return Promise.reject(
                new Error(
                  `Can not claim free rift ticket : user ID - ${userId} already has an active rift run`,
                ),
              );
            }
      ...
      .then(function () {
        Logger.module('RiftModule').debug(
          `claimFirstFreeRiftTicket() -> User ${userId.blue}`.green +
            ` claimed free first rift ticket ${_chainState.ticketId}.`.green,
        );
  ...
  static addRiftTicketToUser(trxPromise, trx, userId, transactionType, transactionId = null) {
    ...
    return knex
      ...
      .then(function () {
        Logger.module('RiftModule').log(
          `addRiftTicketToUser() -> added ${ticketId} to user ${userId.blue}.`.green,
        );
        return Promise.resolve(ticketId);
      });
  }
    ...
    var txPromise = knex
      .transaction(function (tx) {
        Promise.all([
          ...
          tx('user_rift_runs').first().where('user_id', userId).forUpdate(),
          tx('user_rift_run_stored_upgrades')
            .select('id')
            .where('user_id', userId)
            .andWhere('assigned_ticket_id', null)
            .forUpdate(),
        ])
          .then(function ([userRow, ticketRow, existingRun, storedUpgradeRows]) {
            ...
            if (ticketRow && (ticketRow != null ? ticketRow.user_id : undefined) === userId) {
              ...
              if (_chainState.isFirstRun) {
                _chainState.runData.upgrades_available_count =
                  RiftModule.RIFT_FIRST_RUN_FREE_UPGRADES;
              }

              // TODO: must make sure fb and db are updated with user total stored upgrades to 0

              if (_chainState.storedUpgradeRows != null) {
                _chainState.runData.stored_upgrades = _.map(
                  _chainState.storedUpgradeRows,
                  (row) => row.id,
                );
              }
              ...
              return Promise.reject(
                new Errors.NotFoundError('Could not start run: rift ticket not found.'),
              );
            }
          })
          ...
          .then(function () {
            ...
            for (var row of Array.from<any>(_chainState.storedUpgradeRows)) {
              storedUpgradeUpdatePromises.push(
                tx('user_rift_run_stored_upgrades').where('id', row.id).update({
          ...
          .then(function (fbRootRef) {
            ...
            allfbPromises.push(
              FirebasePromises.set(
                fbRootRef.child('user-rift-runs').child(userId).child(ticketId),
                _chainState.runData,
              ),
            );
            allfbPromises.push(
              FirebasePromises.set(
                fbRootRef.child('users').child(userId).child('rift_stored_upgrade_count'),
                0,
              ),
            );
      ...
      .then(function () {
        Logger.module('RiftModule').log(
          `startRun() -> User ${userId.blue}`.green +
            ` started run ${_chainState.runData.ticket_id}.`.green,
        );
  ...
  static chooseGeneral(userId, ticketId, generalId) {
    ...
    if (!generalId) {
      Logger.module('RiftModule').log(
        `chooseGeneral() -> ERROR: invalid general ID: ${generalId}`.red,
      );
    ...
    const txPromise = knex
      .transaction(function (tx) {
        tx('user_rift_runs')
          ...
          .then(function (existingRun) {
            if (existingRun != null) {
              if (
                existingRun.general_choices != null &&
                !_.contains(existingRun.general_choices, generalId)
              ) {
              ...
              _chainState.runData.faction_id = SDK.CardFactory.cardForIdentifier(
                generalId,
                SDK.GameSession.current(),
              ).getFactionId();
              _chainState.runData.deck = _.map(
                SDK.FactionFactory.starterDeckForFactionLevel(_chainState.runData.faction_id, 12),
                (c) => c.id,
              );
              _chainState.runData.deck.shift();
              _chainState.runData.deck.unshift(generalId);

              return tx('user_rift_runs')
                .where('user_id', userId)
                .andWhere('ticket_id', ticketId)
                .update({
          ...
          .then(function (fbRootRef) {
            if (_chainState.runData.updated_at) {
              _chainState.runData.updated_at = moment.utc(_chainState.runData.updated_at).valueOf();
            }
      ...
      .then(function () {
        Logger.module('RiftModule').log(
          `chooseGeneral() -> User ${userId.blue}`.green +
            ` chose general ${generalId} for run ${_chainState.runData.ticket_id}.`.green,
        );
   ...
   */
  static updateRiftRunWithGameOutcome(
    userId,
    ticketId,
    isWinner,
    gameId,
    ...
    if (
      ...
      gameSessionData.aiPlayerId != null &&
      gameSessionData.aiPlayerId === userId
    ) {
    ...
    if (!userId) {
      Logger.module('RiftModule').log(
        `updateRiftRunWithGameOutcome() -> ERROR: invalid user ID: ${userId}`.red,
      );
      return Promise.reject(new Error(`invalid user ID: ${userId}`));
    }
    ...
    if (!ticketId) {
      Logger.module('RiftModule').log(
        `updateRiftRunWithGameOutcome() -> ERROR: invalid ticket ID: ${ticketId}`.red,
      );
      return Promise.reject(new Error(`invalid ticket ID: ${ticketId}`));
    }
    ...
    if (!gameId) {
      Logger.module('RiftModule').log(
        `updateRiftRunWithGameOutcome() -> ERROR: invalid game ID: ${gameId}`.red,
      );
    ...
    if (gameSessionData == null || gameSessionData.gameSetupData == null) {
      Logger.module('RiftModule').log(
        `updateRiftRunWithGameOutcome() -> ERROR: invalid game data: ${gameSessionData}`.red,
      );
      return Promise.reject(new Error(`invalid game data: ${gameSessionData}`));
    }

    Logger.module('RiftModule').debug(
      `updateRiftRunWithGameOutcome() -> user ${userId} ticket ${ticketId}`,
    );
      ...
      .transaction((tx) =>
        PromiseUtils.withTimeout(
          Promise.resolve(tx('users').first('id').where('id', userId).forUpdate())
            ...
            .then(function ([userRow, existingRun]) {
              if (existingRun != null) {
                ...
                if (_chainState.runData.rift_level == null) {
                  _chainState.runData.rift_level = 1;
                }
                ...
                const riftLevelAfter = RiftHelper.levelForPoints(
                  existingRun.rift_points + riftPointsEarned,
                );
                ...
                _chainState.runData.updated_at = NOW_UTC_MOMENT.toDate();
                if (_chainState.runData.games == null) {
                  _chainState.runData.games = [];
                }
                if (_chainState.runData.rift_points == null) {
                  _chainState.runData.rift_points = 0;
                }
                _chainState.runData.rift_points += _chainState.riftPointsEarned;
                if (_chainState.runData.upgrades_available_count == null) {
                  _chainState.runData.upgrades_available_count = 0;
                }
                ...
                if (!isDraw) {
                  ...
                  const winnerRiftRating = __guard__(
                    UtilsGameSession.getPlayerSetupDataForPlayerId(gameSessionData, winnerUserId),
                    (x) => x.riftRating,
                  );
                  const loserRiftRating = __guard__(
                    UtilsGameSession.getPlayerSetupDataForPlayerId(
                      gameSessionData,
                      UtilsGameSession.getOpponentIdToPlayerId(gameSessionData, winnerUserId),
                    ),
                    (x1) => x1.riftRating,
                  );

                  riftRatingDelta = RiftModule._getRiftRatingDeltaForGameRiftRatings(
                    winnerRiftRating,
                    loserRiftRating,
                  );
                  if (!isWinner) {
                ...
                Logger.module('RiftModule').debug(
                  `updateRiftRunWithGameOutcome() -> rift level: ${_chainState.runData.rift_level} points: ${_chainState.runData.rift_points} points earned: ${_chainState.riftPointsEarned}`,
                );
                ...
                allPromises.push(
                  tx('user_games').where('user_id', userId).andWhere('game_id', gameId).update({
            ...
            .then(function () {
              // Update redis
              return RiftManager.updateUserRunRiftRating(
                userId,
                ticketId,
                _chainState.runData.rift_rating,
                NOW_UTC_MOMENT,
              );
            })
            .then(() => DuelystFirebase.connect().getRootRef())
            .then(function (fbRootRef) {
              ...
              allFbPromises.push(
                FirebasePromises.set(
                  fbRootRef.child('user-games').child(userId).child(gameId).child('rift_points'),
                  _chainState.runData.rift_points,
                ),
              );
              allFbPromises.push(
                FirebasePromises.set(
                  fbRootRef
                    ...
                    .child(userId)
                    .child(gameId)
                    .child('rift_points_earned'),
                  _chainState.riftPointsEarned,
                ),
              );
              allFbPromises.push(
                FirebasePromises.set(
              ...
              allFbPromises.push(
                FirebasePromises.set(
                  fbRootRef
                    ...
                    .child(userId)
                    .child(gameId)
                    .child('rift_rating_earned'),
                  _chainState.riftRatingDelta,
                ),
              );
          ...
          onType(PromiseUtils.TimeoutError, function (e) {
            Logger.module('RiftModule').error(
              `updateArenaRunWithGameOutcome() -> ERROR, operation timeout for u:${userId} g:${gameId}`,
            );
            throw e;
          }),
        ),
  ...
  static getRiftRunDeck(userId, ticketId) {
    ...
    return knex('user_rift_runs')
      ...
      .then(function (run) {
        Logger.module('RiftModule').debug(
          `getRiftRunDeck() -> found deck for ${userId}: `,
          run != null ? run.deck : undefined,
        );
        if (__guard__(run != null ? run.deck : undefined, (x) => x.length) > 0) {
          ...
          return Promise.reject(
            new Errors.NoArenaDeckError(`Could not load user ${userId} rift ${ticketId} deck.`),
          );
        }
      });
  }
  ...
  static getRunRating(userId, ticketId) {
    ...
    if (!ticketId) {
      Logger.module('RiftModule').log(
        `getRunRating() -> ERROR: invalid ticket ID: ${ticketId}`.red,
      );
      return Promise.reject(new Error(`invalid ticket ID: ${ticketId}`));
    }
  ...
  static getRunMatchmakingMetric(userId, ticketId) {
    ...
    if (!userId) {
      Logger.module('RiftModule').log(
        `getRunMatchmakingMetric() -> ERROR: invalid user ID: ${userId}`.red,
      );
      return Promise.reject(new Error(`invalid user ID: ${userId}`));
    }
    ...
    if (!ticketId) {
      Logger.module('RiftModule').log(
        `getRunMatchmakingMetric() -> ERROR: invalid ticket ID: ${ticketId}`.red,
      );
      return Promise.reject(new Error(`invalid ticket ID: ${ticketId}`));
    }
  ...
  static chooseCardToUpgrade(userId, ticketId, cardId) {
    ...
    if (!userId) {
      Logger.module('RiftModule').log(
        `chooseCardToUpgrade() -> ERROR: invalid user ID: ${userId}`.red,
      );
      return Promise.reject(new Error(`invalid user ID: ${userId}`));
    }
    ...
    if (!cardId) {
      Logger.module('RiftModule').log(
        `chooseCardToUpgrade() -> ERROR: invalid faction ID: ${cardId}`.red,
      );
    ...
    var txPromise = knex
      .transaction(function (tx) {
        tx('user_rift_runs')
          ...
          .then(function (existingRun) {
            if (existingRun != null) {
              const hasStoredUpgrades =
                existingRun.stored_upgrades != null && existingRun.stored_upgrades.length > 0;
              if (existingRun.upgrades_available_count == null) {
              ...
              if (hasStoredUpgrades) {
                ...
                cardChoicesPromise = tx('user_rift_run_stored_upgrades')
                  .first()
                  .where('user_id', userId)
                  .andWhere('id', _chainState.storedUpgradeUsedId)
                  .then(function (storedUpgradeRow) {
                    if (storedUpgradeRow == null || storedUpgradeRow.card_choices == null) {
                      return Promise.reject(
                        new Errors.BadRequestError(
                          `Rift run stored upgrade ${_chainState.storedUpgradeUsedId} does not exist or belong to user ${userId}.`,
                        ),
                      );
                    }
                ...
                cardChoicesPromise = RiftModule._generateCardUpgradeChoices(
                  txPromise,
                  tx,
                  userId,
                  ticketId,
              ...
              return cardChoicesPromise.then(function (cardChoices) {
                _chainState.runData.card_choices = cardChoices;
                const allCardChoicePromises = [];
                allCardChoicePromises.push(
                  tx('user_rift_runs')
                    .where('user_id', userId)
                    .andWhere('ticket_id', ticketId)
                    .update({
                ...
                if (_chainState.storedUpgradeUsedId != null) {
                  allCardChoicePromises.push(
                    tx('user_rift_run_stored_upgrades')
                      .where('id', _chainState.storedUpgradeUsedId)
                      .delete(),
                  );
                }
      ...
      .then(function () {
        Logger.module('RiftModule').log(
          `chooseCardToUpgrade() -> User ${userId.blue}`.green +
            ` chose card ${cardId} for upgrade in run ${_chainState.runData.ticket_id}.`.green,
        );
  ...
  static _getRiftRatingDeltaForGameRiftRatings(winnerRiftRating?, loserRiftRating?) {
    ...
    const ratingAlphaDivisor =
      RiftModule.RIFT_RATING_DIFFERENCE_FOR_MIN_RATING_GAIN -
      RiftModule.RIFT_RATING_DIFFERENCE_FOR_MAX_RATING_GAIN;
    let ratingAlphaQuotient = winnerRiftRating - loserRiftRating;
    ratingAlphaQuotient = Math.max(
      ratingAlphaQuotient,
      RiftModule.RIFT_RATING_DIFFERENCE_FOR_MAX_RATING_GAIN,
    );
    ratingAlphaQuotient = Math.min(
      ratingAlphaQuotient,
      RiftModule.RIFT_RATING_DIFFERENCE_FOR_MIN_RATING_GAIN,
    );
    ...
    let ratingDelta =
      RiftModule.RIFT_MIN_RATING_DELTA +
      (RiftModule.RIFT_MAX_RATING_DELTA - RiftModule.RIFT_MIN_RATING_DELTA) * ratingAlpha;
  ...
  static upgradeCard(userId, ticketId, cardId) {
    ...
    const txPromise = knex
        ...
        tx('user_rift_runs')
          ...
          .then(function (existingRun) {
            if (existingRun != null) {
              ...
              if (
                existingRun.card_choices != null &&
                !_.contains(existingRun.card_choices, cardId)
              ) {
      ...
      .then(function () {
        Logger.module('RiftModule').log(
          `upgradeCard() -> User ${userId.blue}`.green +
            ` chose card ${cardId} as upgrade for run ${_chainState.runData.ticket_id}.`.green,
        );
   ...
   */
  static _generateCardUpgradeChoices(
    txPromise,
    tx,
    userId,
    ticketId,
    ...
    const randomCardSet = function (rarityId, factionId, excludeNeutralBloodborn, excludeUnity) {
      if (excludeNeutralBloodborn == null) {
      ...
      const excludeBloodbornForEpicNeutralFlag =
        rarityId === SDK.Rarity.Epic && factionId === SDK.Factions.Neutral;
      const excludeNeutralBloodbornFlag =
        factionId === SDK.Factions.Neutral && excludeNeutralBloodborn;
      ...
      includeUnitySet =
        includeUnitySet && !(rarityId === SDK.Rarity.Epic && factionId === SDK.Factions.Neutral);
      if (includeUnitySet) {
    ...
    const randomFaction = function () {
      ...
      Logger.module('RiftModule').debug(
        `_generateCardChoices() -> faction ${fId} of factions:`,
        factions,
      );
      return fId;
    };
    ...
    return RiftModule._getSkinnedCardChoices(txPromise, tx, userId, cardIds).then(
      (skinnedCardIds) =>
        RiftModule._getPrismaticCardChoices(txPromise, tx, userId, skinnedCardIds),
    );
  }
  ...
  static sanitizeRunCardChoicesIfNeeded(riftRunRow, systemTime?) {
    ...
    var txPromise = knex.transaction((tx) =>
      tx('users')
        .first('id')
        .where('id', _chainState.userId)
        .forUpdate()
        .then(function () {
          return RiftModule._generateCardUpgradeChoices(
            txPromise,
            tx,
            _chainState.userId,
            _chainState.ticketId,
            _chainState.factionId,
            _chainState.cardIdToUpgrade,
            _chainState.riftLevel,
          );
        })
        .then(function (cardChoices) {
          ...
          return tx('user_rift_runs')
            .where('ticket_id', _chainState.ticketId)
            .andWhere('user_id', _chainState.userId)
            .update({
  ...
  static storeCurrentUpgrade(userId, riftTicketId) {
    ...
    if (!userId) {
      Logger.module('RiftModule').log(
        `storeCurrentUpgrade() -> ERROR: invalid user ID: ${userId}`.red,
      );
      return Promise.reject(new Error(`invalid user ID: ${userId}`));
    }
    ...
    if (!riftTicketId) {
      Logger.module('RiftModule').log(
        `storeCurrentUpgrade() -> ERROR: invalid rift ID: ${riftTicketId}`.red,
      );
    ...
    const txPromise = knex
      .transaction((tx) =>
        Promise.all([
          ...
          tx('user_rift_runs')
            .first()
            .where('user_id', userId)
            .andWhere('ticket_id', riftTicketId)
            .forUpdate(),
        ])
          .then(function ([userRow, existingRun]) {
            if (userRow == null) {
            ...
            if (
              userRow.rift_stored_upgrade_count != null &&
              userRow.rift_stored_upgrade_count >= RiftModule.RIFT_MAX_STORED_UPGRADES
            ) {
              throw new Errors.MaxRiftUpgradesReachedError(
                `User ${userId} already has max rift upgrades stored`,
              );
            }
            ...
            if (_chainState.userRow.rift_stored_upgrade_count == null) {
              _chainState.userRow.rift_stored_upgrade_count = 0;
            }
            ...
            if (existingRun != null) {
              ...
              if (existingRun.disable_storing_upgrade === true) {
                throw new Errors.BadRequestError(
                  `User ${userId} can not store current upgrade for rift run ${riftTicketId}`,
                );
              }
              ...
              allPromises.push(
                tx('user_rift_run_stored_upgrades').insert({
                  id: generatePushId(),
                  user_id: userId,
          ...
          .then(function (fbRootRef) {
            ...
            allFbPromises.push(
              FirebasePromises.update(
                fbRootRef.child('user-rift-runs').child(userId).child(riftTicketId),
                fbRiftUpdateData,
              ),
            );
            allFbPromises.push(
              FirebasePromises.update(fbRootRef.child('users').child(userId), fbUserUpdateData),
            );
      ...
      .then(function () {
        Logger.module('RiftModule').log(
          `storeCurrentUpgrade() -> User ${userId.blue}`.green +
            ` store upgrade for run ${_chainState.runData.ticket_id}.`.green,
        );
  ...
  static rerollCurrentUpgrade(userId, riftTicketId) {
    ...
    if (!userId) {
      Logger.module('RiftModule').log(
        `rerollCurrentUpgrade() -> ERROR: invalid user ID: ${userId}`.red,
      );
      return Promise.reject(new Error(`invalid user ID: ${userId}`));
    }
    ...
    if (!riftTicketId) {
      Logger.module('RiftModule').log(
        `rerollCurrentUpgrade() -> ERROR: invalid rift ID: ${riftTicketId}`.red,
      );
      return Promise.reject(new Error(`invalid card ID: ${riftTicketId}`));
    }
    ...
    var txPromise = knex
        ...
        Promise.all([
          ...
          .then(function ([userRow, existingRun]) {
            ...
            if (existingRun != null) {
              ...
              // Defaults
              _chainState.currentRerollCount =
                _chainState.runData.current_upgrade_reroll_count || 0;
              ...
              _chainState.rerollSpiritCost = RiftHelper.spiritCostForNextReroll(
                _chainState.currentRerollCount,
                _chainState.totalRerollCount,
              );
              if (_chainState.userSpirit < _chainState.rerollSpiritCost) {
                Logger.module('RiftModule').log(
                  `rerollCurrentUpgrade() -> Cannot reroll current upgrade because user ${userId.blue} has insufficient funds`
                    .red,
                );
                return Promise.reject(
                  new Errors.InsufficientFundsError(
                    `Insufficient funds in wallet to reroll current rift upgrade ${userId}`,
                  ),
                );
              }
              ...
              return RiftModule._generateCardUpgradeChoices(
                ...
                userId,
                riftTicketId,
                existingRun.faction_id,
                existingRun.card_id_to_upgrade,
                existingRun.rift_level,
              );
            } else {
              return Promise.reject(new Errors.NotFoundError('Rift run not found.'));
            }
          ...
          .then(function (cardChoices) {
            ...
            allPromises.push(
              InventoryModule.debitSpiritFromUser(
                ...
                tx,
                userId,
                -1 * _chainState.rerollSpiritCost,
                'rift reroll',
                `${riftTicketId}:${_chainState.totalRerollCount}`,
              ),
            );
      ...
      .then(function () {
        Logger.module('RiftModule').debug(
          `rerollCurrentUpgrade() -> User ${userId.blue}`.green +
            ` rerolled upgrade for run ${_chainState.runData.ticket_id}.`.green,
        );
  ...
  static _generateGeneralChoices(txPromise, tx, userId) {
    let generalIds = _.map(
      SDK.FactionFactory.getAllPlayableFactions(),
      (f) => (f.generalIds != null ? f.generalIds.slice(0) : undefined) || [],
    );
    generalIds = _.flatten(generalIds);
    generalIds = _.sample(generalIds, 4);

    return RiftModule._getSkinnedCardChoices(txPromise, tx, userId, generalIds).then(
      (skinnedGeneralIds) =>
        RiftModule._getPrismaticCardChoices(txPromise, tx, userId, skinnedGeneralIds),
    );
  }
  ...
  static _getSkinnedCardChoices(txPromise, tx, userId, cardIds) {
    return new Promise(function (resolve, reject) {
      ...
      return PromiseUtils.map(cardIds, (cardId) =>
        RiftModule._getSkinnedCardChoice(txPromise, tx, userId, cardId).then((skinnedCardId) =>
          cardIdsOut.push(skinnedCardId),
        ),
      )
        .then(() => resolve(cardIdsOut))
  ...
  static _getSkinnedCardChoice(txPromise, tx, userId, cardId) {
    ...
    if (cardSkinIds.length === 0) {
      ...
      return InventoryModule.filterUsableCosmetics(
        txPromise,
        tx,
        userId,
        cardSkinIds,
        ...
        if (usableSkinIds.length <= 0) {
          ...
          return SDK.Cards.getCardIdForCardSkinId(
            usableSkinIds[Math.floor(Math.random() * usableSkinIds.length)],
          );
        }
      });
    }
