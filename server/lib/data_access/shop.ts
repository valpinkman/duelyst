...
class ShopModule {
  ...
  static _addChargeToUser(
    ...
    txPromise
      ...
      .then(function (rootRef) {
        FirebasePromises.setWithPriority(
          rootRef.child('user-charges').child(userId).child(chargeId),
          chargeJson,
          createdAt.valueOf(),
        );
        FirebasePromises.setWithPriority(
          rootRef.child('user-premium-receipts').child(userId).child(chargeId),
          fbReceiptData,
          createdAt.valueOf(),
        );
        return FirebasePromises.safeTransaction(
          rootRef.child('user-purchase-counts').child(userId).child(sku),
          function (purchaseCountRecord) {
            if (purchaseCountRecord == null) {
              purchaseCountRecord = {};
            }
            if (purchaseCountRecord.count == null) {
    ...
    return Promise.all(allPromises)
      ...
      .then(function (rootRef) {
        ...
        allPromises.push(
          FirebasePromises.safeTransaction(rootRef.child('users').child(userId), updateUserLtv),
        );
        return Promise.all(allPromises);
      });
  }
   ...
   * @return  {Promise}
   */
  static _addPremiumChargeToUser(
    txPromise,
    tx,
    userId,
    userRow,
      ...
      .then(function (rootRef) {
        ...
        if (sku === 'STARTERBUNDLE_201604') {
          return FirebasePromises.set(
            rootRef.child('users').child(userId).child('has_purchased_starter_bundle'),
            true,
          );
        }
      });
  ...
  static purchaseProductWithPremiumCurrency(userId, sku, shopSaleId) {
    ...
    Logger.module('ShopModule').debug(
      `purchaseProductWithPremiumCurrency() -> user ${userId} buying ${sku}`,
    );

    // userId must be defined
    if (!userId) {
      Logger.module('ShopModule').debug(
        `purchaseProductWithPremiumCurrency() -> invalid user ID - ${userId != null ? userId.blue : undefined}.`
          .red,
      );
    ...
    if (!sku) {
      Logger.module('ShopModule').debug(
        `purchaseProductWithPremiumCurrency() -> invalid SKU - ${sku != null ? sku.blue : undefined}.`
          .red,
      );
    ...
    if (productData == null) {
      Logger.module('ShopModule').debug(
        `purchaseProductWithPremiumCurrency() -> no product found for SKU - ${sku != null ? sku.blue : undefined}.`
          .red,
      );
    ...
    var txPromise = knex
      .transaction((tx) =>
        tx('users')
          .where('id', userId)
          .first('ltv', 'username', 'has_purchased_starter_bundle')
          .then(function (userRow) {
              ...
              return tx('user_currency_log')
                ...
                .then(function (count) {
                  ...
                  Logger.module('InventoryModule').debug(
                    `purchaseProductWithPremiumCurrency() -> product ${sku} has a purchase limit of ${productData.purchase_limit} and user ${userId.blue} has purchased ${count} so far.`,
                  );
            ...
            if (productData.type === 'cosmetic') {
              return tx('user_cosmetic_inventory')
                .where('user_id', userId)
                .andWhere('cosmetic_id', productData.id)
                .first()
                .then(function (row) {
                  if (row != null) {
                    throw new Errors.AlreadyExistsError(
                      'This cosmetic item is already in the user inventory.',
                    );
                  }
                });
            }
          ...
          .then(function () {
            if (shopSaleId == null) {
              ...
              const bufferedTimeToExpireSales = NOW_UTC_MOMENT.clone().subtract(
                ShopModule.SHOP_SALE_BUFFER_MINUTES,
                'minutes',
              );
              return tx('shop_sales')
                .first()
                .where('sku', sku)
                .andWhere('sale_starts_at', '<', NOW_UTC_MOMENT.toDate())
                .andWhere('sale_ends_at', '>', bufferedTimeToExpireSales.toDate())
                .andWhere('disabled', '=', false)
                .then(function (shopSaleRow) {
                  if (shopSaleRow != null) {
                    throw new Error(
                      `Attempting to purchase an item ${sku} that is on sale ${shopSaleRow.sale_id} without sale price.`,
                    );
                  } else {
              ...
              return tx('shop_sales')
                .first()
                .where('sale_id', shopSaleId)
                .then(function (shopSaleRow) {
                  if (shopSaleRow == null) {
                    throw new Errors.ShopSaleDoesNotExistError(
                      `There is no matching sale with id (${shopSaleId}) for product sku ${sku}.`,
                    );
                  } else if (
                    shopSaleRow.sale_starts_at == null ||
                    moment.utc(shopSaleRow.sale_starts_at).isAfter(NOW_UTC_MOMENT)
                  ) {
                    throw new Errors.ShopSaleDoesNotExistError(
                      `Matching sale with id (${shopSaleId}) for product sku ${sku} has not started.`,
                    );
                  } else if (
                    shopSaleRow.sale_ends_at == null ||
                    moment
                      .utc(shopSaleRow.sale_ends_at)
                      .add(ShopModule.SHOP_SALE_BUFFER_MINUTES, 'minutes')
                      .isBefore(NOW_UTC_MOMENT)
                  ) {
                    throw new Errors.ShopSaleDoesNotExistError(
                      `Matching sale with id (${shopSaleId}) for product sku ${sku} has expired.`,
                    );
                  } else if (shopSaleRow.disabled) {
                    throw new Error(
                      `Matching sale with id (${shopSaleId}) for product sku ${sku} is disabled.`,
                    );
                  } else if (shopSaleRow.sku !== sku) {
                    throw new Errors.ShopSaleDoesNotExistError(
                      `No matching sale with id (${shopSaleId}) for product sku ${sku}.`,
                    );
                  } else {
          ...
          .then(function () {
            ...
            return InventoryModule.debitPremiumFromUser(
              txPromise,
              tx,
              userId,
              _chainState.premCurrencyPrice,
            );
          })
          .then(function () {
            // txPromise, tx, userId, userRow, sku, price, shopSaleId, systemTime
            return ShopModule._addPremiumChargeToUser(
              txPromise,
              tx,
              userId,
              _chainState.userRow,
          ...
          .then(() =>
            ShopModule._awardProductDataContents(
              txPromise,
              tx,
              userId,
              generatePushId(),
    ...
    return txPromise.then(() => DuelystFirebase.connect().getRootRef());
  }
  ...
  static _awardProductDataContents(txPromise, tx, userId, chargeId, productData, systemTime) {
    ...
    if (productData.type === 'card_pack') {
      let asc, end;
      for (
        i = 1, end = productData.qty, asc = end >= 1;
        asc ? i <= end : i >= end;
        asc ? i++ : i--
      ) {
        allPromises.push(
    ...
    if (productData.type === 'gauntlet_tickets') {
      let asc1, end1;
      for (
        i = 1, end1 = productData.qty, asc1 = end1 >= 1;
        asc1 ? i <= end1 : i >= end1;
        asc1 ? i++ : i--
      ) {
        allPromises.push(
          InventoryModule.addArenaTicketToUser(txPromise, tx, userId, 'hard', chargeId),
        );
      }
    }
    ...
    if (productData.type === 'cosmetic') {
      allPromises.push(
        InventoryModule.giveUserCosmeticId(txPromise, tx, userId, productData.id, 'hard', chargeId),
      );
    }
    ...
    // Create an array of promises
    if (
      (productData.bundle_cosmetic_ids != null
        ? productData.bundle_cosmetic_ids.length
        : undefined) > 0
    ) {
      for (cosmeticId of Array.from<any>(productData.bundle_cosmetic_ids)) {
        allPromises.push(
          InventoryModule.giveUserCosmeticId(txPromise, tx, userId, cosmeticId, 'hard', chargeId),
        );
      }
    }
    ...
    if (productData.type === 'starter_bundle') {
      for (var orbsData of Array.from<any>(productData.spirit_orbs)) {
        var asc2, end2;
        for (
          i = 1, end2 = orbsData.count, asc2 = end2 >= 1;
          asc2 ? i <= end2 : i >= end2;
          asc2 ? i++ : i--
        ) {
          allPromises.push(
    ...
    if (productData.type === 'newbie_bundle') {
      allPromises.push(
        InventoryModule.giveUserCards(txPromise, tx, userId, productData.cards, 'hard', chargeId),
      );
    ...
    if (productData.type === 'rift_tickets') {
      let asc3, end3;
      for (
        i = 1, end3 = productData.qty, asc3 = end3 >= 1;
        asc3 ? i <= end3 : i >= end3;
        asc3 ? i++ : i--
      ) {
    ...
    if (productData.type === 'gift_crate') {
      let asc4, end4;
      for (
        i = 1, end4 = productData.qty, asc4 = end4 >= 1;
        asc4 ? i <= end4 : i >= end4;
        asc4 ? i++ : i--
      ) {
        allPromises.push(
    ...
    txPromise.then(function () {
      if (
        productData.type === 'cosmetic' &&
        __guard__(CosmeticsFactory.cosmeticForIdentifier(productData.id), (x) => x.typeId) ===
          CosmeticsTypeLookup.BattleMap
      ) {
      ...
      .transaction((tx) =>
        tx('users')
          ...
          .then(function (userPremCurrencyRow) {
            ...
            if (_chainState.userPremCurrencyRow.amount == null) {
              _chainState.userPremCurrencyRow.amount = 0;
            }
            _chainState.userPremCurrencyRow.amount += amount;

            if (needsInsert) {
              allPromises.push(
                tx('user_premium_currency')
                  .where('user_id', userId)
                  .first('amount')
                  .insert(_chainState.userPremCurrencyRow),
              );
            } else {
              allPromises.push(
                tx('user_premium_currency').where('user_id', userId).first('amount').update({
