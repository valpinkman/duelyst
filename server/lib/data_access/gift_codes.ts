...
class GiftCodesModule {
  static redeemGiftCode(userId, giftCode, systemTime) {
    ...
    return (txPromise = knex
      .transaction(function (tx) {
        Promise.all([
          ...
          .then(function ([userRow, giftCodeRow, progressionRow]) {
            ...
            if (
              giftCodeRow.valid_for_users_created_after != null &&
              moment
                .utc(userRow.created_at)
                .isBefore(moment.utc(giftCodeRow.valid_for_users_created_after))
            ) {
              throw new Errors.BadRequestError(
                'This Gift Code can not be claimed by this account.',
              );
            }

            if (
              giftCodeRow.expires_at != null &&
              moment.utc().isAfter(moment.utc(giftCodeRow.expires_at))
            ) {
              throw new Errors.BadRequestError('This Gift Code has expired.');
            }

            if (
              giftCodeRow.game_count_limit != null &&
              progressionRow.game_count > giftCodeRow.game_count_limit
            ) {
              throw new Errors.BadRequestError(
                `This Gift Code can not be applied to an accont with ${progressionRow.game_count} games played.`,
              );
            }

            let exclusionCheckPromise = Promise.resolve(null);
            if (_chainState.giftCodeRow.exclusion_id != null) {
              exclusionCheckPromise = tx('gift_codes')
                .first()
                .where('claimed_by_user_id', userId)
                .andWhere('exclusion_id', _chainState.giftCodeRow.exclusion_id);
            }
          ...
          .then(function (claimedGiftCodeWithMatchingExclusionRow) {
            ...
            if (_chainState.giftCodeRow.type === 'ks-1') {
              card_ids = _.chain(
                ...
                .filter(
                  (c) =>
                    c.getFactionId() !== SDK.Factions.Vanar &&
                    c.getFactionId() !== SDK.Factions.Magmar,
                )
                .map((c) => c.getId())
                .value();
              allPromises.push(
            ...
            if (_chainState.giftCodeRow.type === 'ks-2') {
              card_ids = SDK.GameSession.getCardCaches()
                .getCardSet(SDK.CardSet.Core)
                .getIsUnlockable(false)
                .getIsCollectible(true)
                .getIsPrismatic(false)
            ...
            if (
              _chainState.giftCodeRow.type === 'rewards' ||
              _chainState.giftCodeRow.type === 'humble'
            ) {
              let i;
              const goldAmount =
                _chainState.giftCodeRow.rewards != null
                  ? _chainState.giftCodeRow.rewards.gold
                  : undefined;
              const spiritAmount =
                _chainState.giftCodeRow.rewards != null
                  ? _chainState.giftCodeRow.rewards.spirit
                  : undefined;
              const spiritOrbs =
                _chainState.giftCodeRow.rewards != null
                  ? _chainState.giftCodeRow.rewards.orbs
                  : undefined;
              const shimzarSpiritOrbs =
                _chainState.giftCodeRow.rewards != null
                  ? _chainState.giftCodeRow.rewards.shimzar_orbs
                  : undefined;
              const comboSpiritOrbs =
                _chainState.giftCodeRow.rewards != null
                  ? _chainState.giftCodeRow.rewards.combo_orbs
                  : undefined;
              const unearthedSpiritOrbs =
                _chainState.giftCodeRow.rewards != null
                  ? _chainState.giftCodeRow.rewards.unearthed_orbs
                  : undefined;
              const immortalSpiritOrbs =
                _chainState.giftCodeRow.rewards != null
                  ? _chainState.giftCodeRow.rewards.immortal_orbs
                  : undefined;
              const mythronSpiritOrbs =
                _chainState.giftCodeRow.rewards != null
                  ? _chainState.giftCodeRow.rewards.mythron_orbs
                  : undefined;
              const gauntletTickets =
                _chainState.giftCodeRow.rewards != null
                  ? _chainState.giftCodeRow.rewards.gauntlet_tickets
                  : undefined;
              const cosmetics =
                _chainState.giftCodeRow.rewards != null
                  ? _chainState.giftCodeRow.rewards.cosmetics
                  : undefined;
              const cardIds =
                _chainState.giftCodeRow.rewards != null
                  ? _chainState.giftCodeRow.rewards.card_ids
                  : undefined;
              const crate_keys =
                _chainState.giftCodeRow.rewards != null
                  ? _chainState.giftCodeRow.rewards.crate_keys
                  : undefined;
              const crates =
                _chainState.giftCodeRow.rewards != null
                  ? _chainState.giftCodeRow.rewards.crates
                  : undefined;
              ...
              if (shimzarSpiritOrbs) {
                let asc1, end1;
                for (
                  i = 0, end1 = shimzarSpiritOrbs, asc1 = end1 >= 0;
                  asc1 ? i < end1 : i > end1;
                  asc1 ? i++ : i--
                ) {
                  allPromises.push(
              ...
              if (comboSpiritOrbs) {
                let asc2, end2;
                for (
                  i = 0, end2 = comboSpiritOrbs, asc2 = end2 >= 0;
                  asc2 ? i < end2 : i > end2;
                  asc2 ? i++ : i--
                ) {
                  allPromises.push(
              ...
              if (unearthedSpiritOrbs) {
                let asc3, end3;
                for (
                  i = 0, end3 = unearthedSpiritOrbs, asc3 = end3 >= 0;
                  asc3 ? i < end3 : i > end3;
                  asc3 ? i++ : i--
                ) {
                  allPromises.push(
              ...
              if (immortalSpiritOrbs) {
                let asc4, end4;
                for (
                  i = 0, end4 = immortalSpiritOrbs, asc4 = end4 >= 0;
                  asc4 ? i < end4 : i > end4;
                  asc4 ? i++ : i--
                ) {
                  allPromises.push(
              ...
              if (mythronSpiritOrbs) {
                let asc5, end5;
                for (
                  i = 0, end5 = mythronSpiritOrbs, asc5 = end5 >= 0;
                  asc5 ? i < end5 : i > end5;
                  asc5 ? i++ : i--
                ) {
                  allPromises.push(
              ...
              if (gauntletTickets) {
                let asc6, end6;
                for (
                  i = 0, end6 = gauntletTickets, asc6 = end6 >= 0;
                  asc6 ? i < end6 : i > end6;
                  asc6 ? i++ : i--
                ) {
                  allPromises.push(
            ...
            allPromises.push(
              tx('gift_codes').where('code', giftCode).update({
      ...
      .then(function () {
        Logger.module('GiftCodesModule').debug(
          `redeemGiftCode() -> user ${userId.blue} `.green + ` reedemed code ${giftCode}`.green,
        );
