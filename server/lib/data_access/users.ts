...
class UsersModule {
    ...
    return knex('referral_codes')
      ...
      .then(function (referralCodeRow) {
        if (
          ...
          ((referralCodeRow != null ? referralCodeRow.signup_limit : undefined) == null ||
            (referralCodeRow != null ? referralCodeRow.signup_count : undefined) <
              (referralCodeRow != null ? referralCodeRow.signup_limit : undefined)) &&
          ((referralCodeRow != null ? referralCodeRow.expires_at : undefined) == null ||
            moment
              .utc(referralCodeRow != null ? referralCodeRow.expires_at : undefined)
              .isAfter(MOMENT_NOW_UTC))
        ) {
  ...
  static isValidInviteCode(inviteCode, cb) {
    ...
    return knex('invite_codes')
      .where('code', inviteCode)
      .first()
      .then(function (codeRow) {
        if (
          !config.get('inviteCodesActive') ||
          codeRow != null ||
          inviteCode === 'kumite14' ||
          inviteCode === 'keysign789'
        ) {
          return true;
        } else {
  ...
  static createNewUser(
    ...
    return knex('invite_codes')
      ...
      .then(function (inviteCodeRow) {
        ...
        return Promise.all([UsersModule.userIdForUsername(username), referralCodePromise]);
      })
        ...
        return knex
          .transaction((tx) => {
            ...
            if (_chainState.referralCodeRow != null) {
              Logger.module('USERS').debug(
                `createNewUser() -> using referral code ${referralCode.yellow} for user ${userId.blue} `,
                _chainState.referralCodeRow.params,
              );
              userRecord.referral_code = referralCode;
              updateReferralCodePromise = knex('referral_codes')
                .where('code', referralCode)
                .increment('signup_count', 1)
                .transacting(tx);
              if (
                _chainState.referralCodeRow.params != null
                  ? _chainState.referralCodeRow.params.gold
                  : undefined
              ) {
                if (userRecord.wallet_gold == null) {
                  userRecord.wallet_gold = 0;
                }
                userRecord.wallet_gold +=
                  _chainState.referralCodeRow.params != null
                    ? _chainState.referralCodeRow.params.gold
                    : undefined;
              }
              if (
                _chainState.referralCodeRow.params != null
                  ? _chainState.referralCodeRow.params.spirit
                  : undefined
              ) {
                if (userRecord.wallet_spirit == null) {
                  userRecord.wallet_spirit = 0;
                }
                userRecord.wallet_spirit +=
                  _chainState.referralCodeRow.params != null
                    ? _chainState.referralCodeRow.params.spirit
                    : undefined;
              }
            ...
            Promise.all([
              ...
              .then(function (rootRef) {
                ...
                const starting_gold =
                  __guard__(
                    _chainState.referralCodeRow != null
                      ? _chainState.referralCodeRow.params
                      : undefined,
                    (x) => x.gold,
                  ) || 0;
                const starting_spirit =
                  __guard__(
                    _chainState.referralCodeRow != null
                      ? _chainState.referralCodeRow.params
                      : undefined,
                ...
                allPromises.push(
                  FirebasePromises.set(rootRef.child('users').child(userId), userData),
                );
                allPromises.push(
                  FirebasePromises.set(rootRef.child('username-index').child(username), userId),
                );
                allPromises.push(
  ...
  static changeUsername(userId, newUsername, forceItForNoGold?, systemTime?) {
    const _chainState: Record<string, any> = {};
    if (forceItForNoGold == null) {
      forceItForNoGold = false;
    }
    const MOMENT_NOW_UTC = systemTime || moment().utc();
    const this_obj: Record<string, any> = {};

    return UsersModule.userIdForUsername(newUsername).then(function (existingUserId) {
      if (existingUserId) {
        ...
        return knex.transaction(function (tx) {
          knex('users')
            .where('id', userId)
            .first('username', 'username_updated_at', 'wallet_gold')
            .forUpdate()
            .transacting(tx)
            .then(function (userRow) {
              ...
              if (!forceItForNoGold && userRow.username_updated_at && userRow.username) {
                price = 100;
                const timeSinceLastChange = moment.duration(
                  MOMENT_NOW_UTC.diff(moment.utc(userRow.username_updated_at)),
                );
                if (timeSinceLastChange.asMonths() < 1.0) {
                  throw new Errors.InvalidRequestError(
                    "Username can't be changed twice in one month",
                  );
                }
              }
            ...
            .then(function (rootRef) {
              const updateWalletData = (walletData?) => {
                ...
                if (walletData.gold_amount == null) {
              ...
              const allPromises = [
                ...
                FirebasePromises.set(
                  rootRef.child('users').child(userId).child('presence').child('username'),
                  newUsername,
                ),
              ];
              // if username was null, we skip setting the updated_at flag since it is being set for first time
              ...
              if (!_chainState.oldUsername) {
                ...
                allPromises.push(
                  FirebasePromises.remove(
                    rootRef.child('username-index').child(_chainState.oldUsername),
                  ),
                );
                allPromises.push(
                  FirebasePromises.update(rootRef.child('users').child(userId), {
                    username: newUsername,
                    username_updated_at: MOMENT_NOW_UTC.valueOf(),
                  }),
                );
              }
              ...
              if (_chainState.price > 0) {
                allPromises.push(
                  FirebasePromises.safeTransaction(
                    rootRef.child('user-inventory').child(userId).child('wallet'),
                    updateWalletData,
                  ),
                );
  ...
  static bumpSessionCountAndSyncDataIfNeeded(userId, userData = null, systemTime = null) {
    ...
    if (userData) {
      ...
      startPromise = knex('users')
        .where('id', userId)
        .first('id', 'created_at', 'session_count', 'last_session_at', 'last_session_version');
    }
    ...
    return startPromise
      ...
      .then(function () {
        // migrate user charge counts for purchase limits
        return MigrationsModule.checkIfUserNeedsChargeCountsMigration(_chainState.userData).then(
      ...
      .then(function () {
        return MigrationsModule.checkIfUserNeedsIncompleteGauntletRefund(_chainState.userData).then(
          function (needsMigration) {
            if (needsMigration) {
      ...
      .then(function () {
        return MigrationsModule.checkIfUserNeedsUnlockableOrbsRefund(_chainState.userData).then(
          function (needsMigration) {
            if (needsMigration) {
      ...
      .then(function () {
        ...
        if (
          moment.utc(_chainState.userData.created_at).isBefore(moment.utc('2016-06-18')) &&
          moment.utc(_chainState.userData.last_session_at).isBefore(moment.utc('2016-06-18'))
        ) {
          Logger.module('UsersModule').debug(
            `bumpSessionCountAndSyncDataIfNeeded() -> starting inventory achievements for user - ${userId.blue}.`,
          );
      ...
      .then(function () {
        ...
        if (
          _chainState.userData.last_session_version == null ||
          _chainState.userData.last_session_version !== version
        ) {
  ...
  static updateDaysSeen(userId, userSeenOn) {
    return knex
      .first('created_at', 'seen_on_days')
      .from('users')
      .where('id', userId)
      .then(function (userRow) {
        const recordedDayIndex = AnalyticsUtil.recordedDayIndexForRegistrationAndSeenOn(
          moment.utc(userRow.created_at),
          userSeenOn,
        );
  ...
  static userIdForGameCenterId(gameCenterId, callback) {
    return knex
      .first('id')
      .from('users')
      .where('gamecenter_id', gameCenterId)
      .then((userRow) =>
        PromiseUtils.nodeify(
          new Promise(function (resolve, reject) {
            if (userRow) {
  ...
  static isAllowedToUseDeck(userId, deck, gameType, ticketId, forceValidation) {
    ...
    if (!userId || !deck) {
      Logger.module('UsersModule').debug(
        `isAllowedToUseDeck() -> invalid user ID or deck parameter - ${userId.blue}.`.red,
      );
    ...
    if (!forceValidation && config.get('allCardsAvailable')) {
      Logger.module('UsersModule').debug(
        `isAllowedToUseDeck() -> valid deck because this environment allows all cards ALL_CARDS_AVAILABLE = ${config.get('allCardsAvailable')} - ${userId.blue}.`
          .green,
      );
    ...
    if (!(generalCard != null ? generalCard.isGeneral : undefined)) {
      Logger.module('UsersModule').debug(
        `isAllowedToUseDeck() -> first card in the deck must be a general - ${userId.blue}.`.yellow,
      );
      return Promise.reject(
        new Errors.InvalidDeckError('First card in the deck must be a general'),
      );
    } else {
    ...
    if (gameType === GameType.Gauntlet) {
      Logger.module('UsersModule').debug(
        `isAllowedToUseDeck() -> Allowing ANY arena deck for now - ${userId.blue}.`.green,
      );
      return Promise.resolve(true);
      ...
      const decksExpireMoment = moment
        .utc()
        .subtract(CONFIG.DAYS_BEFORE_GAUNTLET_DECK_EXPIRES, 'days');
      const currentDeckPromise = knex('user_gauntlet_run')
        .first('deck')
        .where('user_id', userId)
        .andWhere('ticket_id', ticketId);
      const oldDeckPromise = knex('user_gauntlet_run_complete')
        .first('deck', 'ended_at')
        .where('user_id', userId)
        .andWhere('id', ticketId)
        .andWhere('ended_at', '>', decksExpireMoment.toDate());
      return Promise.all([currentDeckPromise, oldDeckPromise]).then(function ([
        currentRunRow,
        completedRunRow,
      ]) {
        ...
        if (currentRunRow != null) {
          ...
          return Promise.reject(
            new Errors.InvalidDeckError(
              'Friendly gauntlet deck has no matching recent run, ticket_id: ' + ticketId,
            ),
          );
        }
        const gauntletDeck = matchingRunRow.deck;

        const sortedDeckIds = _.sortBy(
          _.map(deck, (cardObject) => cardObject.id),
          (id) => id,
        );
        ...
        if (sortedDeckIds.length !== sortedGauntletDeckIds.length) {
          return Promise.reject(
            new Errors.InvalidDeckError(
              'Friendly gauntlet deck has incorrect card count: ' + sortedDeckIds.length,
            ),
          );
        }

        for (
          let i = 0, end = sortedDeckIds.length, asc = end >= 0;
          asc ? i < end : i > end;
          asc ? i++ : i--
        ) {
          if (sortedDeckIds[i] !== sortedGauntletDeckIds[i]) {
            return Promise.reject(
              new Errors.InvalidDeckError('Friendly gauntlet deck has incorrect cards'),
            );
          }
        }
      ...
      for (var card of Array.from<any>(deck)) {
        ...
        if (sdkCard.factionId !== deckFactionId && sdkCard.factionId !== SDK.Factions.Neutral) {
          Logger.module('UsersModule').debug(
            `isAllowedToUseDeck() -> found a card with faction ${sdkCard.factionId} that doesn't belong in a ${deckFactionId} faction deck - ${userId.blue}.`
              .yellow,
          );
          return Promise.reject(
            new Errors.InvalidDeckError('Deck has cards from more than one faction'),
          );
        }
        if (
          (sdkCard.rarityId !== SDK.Rarity.Fixed && sdkCard.rarityId !== SDK.Rarity.TokenUnit) ||
          sdkCard.getIsUnlockable()
        ) {
          ...
          if (cardSkinId != null) {
            ...
            var unskinnedSDKCard = SDK.GameSession.getCardCaches()
              .getIsSkinned(false)
              .getCardById(unskinnedCardId);
            if (
              unskinnedSDKCard.getRarityId() !== SDK.Rarity.Fixed &&
              unskinnedSDKCard.getRarityId() !== SDK.Rarity.TokenUnit
            ) {
      ...
      const maxDeckSize = CONFIG.DECK_SIZE_INCLUDES_GENERAL
        ? CONFIG.MAX_DECK_SIZE
        : CONFIG.MAX_DECK_SIZE + 1;
      if (basicsOnly) {
        if (deck.length < CONFIG.MIN_BASICS_DECK_SIZE) {
          Logger.module('UsersModule').debug(
            `isAllowedToUseDeck() -> invalid starter deck (${deck.length}) - ${userId.blue}.`
              .yellow,
          );
          return Promise.reject(
            new Errors.InvalidDeckError(
              `Starter decks must have at least ${CONFIG.MIN_BASICS_DECK_SIZE} cards!`,
            ),
          );
          ...
          return Promise.reject(
            new Errors.InvalidDeckError(
              `Starter decks must not have more than ${maxDeckSize} cards!`,
            ),
          );
        }
      } else if (deck.length !== maxDeckSize) {
        Logger.module('UsersModule').debug(
          `isAllowedToUseDeck() -> invalid deck length (${deck.length}) - ${userId.blue}.`.yellow,
        );
      ...
      for (var gameSessionCard of Array.from<any>(gameSessionCards)) {
        ...
        if (!gameSessionCard.getIsAvailable(null, forceValidation)) {
          Logger.module('UsersModule').error(
            `isAllowedToUseDeck() -> Deck has cards (${gameSessionCard.id}) that are not yet available - player ${userId.blue}.`
              .red,
          );
          return Promise.reject(
            new Errors.NotFoundError('Deck has cards that are not yet available'),
          );
        }
        if (gameSessionCard.getIsHiddenInCollection()) {
          Logger.module('UsersModule').error(
            `isAllowedToUseDeck() -> Deck has cards (${gameSessionCard.id}) that are in hidden to collection - player ${userId.blue}.`
              .red,
          );
          return Promise.reject(
            new Errors.InvalidDeckError('Deck has cards that are in hidden to collection'),
          );
        }
        if (
          (gameSessionCard.getIsLegacy() ||
            CardSetFactory.cardSetForIdentifier(gameSessionCard.getCardSetId()).isLegacy != null) &&
          GameType.getGameFormatForGameType(gameType) === GameFormat.Standard
        ) {
          Logger.module('UsersModule').error(
            `isAllowedToUseDeck() -> Deck has cards (${gameSessionCard.id}) that are in LEGACY format but game mode is STANDARD format`,
          );
          return Promise.reject(
            new Errors.InvalidDeckError('Game Mode is STANDARD but deck contains LEGACY card'),
          );
        }
      }
      ...
      if (generalCount !== 1) {
        return Promise.reject(
          new Errors.InvalidDeckError('Deck has ' + generalCount + ' generals'),
        );
      }
      ...
      for (k in mythronCardCountsById) {
        ...
        if (k !== '-1' && v > 1) {
          return Promise.reject(
            new Errors.InvalidDeckError('Deck has more than 1 of a mythron card'),
          );
        }
      }
      ...
      const trialCardCount = _.countBy(gameSessionCards, function (card) {
        ...
        if (
          [
            Cards.Faction1.RightfulHeir,
            Cards.Faction2.DarkHeart,
            Cards.Faction3.KeeperOfAges,
            Cards.Faction4.DemonOfEternity,
            Cards.Faction5.Dinomancer,
            Cards.Faction6.VanarQuest,
            Cards.Neutral.Singleton,
          ].includes(baseCardId)
        ) {
          return 'trialCard';
        } else {
      ...
      for (k in trialCardCount) {
        ...
        if (k !== '-1' && v > 1) {
          return Promise.reject(
            new Errors.InvalidDeckError('Deck has more than 1 total trial card'),
          );
        }
      }
      ...
      const validateCards = function () {
        Logger.module('UsersModule').debug(
          `isAllowedToUseDeck() -> doesUserHaveCards -> ${cardsToValidateAgainstInventory.length} cards to validate - ${userId.blue}.`
            .green,
        );
        ...
        if (cardsToValidateAgainstInventory.length === 0) {
          ...
          return InventoryModule.isAllowedToUseCards(
            Promise.resolve(),
            knex,
            userId,
      ...
      const validateSkins = function () {
        Logger.module('UsersModule').debug(
          `isAllowedToUseDeck() -> doesUserHaveSkins -> ${cardSkinsToValidateAgainstInventory.length} skins to validate - ${userId.blue}.`
            .green,
        );
        if (cardSkinsToValidateAgainstInventory.length === 0) {
          return Promise.resolve(true);
        } else {
          return InventoryModule.isAllowedToUseCosmetics(
            Promise.resolve(),
            knex,
            userId,
            cardSkinsToValidateAgainstInventory,
            SDK.CosmeticsTypeLookup.CardSkin,
          );
        }
      };

      return Promise.all([validateCards(), validateSkins()]);
    } else {
  ...
  static createFactionProgressionRecord(userId, factionId, gameId, gameType, systemTime?) {
    ...
    if (!userId) {
      return Promise.reject(
        new Error(`Can not createFactionProgressionRecord(): invalid user ID - ${userId}`),
      );
    }
    ...
    if (!factionId) {
      return Promise.reject(
        new Error(`Can not createFactionProgressionRecord(): invalid faction ID - ${factionId}`),
      );
    }
      ...
      PromiseUtils.withTimeout(
        Promise.resolve(tx('users').where('id', userId).first('id').forUpdate())
          .then((userRow) =>
            Promise.all([
              userRow,
              tx('user_faction_progression')
                .where({ user_id: userId, faction_id: factionId })
                .first()
                .forUpdate(),
            ]),
          )
          .then(function ([userRow, factionProgressionRow]) {
            ...
            if (factionProgressionRow.xp == null) {
              factionProgressionRow.xp = 0;
            }
            if (factionProgressionRow.game_count == null) {
              factionProgressionRow.game_count = 0;
            }
            if (factionProgressionRow.unscored_count == null) {
              factionProgressionRow.unscored_count = 0;
            }
            if (factionProgressionRow.loss_count == null) {
              factionProgressionRow.loss_count = 0;
            }
            if (factionProgressionRow.win_count == null) {
              factionProgressionRow.win_count = 0;
            }
            if (factionProgressionRow.draw_count == null) {
              factionProgressionRow.draw_count = 0;
            }
            if (factionProgressionRow.single_player_win_count == null) {
              factionProgressionRow.single_player_win_count = 0;
            }
            if (factionProgressionRow.friendly_win_count == null) {
              factionProgressionRow.friendly_win_count = 0;
            }
            if (factionProgressionRow.xp_earned == null) {
            ...
            factionProgressionRow.level = SDK.FactionProgression.levelForXP(
              factionProgressionRow.xp,
            );
          ...
          .then(function (rootRef) {
            ...
            _chainState.factionProgressionRow.updated_at = moment
              .utc(_chainState.factionProgressionRow.updated_at)
              .valueOf();
            return FirebasePromises.set(
              rootRef
                ...
                .child(factionId)
                .child('stats'),
              _chainState.factionProgressionRow,
            );
          })
          .then(() => SyncModule._bumpUserTransactionCounter(tx, userId)),
        10000,
        ...
        onType(PromiseUtils.TimeoutError, function (e) {
          Logger.module('UsersModule').error(
            `createFactionProgressionRecord() -> ERROR, operation timeout for u:${userId} g:${gameId}`,
          );
          throw e;
        }),
      ),
   ...
   * @return  {Promise}          Promise that will notify when complete.
   */
  static updateUserFactionProgressionWithGameOutcome(
    userId,
    factionId,
    isWinner,
    gameId,
    ...
    if (!userId) {
      return Promise.reject(
        new Error(
          `Can not updateUserFactionProgressionWithGameOutcome(): invalid user ID - ${userId}`,
        ),
      );
    }
    ...
    if (!factionId) {
      return Promise.reject(
        new Error(
          `Can not updateUserFactionProgressionWithGameOutcome(): invalid faction ID - ${factionId}`,
        ),
      );
    }
    ...
    var txPromise = knex
      .transaction((tx) =>
        PromiseUtils.withTimeout(
          Promise.resolve(tx('users').first('id', 'is_bot').where('id', userId).forUpdate())
            .then((userRow) =>
              Promise.all([
                userRow,
                tx('user_faction_progression')
            ...
            .then(function ([userRow, factionProgressionRow]) {
              ...
              if (factionProgressionRow.level == null) {
              ...
              if (
                ...
                factionProgressionRow.level >= 10
              ) {
              ...
              let xp_earned = SDK.FactionProgression.xpEarnedForGameOutcome(
                isWinner,
                factionProgressionRow.level,
              );
              ...
              const { xp } = factionProgressionRow;
              const { game_count } = factionProgressionRow;
              const { win_count } = factionProgressionRow;

              const xp_cap = SDK.FactionProgression.totalXPForLevel(
              ...
              if (isUnscored) {
                const { unscored_count } = factionProgressionRow;
              ...
              if (needsInsert) {
                allPromises.push(
                  knex('user_faction_progression').insert(factionProgressionRow).transacting(tx),
                );
              } else {
                allPromises.push(
                  knex('user_faction_progression')
                    .where({ user_id: userId, faction_id: factionId })
                    .update(factionProgressionRow)
                    .transacting(tx),
                );
              }
              ...
              if (!isUnscored && !_chainState.factionProgressionRow.xp_earned) {
                ...
                allPromises.push(
                  knex('user_games')
                    ...
                    .update(_chainState.updateUserGameParams)
                    .transacting(tx),
                );
              } else {
                const level = SDK.FactionProgression.levelForXP(
                  _chainState.factionProgressionRow.xp,
                );

                Logger.module('UsersModule').debug(
                  `updateUserFactionProgressionWithGameOutcome() -> At F${factionId} L:${level} [${_chainState.factionProgressionRow.xp}] earned ${_chainState.factionProgressionRow.xp_earned} for G:${gameId}`,
                );
                ...
                _chainState.updateUserGameParams = {
                  faction_xp:
                    _chainState.factionProgressionRow.xp -
                    _chainState.factionProgressionRow.xp_earned,
                  faction_xp_earned: _chainState.factionProgressionRow.xp_earned,
                };

                allPromises.push(
                  knex('user_faction_progression_events').insert(progressData).transacting(tx),
                );
                allPromises.push(
                  knex('user_games')
                    .where({ user_id: userId, game_id: gameId })
            ...
            .then(function () {
              ...
              if (
                !isUnscored &&
                _chainState.factionProgressionRow &&
                SDK.FactionProgression.hasLeveledUp(
                  _chainState.factionProgressionRow.xp,
                  _chainState.factionProgressionRow.xp_earned,
                )
              ) {
                ...
                if (rewardData != null) {
                  ...
                  if (rewardData.gold != null) {
                    ...
                    // update wallet
                    earnRewardInventoryPromise = InventoryModule.giveUserGold(
                      txPromise,
                      tx,
                      userId,
                      rewardData.gold,
                    ...
                    // update wallet
                    earnRewardInventoryPromise = InventoryModule.giveUserSpirit(
                      txPromise,
                      tx,
                      userId,
                      rewardData.spirit,
                    ...
                    // give cards
                    earnRewardInventoryPromise = InventoryModule.giveUserCards(
                      txPromise,
                      tx,
                      userId,
                      cardIds,
                    ...
                    earnRewardInventoryPromise = InventoryModule.addBoosterPackToUser(
                      txPromise,
                      tx,
                      userId,
                      1,
              ...
              const winCountForRibbons =
                _chainState.factionProgressionRow.win_count -
                (_chainState.factionProgressionRow.single_player_win_count +
                  _chainState.factionProgressionRow.friendly_win_count);
              if (
                isWinner &&
                winCountForRibbons > 0 &&
                winCountForRibbons % 100 === 0 &&
                !_chainState.userRow.is_bot
              ) {
                Logger.module('UsersModule').debug(
                  `updateUserFactionProgressionWithGameOutcome() -> user ${userId.blue}`.green +
                    ` game ${gameId} earned WIN RIBBON for faction ${factionId}`,
                );
            ...
            .then(function () {
              ...
              if (
                ...
                if (_chainState.factionProgressionRow) {
                  // and shouldProcessQuests # TODO: shouldprocessquests? also this may fail for people who already have faction lvl 10 by the time they reach this stage
                  return QuestsModule.updateQuestProgressWithProgressedFactionData(
                    txPromise,
                    tx,
                    userId,
                    _chainState.factionProgressionRow,
            ...
            .then(function (rootRef) {
              ...
              for (var key in _chainState.updateUserGameParams) {
                ...
                allPromises.push(
                  FirebasePromises.set(
                    rootRef.child('user-games').child(userId).child(gameId).child(key),
                    val,
                  ),
                );
              }
              ...
              if (_chainState.ribbon) {
                ...
                allPromises.push(
                  FirebasePromises.safeTransaction(
                    rootRef.child('user-ribbons').child(userId).child(ribbonData.ribbon_id),
                    function (data?) {
                      if (data == null) {
                        data = {};
                      }
                      if (data.ribbon_id == null) {
          ...
          onType(PromiseUtils.TimeoutError, function (e) {
            Logger.module('UsersModule').error(
              `updateUserFactionProgressionWithGameOutcome() -> ERROR, operation timeout for u:${userId} g:${gameId}`,
            );
            throw e;
          }),
        ),
      ...
      .then(function () {
        ...
        if (
          ...
          _chainState.factionProgressionRow.game_count === 1
        ) {
          Jobs.enqueue(
            'update-user-achievements',
            {
        ...
        Logger.module('UsersModule').debug(
          `updateUserFactionProgressionWithGameOutcome() -> user ${userId.blue}`.green +
            ` game ${gameId} (${_chainState.factionProgressionRow.game_count}) faction progression recorded. Unscored: ${isUnscored != null ? isUnscored.toString().cyan : undefined}`
              .green,
        );
        return _chainState.factionProgressionRow;
      })
      .catch(
        onType(Errors.MaxFactionXPForSinglePlayerReachedError, function (e) {
          Logger.module('UsersModule').debug(
            `updateUserFactionProgressionWithGameOutcome() -> user ${userId.blue}`.green +
              ` game ${gameId} for faction ${factionId} not recorded. MAX LVL 11 for single player games reached.`,
          );
          return null;
        }),
      )
      .finally(() =>
        GamesModule.markClientGameJobStatusAsComplete(userId, gameId, 'faction_progression'),
      );
    return txPromise;
  }
   ...
   * @return  {Promise}        Promise that will notify when complete.
   */
  static updateUserProgressionWithGameOutcome(
    userId,
    opponentId,
    isWinner,
    gameId,
    ...
    if (!userId) {
      return Promise.reject(
        new Error(`Can not updateUserProgressionWithGameOutcome(): invalid user ID - ${userId}`),
      );
    }
    ...
    if (!gameId) {
      return Promise.reject(
        new Error(`Can not updateUserProgressionWithGameOutcome(): invalid game ID - ${gameId}`),
      );
    }
    ...
    var txPromise = knex
      .transaction(function (tx) {
        const start_of_day_int = parseInt(
          moment(MOMENT_NOW_UTC).startOf('day').utc().format('YYYYMMDD'),
        );

        return PromiseUtils.withTimeout(
          Promise.resolve(tx('users').first('id').where('id', userId).forUpdate())
            .then((userRow) =>
              Promise.all([
                userRow,
                tx('user_progression').where('user_id', userId).first().forUpdate(),
                tx('user_progression_days')
                  .where({ user_id: userId, date: start_of_day_int })
                  .first()
                  .forUpdate(),
              ]),
            )
            .then(function ([userRow, progressionRow, progressionDayRow]) {
              ...
              _chainState.progressionDayRow = progressionDayRow || {
                user_id: userId,
                date: start_of_day_int,
              };
              if (_chainState.progressionDayRow.game_count == null) {
                _chainState.progressionDayRow.game_count = 0;
              }
              if (_chainState.progressionDayRow.unscored_count == null) {
                _chainState.progressionDayRow.unscored_count = 0;
              }
              _chainState.progressionDayRow.game_count += 1;

              // controls for daily maximum of play rewards
              if (
                _chainState.progressionDayRow.game_count -
                  _chainState.progressionDayRow.unscored_count >
                UsersModule.DAILY_REWARD_GAME_CAP
              ) {
              ...
              if (isDraw) {
                if (_chainState.progressionDayRow.draw_count == null) {
                  _chainState.progressionDayRow.draw_count = 0;
                }
                _chainState.progressionDayRow.draw_count += 1;
              } else if (isWinner) {
                // iterate win count
                if (_chainState.progressionDayRow.win_count == null) {
                  _chainState.progressionDayRow.win_count = 0;
                }
                ...
                // iterate loss count
                if (_chainState.progressionDayRow.loss_count == null) {
                  _chainState.progressionDayRow.loss_count = 0;
                }
              ...
              if (progressionDayRow != null) {
                allPromises.push(
                  knex('user_progression_days')
                    .where({ user_id: userId, date: start_of_day_int })
                    .update(_chainState.progressionDayRow)
                    .transacting(tx),
                );
              } else {
                allPromises.push(
                  knex('user_progression_days')
                    .insert(_chainState.progressionDayRow)
                    .transacting(tx),
                );
              }
              ...
              if (_chainState.progressionRow.game_count == null) {
                _chainState.progressionRow.game_count = 0;
              }
              if (_chainState.progressionRow.unscored_count == null) {
                _chainState.progressionRow.unscored_count = 0;
              }
              ...
              if (_chainState.progressionRow.last_awarded_game_count == null) {
                _chainState.progressionRow.last_awarded_game_count = 0;
              }
              if (_chainState.progressionRow.last_awarded_win_count == null) {
                _chainState.progressionRow.last_awarded_win_count = 0;
              }
              ...
              if (isUnscored) {
                ...
                if (!hasReachedDailyPlayRewardMaxium) {
                  play_count_reward_progress =
                    _chainState.progressionRow.game_count -
                    _chainState.progressionRow.last_awarded_game_count;

                  if (
                    _chainState.progressionRow.game_count > 0 &&
                    play_count_reward_progress > 0 &&
                    play_count_reward_progress % 4 === 0
                  ) {
                    _chainState.progressionRow.last_awarded_game_count =
                      _chainState.progressionRow.game_count;
                ...
                if (isDraw) {
                  if (_chainState.progressionRow.draw_count == null) {
                    _chainState.progressionRow.draw_count = 0;
                  }
                  ...
                  // iterate win count
                  if (_chainState.progressionRow.win_count == null) {
                    _chainState.progressionRow.win_count = 0;
                  }
                  _chainState.progressionRow.win_count += 1;
                  // iterate win streak
                  if (gameType !== GameType.Casual) {
                    if (_chainState.progressionRow.win_streak == null) {
                  ...
                  if (!hasReachedDailyWinRewardMaxium) {
                    win_count_reward_progress =
                      _chainState.progressionRow.win_count -
                      _chainState.progressionRow.last_awarded_win_count;

                    // if we've had 3 wins since last award, the user has earned an award
                    if (
                      _chainState.progressionRow.win_count -
                        _chainState.progressionRow.last_awarded_win_count >=
                      CONFIG.WINS_REQUIRED_FOR_WIN_REWARD
                    ) {
                      _chainState.hasEarnedWinReward = true;
                      _chainState.progressionRow.last_awarded_win_count_at =
                  ...
                  // iterate loss count
                  if (_chainState.progressionRow.loss_count == null) {
                    _chainState.progressionRow.loss_count = 0;
                  }
                  ...
                  if (!isUnscored) {
                    if (_chainState.progressionRow.loss_streak == null) {
                      _chainState.progressionRow.loss_streak = 0;
                    }
              ...
              if (progressionRow != null) {
                allPromises.push(
                  knex('user_progression')
                    .where({ user_id: userId })
                    .update(_chainState.progressionRow)
                    .transacting(tx),
                );
              } else {
                allPromises.push(
                  knex('user_progression').insert(_chainState.progressionRow).transacting(tx),
                );
              }
            ...
            .then(function () {
              const { hasEarnedWinReward } = _chainState;
              const { hasEarnedPlayReward } = _chainState;
              const { hasEarnedFirstWinOfTheDayReward } = _chainState;
              ...
              if (!isUnscored) {
                ...
                if (hasEarnedFirstWinOfTheDayReward) {
                  Logger.module('UsersModule').debug(
                    `updateUserProgressionWithGameOutcome() -> user ${userId.blue} HAS earned a FIRST-WIN-OF-THE-DAY reward at ${_chainState.progressionRow.game_count} games!`,
                  );
                ...
                if (hasEarnedWinReward) {
                  ...
                  Logger.module('UsersModule').debug(
                    `updateUserProgressionWithGameOutcome() -> user ${userId.blue} HAS earned a ${gold_amount}G WIN-COUNT reward!`,
                  );
                ...
                _chainState.codexChapterIdsEarned = SDK.Codex.chapterIdsAwardedForGameCount(
                  _chainState.progressionRow.game_count,
                );

                if (
                  _chainState.codexChapterIdsEarned &&
                  _chainState.codexChapterIdsEarned.length !== 0
                ) {
                  Logger.module('UsersModule').debug(
                    `updateUserProgressionWithGameOutcome() -> user ${userId.blue} HAS earned codex chapters ${_chainState.codexChapterIdsEarned} reward!`,
                  );
                  for (var codexChapterIdEarned of Array.from<any>(
                    _chainState.codexChapterIdsEarned,
                  )) {
                    // set up reward data
                    rewardData = {
                    ...
                    promises.push(
                      InventoryModule.giveUserCodexChapter(
                        txPromise,
                        tx,
                        userId,
                        codexChapterIdEarned,
                        MOMENT_NOW_UTC,
                      ),
                    );
                    promises.push(knex('user_rewards').insert(rewardData).transacting(tx));
            ...
            .then(function (rootRef) {
              ...
              if (_chainState.progressionRow.last_win_at) {
                _chainState.progressionRow.last_win_at = moment
                  .utc(_chainState.progressionRow.last_win_at)
                  .valueOf();
              }
              if (_chainState.progressionRow.last_daily_win_at) {
                _chainState.progressionRow.last_daily_win_at = moment
                  .utc(_chainState.progressionRow.last_daily_win_at)
                  .valueOf();
              }
              if (_chainState.progressionRow.last_awarded_win_count_at) {
                _chainState.progressionRow.last_awarded_win_count_at = moment
                  .utc(_chainState.progressionRow.last_awarded_win_count_at)
                  .valueOf();
              }
              if (_chainState.progressionRow.play_awards_last_maxed_at) {
                _chainState.progressionRow.play_awards_last_maxed_at = moment
                  .utc(_chainState.progressionRow.play_awards_last_maxed_at)
                  .valueOf();
              }
              if (_chainState.progressionRow.win_awards_last_maxed_at) {
                _chainState.progressionRow.win_awards_last_maxed_at = moment
                  .utc(_chainState.progressionRow.win_awards_last_maxed_at)
                  .valueOf();
              }
              if (_chainState.progressionRow.updated_at) {
                _chainState.progressionRow.updated_at = moment
                  .utc()
                  .valueOf(_chainState.progressionRow.updated_at);
              }

              allPromises.push(
                FirebasePromises.set(
                  rootRef.child('user-progression').child(userId).child('game-counter'),
          ...
          onType(PromiseUtils.TimeoutError, function (e) {
            Logger.module('UsersModule').error(
              `updateUserProgressionWithGameOutcome() -> ERROR, operation timeout for u:${userId} g:${gameId}`,
            );
            throw e;
          }),
        );
      ...
      .then(function () {
        return Logger.module('UsersModule').debug(
          `updateUserProgressionWithGameOutcome() -> user ${userId.blue}`.green +
            ` game ${gameId} G:${_chainState.progressionRow.game_count} W:${_chainState.progressionRow.win_count} L:${_chainState.progressionRow.loss_count} U:${_chainState.progressionRow.unscored_count} progression recorded. Unscored: ${isUnscored != null ? isUnscored.toString().cyan : undefined}`
              .green,
        );
      })
   ...
   * @return  {Promise}        Promise that will notify when complete.
   */
  static updateUserBossProgressionWithGameOutcome(
    userId,
    opponentId,
    isWinner,
    gameId,
    ...
    if (!userId) {
      return Promise.reject(
        new Error(
          `Can not updateUserBossProgressionWithGameOutcome(): invalid user ID - ${userId}`,
        ),
      );
    }
    ...
    if (!gameId) {
      return Promise.reject(
        new Error(
          `Can not updateUserBossProgressionWithGameOutcome(): invalid game ID - ${gameId}`,
        ),
      );
    }
    ...
    if (gameType == null || gameType !== SDK.GameType.BossBattle) {
      return Promise.reject(
        new Error(
          `Can not updateUserBossProgressionWithGameOutcome(): invalid game game type - ${gameType}`,
        ),
      );
    }
    ...
    if (bossId == null || sdkBossData == null || sdkBossData.getFactionId() !== SDK.Factions.Boss) {
      return Promise.reject(
        new Error(
          `Can not updateUserBossProgressionWithGameOutcome(): invalid boss ID - ${gameId}`,
        ),
      );
    }
    ...
    var txPromise = knex
      .transaction((tx) =>
        PromiseUtils.withTimeout(
          Promise.resolve(tx('users').first('id').where('id', userId).forUpdate())
            ...
            .then(function (bossEventsSnapshot) {
              ...
              for (var eventId in bossEventsData) {
                ...
                if (
                  eventData.event_start == null ||
                  eventData.event_start > MOMENT_NOW_UTC.valueOf()
                ) {
                  continue;
                }
              ...
              if (_chainState.matchingEventData == null) {
                Logger.module('UsersModule').debug(
                  `updateUserBossProgressionWithGameOutcome() -> no matching boss event id for user ${userId} in game ${gameId}.`
                    .red,
                );
                return Promise.reject(
                  new Error(
                    `Can not updateUserBossProgressionWithGameOutcome(): No matching boss event - ${gameId}`,
                  ),
                );
              }
            })
            ...
            .then(function ([userRow, userBossDefeatedRow]) {
              ...
              Logger.module('UsersModule').debug(
                `updateUserBossProgressionWithGameOutcome() -> user ${userId.blue} HAS earned a BOSS BATTLE reward!`,
              );
              ...
              allPromises.push(
                InventoryModule.addBoosterPackToUser(
                  ...
                  tx,
                  userId,
                  rewardData.spirit_orbs,
                  'boss battle',
                  _chainState.matchingEventId,
                ),
              );
              allPromises.push(
                GamesModule._addRewardIdToUserGame(tx, userId, gameId, rewardData.id),
            ...
            .then(function (rootRef) {
              ...
              allPromises.push(
                FirebasePromises.set(
                  rootRef.child('user-bosses-defeated').child(userId).child(bossId),
                  defeatedBossFBData,
                ),
              );
          ...
          onType(PromiseUtils.TimeoutError, function (e) {
            Logger.module('UsersModule').error(
              `updateUserBossProgressionWithGameOutcome() -> ERROR, operation timeout for u:${userId} g:${gameId}`,
            );
            throw e;
          }),
        ),
      ...
      .then(() =>
        Logger.module('UsersModule').debug(
          `updateUserBossProgressionWithGameOutcome() -> user ${userId.blue}`.green +
            ` game ${gameId} boss id:${bossId}`.green,
        ),
      )
      .finally(() => GamesModule.markClientGameJobStatusAsComplete(userId, gameId, 'progression'));
  ...
  static updateGameCounters(
    ...
    return knex
        ...
        PromiseUtils.withTimeout(
          Promise.resolve(tx('users').first('id').where('id', userId).forUpdate())
            ...
            .then(function ([
              ...
              // game type counter
              const counter = DataAccessHelpers.updateCounterWithGameOutcome(
                counterRow,
                isWinner,
                isDraw,
                isUnscored,
              ...
              // faction counter
              const factionCounter = DataAccessHelpers.updateCounterWithGameOutcome(
                factionCounterRow,
                isWinner,
                isDraw,
                isUnscored,
              ...
              if (factionCounter.created_at == null) {
              ...
              if (factionCounterRow) {
                allPromises.push(
                  knex('user_game_faction_counters')
                    .where({
                      user_id: userId,
                      faction_id: factionId,
                      game_type: gameType,
                    })
                    .update(factionCounter)
                    .transacting(tx),
                );
              } else {
                allPromises.push(
                  knex('user_game_faction_counters').insert(factionCounter).transacting(tx),
                );
              }

              // general counter
              const generalCounter = DataAccessHelpers.updateCounterWithGameOutcome(
                generalCounterRow,
                isWinner,
                isDraw,
                isUnscored,
              ...
              if (generalCounter.created_at == null) {
              ...
              if (generalCounterRow) {
                allPromises.push(
                  knex('user_game_general_counters')
                    .where({
                      user_id: userId,
                      general_id: generalId,
                      game_type: gameType,
                    })
                    .update(generalCounter)
                    .transacting(tx),
                );
              } else {
                allPromises.push(
                  knex('user_game_general_counters').insert(generalCounter).transacting(tx),
                );
              }

              // season counter
              const seasonCounter = DataAccessHelpers.updateCounterWithGameOutcome(
                seasonCounterRow,
                isWinner,
                isDraw,
                isUnscored,
              ...
              if (seasonCounter.season_starting_at == null) {
                seasonCounter.season_starting_at = MOMENT_SEASON_START_UTC.toDate();
              }
              if (seasonCounter.created_at == null) {
              ...
              if (seasonCounterRow) {
                allPromises.push(
                  knex('user_game_season_counters')
                    .where({
                      user_id: userId,
                      season_starting_at: MOMENT_SEASON_START_UTC.toDate(),
                      game_type: gameType,
                    })
                    .update(seasonCounter)
                    .transacting(tx),
                );
              } else {
                allPromises.push(
                  knex('user_game_season_counters').insert(seasonCounter).transacting(tx),
                );
              }
          ...
          onType(PromiseUtils.TimeoutError, function (e) {
            Logger.module('UsersModule').error(
              `updateGameCounters() -> ERROR, operation timeout for u:${userId}`,
            );
            throw e;
          }),
        ),
      ...
      .then(function () {
        Logger.module('UsersModule').debug(
          `updateGameCounters() -> updated ${gameType} game counters for ${userId.blue}`,
        );
        return {
  ...
  static updateUserStatsWithGame(userId, gameId, gameType, gameData, systemTime?) {
    ...
    if (!userId || !gameId) {
      return Promise.reject(
        new Error(
          `Can not update user-stats : invalid user ID - ${userId} - or game ID - ${gameId}`,
        ),
      );
    }
    ...
    return DuelystFirebase.connect()
      ...
      .then(function (fbRootRef) {
        ...
        return new Promise((resolve, reject) =>
          statsRef.once('value', (statsSnapshot) => resolve(statsSnapshot.val())),
        );
      })
      .then(function (statsData) {
        ...
        try {
          ...
          if (queueStatsData.winStreak == null) {
          ...
          for (var cardIndex of Array.from<any>(cardIndices)) {
            ...
            if (card.ownerId === userId) {
              factionStatsData.cardsPlayedCounts[card.id] =
                (factionStatsData.cardsPlayedCounts[card.id] || 0) + 1;
            }
          }
          ...
          factionStatsData.totalDamageDealt =
            (factionStatsData.totalDamageDealt || 0) + playerData.totalDamageDealt;
          factionStatsData.totalDamageDealtToGeneral =
            (factionStatsData.totalDamageDealtToGeneral || 0) +
            playerData.totalDamageDealtToGeneral;
          factionStatsData.totalMinionsKilled =
            (factionStatsData.totalMinionsKilled || 0) + playerData.totalMinionsKilled;
          factionStatsData.totalMinionsPlayedFromHand =
            (factionStatsData.totalMinionsPlayedFromHand || 0) +
            playerData.totalMinionsPlayedFromHand;
          factionStatsData.totalMinionsSpawned =
            (factionStatsData.totalMinionsSpawned || 0) + playerData.totalMinionsSpawned;
          factionStatsData.totalSpellsCast =
            (factionStatsData.totalSpellsCast || 0) + playerData.totalSpellsCast;
          factionStatsData.totalSpellsPlayedFromHand =
            (factionStatsData.totalSpellsPlayedFromHand || 0) +
            playerData.totalSpellsPlayedFromHand;
        } catch (e) {
          Logger.module('UsersModule').debug(
            `updateUserStatsWithGame() -> caught ERROR processing stats data for user ${userId}: ${e.message}`
              .red,
          );
        ...
        return new Promise(function (resolve, reject) {
          Logger.module('UsersModule').debug(
            `updateUserStatsWithGame() -> UPDATING stats for user ${userId}`,
          );
          ...
          const onUpdateUserStatsTransactionComplete = function (error, committed, snapshot) {
            if (error) {
              ...
              Logger.module('UsersModule').debug(
                `updateUserStatsWithGame() -> updated user-stats committed for ${userId.blue}`,
              );
              return resolve(snapshot.val());
            } else {
              return reject(
                new Errors.FirebaseTransactionDidNotCommitError(
                  `User Stats for ${userId.blue} did NOT COMMIT`,
                ),
              );
            }
          };
          ...
          // update users stats
          return statsRef.transaction(
            onUpdateUserStatsTransaction,
            onUpdateUserStatsTransactionComplete,
          );
        });
      });
  }
  ...
  static iterateNewPlayerCoreProgression(userId) {
    ...
    return knex('user_new_player_progression')
      .where('user_id', userId)
      .andWhere('module_name', NewPlayerProgressionModuleLookup.Core)
      .first()
      .then(function (moduleProgression) {
        const stage =
          NewPlayerProgressionStageEnum[
            moduleProgression != null ? moduleProgression.stage : undefined
          ] || NewPlayerProgressionStageEnum.Tutorial;
        ...
        return Promise.all([
          ...
          .then(function ([quests, questsComplete]) {
            ...
            if ((quests != null ? quests.length : undefined) > 0) {
              ...
              const beginnerQuestInProgress = _.find(quests, (q) =>
                _.find(beginnerQuests, (bq) => bq.id === q.quest_type_id),
              );
            ...
            const beginnerQuestsComplete = _.filter(questsComplete, (q) =>
              _.find(beginnerQuests, (bq) => bq.id === q.quest_type_id),
            );
            ...
            if (
              (beginnerQuestsComplete != null ? beginnerQuestsComplete.length : undefined) <
              beginnerQuests.length
            ) {
              // throw new Error("Invalid state: user never received all required stage quests")
              Logger.module('SDK').warn(
                `iterateNewPlayerCoreProgression() -> Invalid state: user ${userId.blue} never received all required stage ${stage.key} quests`,
              );
              return QuestsModule.generateBeginnerQuests(userId)
                ...
                .catch(
                  onType(Errors.NoNeedForNewBeginnerQuestsError, (e) =>
                    Logger.module('SDK').debug(
                      `iterateNewPlayerCoreProgression() -> no need for new quests at ${nextStage.key} for ${userId.blue}`,
                    ),
                  ),
                )
                .then(function () {
            ...
            for (var s of Array.from<any>(NewPlayerProgressionStageEnum.enums)) {
              if (s.value > stage.value) {
                Logger.module('SDK').debug(
                  `iterateNewPlayerCoreProgression() -> from stage ${stage.key} to next stage ${s.key} for ${userId.blue}`,
                );
                nextStage = s;
                break;
              }
            ...
            return UsersModule.setNewPlayerFeatureProgression(
              userId,
  ...
  static setNewPlayerFeatureProgression(userId, moduleName, stage) {
    ...
    Logger.module('UsersModule').time(
      `setNewPlayerFeatureProgression() -> user ${userId.blue} marking module ${moduleName} as ${stage}.`,
    );

    if (
      moduleName === NewPlayerProgressionModuleLookup.Core &&
      NewPlayerProgressionStageEnum[stage] == null
    ) {
        ...
        tx('user_new_player_progression')
          ...
          .then(function (progressionRow) {
            ...
            if (moduleName === NewPlayerProgressionModuleLookup.Core) {
              const currentStage =
                (progressionRow != null ? progressionRow.stage : undefined) ||
                NewPlayerProgressionStageEnum.Tutorial;
              if (NewPlayerProgressionStageEnum[stage].value < currentStage.value) {
                throw new Errors.BadRequestError(
                  'Can not roll back to a previous core new player stage',
                );
              }
            }
            ...
            if (progressionRow && progressionRow.stage === stage) {
              Logger.module('UsersModule').error(
                `setNewPlayerFeatureProgression() -> ERROR: requested same stage: ${stage}.`,
              );
              throw new Errors.BadRequestError(
                'New player progression stage already at the requested stage',
              );
              ...
              _chainState.progressionRow.updated_at = MOMENT_NOW_UTC.toDate();
              queryPromise = tx('user_new_player_progression')
                .where({ user_id: userId, module_name: moduleName })
                .update({
  ...
  static setPortraitId(userId, portraitId) {
    ...
    if (userId == null) {
      return Promise.reject(
        new Errors.NotFoundError(`Can not setPortraitId(): invalid user ID - ${userId}`),
      );
    }
    ...
    return (txPromise = knex
      .transaction(function (tx) {
        InventoryModule.isAllowedToUseCosmetic(txPromise, tx, userId, portraitId)
          ...
          .then((rootRef) =>
            FirebasePromises.set(
              rootRef.child('users').child(userId).child('presence').child('portrait_id'),
              portraitId,
            ),
          )
          .then(() => SyncModule._bumpUserTransactionCounter(tx, userId))
  ...
  static setBattleMapId(userId, battleMapId) {
    ...
    return (txPromise = knex
      .transaction(function (tx) {
        const checkForInventoryPromise =
          battleMapId !== null
            ? InventoryModule.isAllowedToUseCosmetic(txPromise, tx, userId, battleMapId)
        ...
        checkForInventoryPromise
          ...
          .then((rootRef) =>
            FirebasePromises.set(
              rootRef.child('users').child(userId).child('battle_map_id'),
              battleMapId,
            ),
          )
          .then(() => SyncModule._bumpUserTransactionCounter(tx, userId))
  ...
  static inGameNotify(userId, message, type = null) {
    ...
    return Promise.resolve()
      .then(() => DuelystFirebase.connect().getRootRef())
      .then((rootRef) =>
        FirebasePromises.push(rootRef.child('user-notifications').child(userId), {
          message,
  ...
  static ___hardWipeUserData(userId) {
    Logger.module('UsersModule').time(
      `___hardWipeUserData() -> WARNING: hard wiping ${userId.blue}`.red,
    );
  ...
  static ___snapshotUserData(userId) {
    Logger.module('UsersModule').time(
      `___snapshotUserData() -> retrieving data for user ID ${userId.blue}`.green,
    );
    ...
    const userTableColumns = [
      ...
      'rank_is_unread',
      'top_rank',
      'top_rank_starting_at',
      'top_rank_updated_at',
      'daily_quests_generated_at',
      'daily_quests_updated_at',
      'achievements_last_read_at',
      'wallet_gold',
      'wallet_spirit',
      'wallet_cores',
      'wallet_updated_at',
      ...
      'card_updated_at',
      'top_gauntlet_win_count',
      'portrait_id',
      'total_gold_tips_given',
      'referral_code',
      'is_suspended',
      'suspended_at',
      'suspended_memo',
      'top_rank_ladder_position',
      'top_rank_rating',
      'is_bot',
    ];
    ...
    return DuelystFirebase.connect()
      ...
      .then(function ([
        ...
        userSnapshot.firebase.fbUserAggregates =
          fbUserAggregates != null ? fbUserAggregates.val() : undefined;
        userSnapshot.firebase.fbUserArenaRun =
          fbUserArenaRun != null ? fbUserArenaRun.val() : undefined;
        userSnapshot.firebase.fbUserChallengeProgression =
          fbUserChallengeProgression != null ? fbUserChallengeProgression.val() : undefined;
        userSnapshot.firebase.fbUserDecks = fbUserDecks != null ? fbUserDecks.val() : undefined;
        userSnapshot.firebase.fbUserFactionProgression =
          fbUserFactionProgression != null ? fbUserFactionProgression.val() : undefined;
        userSnapshot.firebase.fbUserGames = fbUserGames != null ? fbUserGames.val() : undefined;
        userSnapshot.firebase.fbUserGameJobStatus =
          fbUserGameJobStatus != null ? fbUserGameJobStatus.val() : undefined;
        userSnapshot.firebase.fbUserInventory =
          fbUserInventory != null ? fbUserInventory.val() : undefined;
        userSnapshot.firebase.fbUserLogs = fbUserLogs != null ? fbUserLogs.val() : undefined;

        userSnapshot.firebase.fbUserMatchmakingErrors =
          fbUserMatchmakingErrors != null ? fbUserMatchmakingErrors.val() : undefined;
        userSnapshot.firebase.fbUserNews = fbUserNews != null ? fbUserNews.val() : undefined;
        userSnapshot.firebase.fbUserProgression =
          fbUserProgression != null ? fbUserProgression.val() : undefined;
        userSnapshot.firebase.fbUserQuests = fbUserQuests != null ? fbUserQuests.val() : undefined;
        userSnapshot.firebase.fbUserRanking =
          fbUserRanking != null ? fbUserRanking.val() : undefined;
        userSnapshot.firebase.fbUserRewards =
          fbUserRewards != null ? fbUserRewards.val() : undefined;
        userSnapshot.firebase.fbUserStats = fbUserStats != null ? fbUserStats.val() : undefined;
        userSnapshot.firebase.fbUserTransactions =
          fbUserTransactions != null ? fbUserTransactions.val() : undefined;
        userSnapshot.firebase.fbUserAchievements =
          fbUserAchievements != null ? fbUserAchievements.val() : undefined;
  ...
  static tipAnotherPlayerForGame(userId, gameId, goldAmount?) {
    ...
    return Promise.all([
      ...
      var txPromise = knex.transaction((tx) =>
        Promise.all([
          // debit user's gold
          InventoryModule.debitGoldFromUser(
            txPromise,
            tx,
            userId,
            -goldAmount,
            `gold tip to ${gameId}:${playerId}`,
          ),
          // give opponent gold
          InventoryModule.giveUserGold(
            txPromise,
            ...
            playerId,
            goldAmount,
            `gold tip from ${gameId}:${userId}`,
          ),
          // update user game record
          knex('user_games')
            .where({ user_id: userId, game_id: gameId })
            .update('gold_tip_amount', goldAmount)
            .transacting(tx),
          // update master game record
          knex('games')
            .where({ id: gameId })
            .increment('gold_tip_amount', goldAmount)
            .transacting(tx),
          // update master game record
          knex('users')
            .where({ id: userId })
            .increment('total_gold_tips_given', goldAmount)
            .transacting(tx),
        ])
          .then(() =>
            Promise.all([
          ...
          .then(() =>
            UsersModule.inGameNotify(
              playerId,
              `${userRow.username} tipped you ${goldAmount} GOLD`,
              'gold tip',
            ),
          )
          .then(tx.commit)
