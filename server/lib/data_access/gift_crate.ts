...
class GiftCrateModule {
  ...
  static addGiftCrateToUser(
    ...
    if (!userId) {
      Logger.module('GiftCrateModule').debug(
        `addGiftCrateToUser() -> invalid user ID - ${userId}.`.red,
      );
    ...
    Logger.module('GiftCrateModule').time(
      `addGiftCrateToUser() -> added ${crateId} to user ${userId.blue}.`.green,
    );
    ...
    return (
      trx
        ...
        .then(function () {
          Logger.module('GiftCrateModule').timeEnd(
            `addGiftCrateToUser() -> added ${crateId} to user ${userId.blue}.`.green,
          );
  ...
  static unlockGiftCrate(userId, crateId, keyId, systemTime) {
    ...
    if (!crateId) {
      return Promise.reject(
        new Error(`Can not claim gift crate: no crate ID for user id - ${userId}`),
      );
    }
    ...
    return (txPromise = knex
      .transaction(function (tx) {
        knex
          ...
          .then(function (giftCrateRow) {
            ...
            if (giftCrateRow == null || giftCrateRow.user_id !== userId) {
              return Promise.reject(
                new Errors.NotFoundError(
                  'The gift crate ID provided does not exist or belong to you.',
                ),
              );
            }
            ...
            const giftTemplateData = GiftCrateFactory.giftCrateTemplateForType(
              giftCrateRow.crate_type,
            );

            if (giftTemplateData == null) {
              return Promise.reject(
                new Errors.UnexpectedBadDataError(
                  `No gift crate data for type ${giftCrateRow.crate_type}`,
                ),
              );
            }
            ...
            if (!GiftCrateFactory.getIsCrateTypeAvailable(giftCrateRow.crate_type, current_utc)) {
              return Promise.reject(
                new Errors.UnexpectedBadDataError('The gift crate ID is not yet available.'),
              );
            }
            ...
            if (giftTemplateData.rewards.spirit) {
              ...
              allPromises.push(
                InventoryModule.giveUserSpirit(
                  ...
                  tx,
                  userId,
                  giftTemplateData.rewards.spirit,
                  'gift crate reward',
                  crateId,
                ),
              );
            ...
            if (
              (giftTemplateData.rewards.random_cards != null
                ? giftTemplateData.rewards.random_cards.length
                : undefined) > 0 ||
              ...
              if (
                ...
                for (var randomCardInfo of Array.from<any>(giftTemplateData.rewards.random_cards)) {
                  var cardsToSampleFrom = SDK.GameSession.getCardCaches()
                    .getCardSet(SDK.CardSet.Core)
                    .getRarity(randomCardInfo.rarity)
            ...
            if (giftTemplateData.rewards.spirit_orbs) {
              ...
              for (
                i = 1, end = giftTemplateData.rewards.spirit_orbs, asc = end >= 1;
                asc ? i <= end : i >= end;
                asc ? i++ : i--
              ) {
                rewardRows.push({
            ...
            if (giftTemplateData.rewards.gauntlet_tickets) {
              let asc1, end1;
              for (
                i = 1, end1 = giftTemplateData.rewards.gauntlet_tickets, asc1 = end1 >= 1;
                asc1 ? i <= end1 : i >= end1;
                asc1 ? i++ : i--
              ) {
                rewardRows.push({
            ...
            if (
              (giftTemplateData.rewards.crate_keys != null
                ? giftTemplateData.rewards.crate_keys.length
                : undefined) > 0
            ) {
              ...
              _chainState.rewardsToReturn.push({
                cosmetic_keys: giftTemplateData.rewards.crate_keys,
              });
            }

            if (
              (giftTemplateData.rewards.cosmetics != null
                ? giftTemplateData.rewards.cosmetics.length
                : undefined) > 0
            ) {
            ...
            if (giftTemplateData.rewards.spirit_box != null) {
              if (
                giftCrateRow.crate_type === GiftCrateLookup.FrostfirePurchasable2017 ||
                giftCrateRow.crate_type === GiftCrateLookup.FrostfirePremiumPurchasable2017
              ) {
                ...
                return Promise.reject(
                  new Errors.NotFoundError('No spirit box template for this gift crate type'),
                );
              }
            }
            ...
            if (giftTemplateData.rewards.cosmetic_box != null) {
              if (
                ...
                return Promise.reject(
                  new Errors.NotFoundError('No cosmetic box template for this gift crate type'),
                );
              }
            }
          ...
          .then(function () {
            ...
            return Promise.all([
              knex('user_gift_crates')
                .where('crate_id', _chainState.crateId)
                .delete()
                .transacting(tx),
      ...
      .then(function () {
        Logger.module('GiftCrateModule').debug(
          `unlockGiftCrate() -> user ${userId.blue} `.green +
            ` unlocked gift crate ${_chainState.crateId}`.green,
        );
