...
const UserId = t.subtype(
  t.Str,
  (s) => s.length === 20 && s.match(/^[A-Za-z0-9\-\_]+$/) !== null,
  'UserId',
);

const Username = t.subtype(
  t.Str,
  (s) => {
...
const ReferralCode = t.subtype(t.Str, (s) => s.length >= 4 && s.length <= 16, 'ReferralCode');
...
const CurrencyType = t.subtype(t.Str, (s) => s === 'hard' || s === 'soft', 'Currency Type');
