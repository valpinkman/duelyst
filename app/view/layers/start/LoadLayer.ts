...
const LoadLayer = FXCompositeLayer.extend({
  ...
  ctor() {
    ...
    this.progress_label = new cc.LabelTTF(
      '0%',
      RSX.font_light.name,
      24,
      cc.size(100, 40),
  ...
  onResize() {
    ...
    for (let i = 1; i <= totalNumPillars; i += 2) {
      ...
      offset = cc.p(
        winWidth * (0.1 - ratioPct * 0.15 - pillarPctSmoothed * 0.575),
        winHeight * (-0.2 + pillarPctSmoothed * 0.25),
      );
      ...
      ratio = cc.p(this.maxParallaxRatio.x * ratioPct, this.maxParallaxRatio.y * ratioPct);
      offset = cc.p(
        winWidth * (0.2 + ratioPct * 0.15 + pillarPctSmoothed * 0.2),
        winHeight * (-0.2 + pillarPctSmoothed * 0.2),
      );
    ...
    this.parallaxLayer.addOrUpdateParallaxedNode(
      ...
      totalNumPillars - 3,
      cc.p(),
      this.birds3.getSourceScreenOffsetPosition(),
    );
    ...
    this.parallaxLayer.addOrUpdateParallaxedNode(
      ...
      totalNumPillars - 3,
      cc.p(),
      this.birds4.getSourceScreenOffsetPosition(),
    );
    ...
    if (this.birds1.getParent() != parallaxNode) {
      if (this._birds1Action == null) {
        this._birds1Action = this.runAction(
          cc.sequence(
            cc.delayTime(6.0),
            cc.callFunc(function () {
              parallaxNode.addChild(
                ...
                totalNumPillars - 2.5,
                cc.p(),
                this.birds1.getSourceScreenOffsetPosition(),
              );
    ...
    if (this.birds2.getParent() != parallaxNode) {
      if (this._birds2Action == null) {
        this._birds2Action = this.runAction(
          cc.sequence(
            cc.delayTime(6.0),
            cc.callFunc(function () {
              parallaxNode.addChild(
                ...
                totalNumPillars - 2.5,
                cc.p(),
                this.birds2.getSourceScreenOffsetPosition(),
              );
              this.parallaxLayer.forceParallaxUpdate();
            }, this),
          ),
    ...
    offset = cc.p(
      -winWidth * 0.5 +
        this.platform.getContentSize().width * (0.5 - ratio.x) * this.platform.getScale(),
      -winHeight * 0.5 +
        this.platform.getContentSize().height * (0.5 - ratio.y * 2.0) * this.platform.getScale(),
    );
