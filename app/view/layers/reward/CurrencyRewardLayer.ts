...
const CurrencyRewardLayer = RewardLayer.extend({
  getRequiredResources() {
    return RewardLayer.prototype.getRequiredResources
      .call(this)
      .concat(PKGS.getPkgForIdentifier('currency_reward'));
  },
  ...
  animateReward(type, amount, title, subtitle, description) {
    return this.whenRequiredResourcesReady().then((requestId) => {
      ...
      // gold AMOUNT # label
      const currency_amount_label = new cc.LabelTTF(
        amount.toString(),
      ...
      const reward_label = new cc.LabelTTF(
        `+${amount.toString()} ${currencyTypeLocalized}`,
        RSX.font_light.name,
        20,
        cc.size(300, 24),
        cc.TEXT_ALIGNMENT_CENTER,
