...
const BoosterRewardLayer = RewardLayer.extend({
  getRequiredResources() {
    return RewardLayer.prototype.getRequiredResources
      .call(this)
      .concat(PKGS.getPkgForIdentifier('booster_reward'));
  },
  ...
  animateReward(title, subtitle, cardSetId) {
    ...
    return Promise.all([
      ...
      const reward_description_label = new cc.LabelTTF(
        i18next.t('rewards.spirit_orb_reward_info'),
        RSX.font_light.name,
        15,
        cc.size(200, 92),
        cc.TEXT_ALIGNMENT_CENTER,
                ...
                cc.callFunc(() => {
                  ...
                  reward_label.runAction(
                    cc.spawn(
                      ...
                      cc
                        .scaleTo(CONFIG.ANIMATE_SLOW_DURATION, 1.0)
                        .easing(cc.easeExponentialOut(3.0)),
                    ),
                  );
