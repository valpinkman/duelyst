...
const UpgradeInstructionsLayer = BaseLayer.extend({
  ...
  ctor(riftData, wonLastGauntletGame) {
    ...
    const hasStoredUpgrades =
      riftData.stored_upgrades != null && riftData.stored_upgrades.length != 0;
    ...
    instructionalArrowSprite.runAction(
      cc.sequence(
        ...
        cc
          .moveBy(CONFIG.MOVE_SLOW_DURATION, cc.p(winSize.width / 2 - 400, 0))
    ...
    this.runDetailsLabel = new cc.LabelTTF(
      i18next.t('rift.choose_card_upgrade_instruction_message'),
      RSX.font_bold.name,
      32,
      cc.size(500, 50),
    ...
    if (hasStoredUpgrades) {
      ...
      upgradeDetailText = i18next.t('rift.upgrade_details_with_stored_packs_message', {
        storedPacks: storedPacksRemaining,
      });
    } else {
      ...
      if (storedUpgradeCount >= 10) {
        ...
        upgradeDetailText += i18next.t('rift.upgrade_added_details_current_saved_message', {
          storedUpgradeCount,
        });
      }
    }
