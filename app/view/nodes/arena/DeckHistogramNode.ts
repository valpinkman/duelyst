...
const DeckHistogramNode = cc.Node.extend({
  ...
  ctor(manaCounts) {
    ...
    horizontalLineNode.drawSegment(
      cc.p(0, 0),
      cc.p(270 + this.barWidth, 0),
      1.0,
      new cc.Color(255, 255, 255, 100),
    );
    ...
    for (let i = 0; i < 10; i++) {
      const columnLabel = new cc.LabelTTF(
        i.toString(),
        RSX.font_light.name,
        14,
        cc.size(this.barWidth, 30),
      ...
      columnBar.drawRect(
        cc.p(0, 0),
        cc.p(this.barWidth, this.barUnitHeight),
