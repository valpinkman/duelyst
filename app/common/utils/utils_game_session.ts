...
UtilsGameSession.getPlayerSetupDataForPlayerId = function (gameSessionData, playerId) {
  const { gameSetupData } = gameSessionData;
...
UtilsGameSession.groupModifiersBySourceCard = function (modifiers) {
  ...
  for (const m of Array.from<any>(modifiers)) {
    ...
    if (sourceCard != null) {
      sourceCardActionIndex = sourceCard.getAppliedToBoardByActionIndex();
    } else {
      sourceCardActionIndex = -1;
    }
    if (modifiersBySourceCardActionIndex[sourceCardActionIndex] == null) {
      modifiersBySourceCardActionIndex[sourceCardActionIndex] = [];
    }
  ...
  const sourceCardActionIndices = Object.keys(modifiersBySourceCardActionIndex).sort(
...
UtilsGameSession.getValidBoardPositionsFromPattern = function (
  board,
  boardPosition,
  pattern,
  allowObstructions,
) {
  if (allowObstructions == null) {
    allowObstructions = true;
  }
  if (
    UtilsPosition.getArrayOfPositionsContainsArrayOfPositions(pattern, CONFIG.PATTERN_WHOLE_BOARD)
  ) {
  ...
  for (const offset of Array.from<any>(pattern)) {
    ...
    if (
      board.isOnBoard(patternPosition) &&
      (allowObstructions || !board.getObstructionAtPosition(patternPosition))
    ) {
 ...
 */
UtilsGameSession.getSmartSpawnPositionsFromPattern = function (
  gameSession,
  sourcePosition,
  pattern,
  cardToSpawn,
  ...
  for (const offset of Array.from<any>(pattern)) {
    ...
    if (
      board.isOnBoard(spawnPosition) &&
      !board.getObstructionAtPositionForEntity(spawnPosition, cardToSpawn)
    ) {
 ...
 */
UtilsGameSession.getRandomSmartSpawnPositionsFromPattern = function (
  gameSession,
  sourcePosition,
  pattern,
  cardToSpawn,
  ...
  const validSpawnPositions = UtilsGameSession.getSmartSpawnPositionsFromPattern(
    gameSession,
    sourcePosition,
    pattern,
    cardToSpawn,
  ...
  for (i = 0, end = spawnCount, asc = end >= 0; asc ? i < end : i > end; asc ? i++ : i--) {
    if (validSpawnPositions.length > 0) {
      spawnPositions.push(
        validSpawnPositions.splice(
          gameSession.getRandomIntegerForExecution(validSpawnPositions.length),
          1,
        )[0],
      );
    } else {
 ...
 */
UtilsGameSession.getRandomNonConflictingSmartSpawnPositionsFromPatterns = function (
  gameSession,
  sourcePositions,
  patternOrPatterns,
  cardOrCardsToSpawn,
  ...
  if (sourcePositions.length === 1) {
    ...
    pattern =
      _.isArray(patternOrPatterns) && _.isArray(patternOrPatterns[0])
        ? patternOrPatterns[0]
        : patternOrPatterns;
    ...
    if (!_.isNumber(spawnCount) || isNaN(spawnCount) || spawnCount <= 0) {
      spawnCount = 1;
    }
    validSpawnPositions = UtilsGameSession.getSmartSpawnPositionsFromPattern(
      gameSession,
      sourcePosition,
      pattern,
      cardToSpawn,
    ...
    let j;
    let conflictDataForPosition;
    let conflicts;
    let nonConflictingPositions;
    let position;
    const getAppliedByActionIndexFromSource = (source) =>
      (typeof source.getAppliedByActionIndex === 'function'
        ? source.getAppliedByActionIndex()
        : undefined) ||
      (typeof source.getAppliedToBoardByActionIndex === 'function'
        ? source.getAppliedToBoardByActionIndex()
        : undefined);
    ...
    for (j = 0, i = j; j < sourcePositions.length; j++, i = j) {
      ...
      pattern =
        _.isArray(patternOrPatterns) && _.isArray(patternOrPatterns[i])
          ? patternOrPatterns[i]
          : patternOrPatterns;
      ...
      UtilsJavascript.arraySortedInsertAscendingByComparator(
    ...
    // find conflicts
    let numConflicts = 0;
    const conflictScoringMethod = (conflictDataForPosition) =>
      conflictDataForPosition.conflicts.length;
    for (spawnData of Array.from<any>(spawnPositionsWithSource)) {
      ({ validSpawnPositions } = spawnData);
      ({ conflicts } = spawnData);
      ({ nonConflictingPositions } = spawnData);
      ...
      for (position of Array.from<any>(validSpawnPositions)) {
          ...
          if (otherSpawnData !== spawnData) {
            ...
            for (let otherIndex = 0; otherIndex < otherSpawnPositions.length; otherIndex++) {
              ...
              if (x === otherPosition.x && y === otherPosition.y) {
                if (conflictDataForPosition == null) {
                  conflictDataForPosition = { position, conflicts: [] };
                }
        ...
        if (conflictDataForPosition != null && conflictDataForPosition.conflicts.length > 0) {
          ...
          UtilsJavascript.arraySortedInsertByScore(
            conflicts,
            conflictDataForPosition,
            conflictScoringMethod,
          );
        } else {
        ...
        if (!UtilsPosition.getIsPositionInPositions(validSpawnPositions, conflictedPosition)) {
          ...
          for (const conflictingSpawnData of Array.from<any>(conflictDataForPosition.conflicts)) {
            ...
            if (numSpawnPositions > 1) {
              UtilsPosition.removePositionFromPositions(
                conflictedPosition,
                conflictingSpawnPositions,
              );
              resolvedConflict = true;
            } else if (numSpawnPositions > 0) {
              const appliedIndex = getAppliedByActionIndexFromSource(source);
              const conflictingAppliedIndex = getAppliedByActionIndexFromSource(
                conflictingSpawnData.source,
              );
              if (
                appliedIndex < conflictingAppliedIndex ||
                (appliedIndex === -1 &&
                  conflictingAppliedIndex === -1 &&
                  source.getIndex() < conflictingSpawnData.source.getIndex())
              ) {
                UtilsPosition.removePositionFromPositions(
                  conflictedPosition,
                  conflictingSpawnPositions,
  ...
  for (spawnData of Array.from<any>(spawnPositionsWithSource)) {
    ...
    var end;
    ({ validSpawnPositions } = spawnData);
    const { spawnPositions } = spawnData;
    ({ spawnCount } = spawnData);
    for (i = 0, end = spawnCount, asc = end >= 0; asc ? i < end : i > end; asc ? i++ : i--) {
      if (validSpawnPositions.length > 0) {
        spawnPositions.push(
          validSpawnPositions.splice(
 ...
 */
UtilsGameSession.getRandomNonConflictingSmartSpawnPositionsForModifier = function (
  modifier,
  modifierClass,
) {
  ...
  for (const coordinatingModifier of Array.from<any>(modifiersToCoordinateWith)) {
    ...
    cardsToSpawn.push(
      gameSession.getExistingCardFromIndexOrCachedCardFromData(
        coordinatingModifier.getCardDataOrIndexToSpawn(),
      ),
    );
 ...
 */
UtilsGameSession.scrubGameSessionData = function (
  gameSession,
  gameSessionData,
  scrubFromPerspectiveOfPlayerId,
  forSpectator,
) {
  ...
  UtilsGameSession.scrubGameSetupData(
    gameSession,
    gameSessionData.gameSetupData,
    scrubFromPerspectiveOfPlayerId,
    forSpectator,
  );

  // scrub player data
  UtilsGameSession.scrubSensitivePlayerData(
    gameSession,
    gameSessionData.players,
    scrubFromPerspectiveOfPlayerId,
    forSpectator,
  );
  ...
  for (index of Array.from<any>(modifierIndices)) {
    ...
    if (
      modifierData.cardAffectedIndex != null &&
      gameSessionData.cardsByIndex[modifierData.cardAffectedIndex] == null
    ) {
 ...
 * @returns {Object}
 */
UtilsGameSession.scrubGameSetupData = function (
  gameSession,
  gameSetupData,
  scrubFromPerspectiveOfPlayerId,
  forSpectator,
  ...
  for (let i = 0; i < gameSetupData.players.length; i++) {
    ...
    if (forSpectator || playerData.userId !== scrubFromPerspectiveOfPlayerId) {
      ...
      if (playerData.userId !== scrubFromPerspectiveOfPlayerId) {
        playerData.startingHand = _.map(playerData.startingHand, (cardData) => {
          if (cardData != null) {
 ...
 * @param  {Boolean}    forSpectator            Should the scrubbing be done for someone watching the game? If so we usually want to blank out the deck since even if a buddy can see your hand, they shouldn't be able to deck snipe.
 */
UtilsGameSession.scrubSensitiveActionData = function (
  gameSession,
  actionData,
  scrubFromPerspectiveOfPlayerId,
  forSpectator,
  ...
  const { resolveSubActionIndices } = actionData;
  if (resolveSubActionIndices != null) {
    for (i = resolveSubActionIndices.length - 1; i >= 0; i--) {
      ...
      if (resolveSubAction == null) {
        ...
        if (
          resolveSubAction.isRemovableDuringScrubbing(
            scrubFromPerspectiveOfPlayerId,
            forSpectator,
          ) &&
          target != null &&
          target.isScrubbable(scrubFromPerspectiveOfPlayerId, forSpectator)
        ) {
    ...
    for (i = subActionsOrderedByEventTypeData.length - 1; i >= 0; i--) {
      ...
      for (let j = subActionsData.length - 1; j >= 0; j--) {
        ...
        if (subAction == null) {
          ...
          if (
            subAction.isRemovableDuringScrubbing(scrubFromPerspectiveOfPlayerId, forSpectator) &&
            target != null &&
            target.isScrubbable(scrubFromPerspectiveOfPlayerId, forSpectator)
          ) {
            // delete sub actions acting on cards that don't exist or haven't been played yet
