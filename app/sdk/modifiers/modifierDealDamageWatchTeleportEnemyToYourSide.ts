...
class ModifierDealDamageWatchTeleportEnemyToYourSide extends ModifierDealDamageWatch {
  ...
  static description =
    'Whenever this minion deals damage to an enemy, teleport it to your starting side of the battlefield';
  ...
  onDealDamage(action) {
    ...
    if (enemy.getOwnerId() !== this.getCard().getOwnerId()) {
      ...
      if (enemy.isOwnedByPlayer1()) {
        // if owned by player 1, we want to teleport onto player 2s side
        randomTeleportAction.setPatternSourcePosition({
          x: Math.ceil(CONFIG.BOARDCOL * 0.5),
          y: 0,
        });
      } else if (enemy.isOwnedByPlayer2()) {
        // if owned by player 2, we want to teleport onto player 1s side
...
ModifierDealDamageWatchTeleportEnemyToYourSide.prototype.type =
  'ModifierDealDamageWatchTeleportEnemyToYourSide';
