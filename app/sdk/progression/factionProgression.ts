...
let factionId;
let i;
let nonPrismaticCardsUpToLevel;
let reward;
let asc1;
let end1;
...
class FactionProgression {
  declare static levelRewardsTable: any;

  static winXP = 10;
  static lossXP = 7;
  static winsBeforeLVLTenGiveFullLvlOfXp = true;
  static maxLevel = 51;
  static levelXPTable = {
    0: 0,
    1: 7,
  ...
  static _unlockedCardsUpToLevel = {};
  static _unlockedNonPrismaticCardsUpToLevel = {};
  static _unlockedCardsUpToLevelByFaction = {};
  static _unlockedNonPrismaticCardsUpToLevelByFaction = {};
  static _levelRequiredByCardId = {};
  static _factionRequiredByCardId = {};
  ...
  static xpEarnedForGameOutcome(isWinner, prevFactionLevel) {
    if (isWinner) {
      if (
        this.winsBeforeLVLTenGiveFullLvlOfXp &&
        prevFactionLevel != null &&
        prevFactionLevel < 10
      ) {
  ...
  static rewardDataForLevel(factionId, level) {
    return this.levelRewardsTable[factionId] != null
      ? this.levelRewardsTable[factionId][level]
      : undefined;
  }
...
FactionProgression.levelRewardsTable = {
  1: {
    // Lyonar
    1: {
      cards: [
...
for (factionId of Array.from<any>(factionIds)) {
  var asc;
  var end;
  factionId = parseInt(factionId);
  var cardsUpToLevelForFaction = (FactionProgression._unlockedCardsUpToLevelByFaction[factionId] =
    {});
  var nonPrismaticCardsUpToLevelForFaction =
    (FactionProgression._unlockedNonPrismaticCardsUpToLevelByFaction[factionId] = {});
  cardsUpToLevel = [];
  nonPrismaticCardsUpToLevel = [];

  for (
    i = 0, end = FactionProgression.maxLevel, asc = end >= 0;
    asc ? i <= end : i >= end;
    asc ? i++ : i--
  ) {
    ...
    if (reward != null ? reward.cards : undefined) {
      for (card of Array.from<any>(reward != null ? reward.cards : undefined)) {
...
nonPrismaticCardsUpToLevel = [];
for (
  i = 0, end1 = FactionProgression.maxLevel, asc1 = end1 >= 0;
  asc1 ? i <= end1 : i >= end1;
  asc1 ? i++ : i--
) {
  for (factionId of Array.from<any>(factionIds)) {
