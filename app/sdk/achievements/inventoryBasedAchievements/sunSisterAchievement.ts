...
class SisterAchievement extends Achievement {
  ...
  static progressForCardCollection(cardCollection, allCards) {
    ...
    const sisterCount =
      (cardCollection[Cards.Faction1.SunSister] != null
        ? cardCollection[Cards.Faction1.SunSister].count
        : undefined) || 0;
    if (sisterCount >= 3) {
      return 0;
    }
    ...
    const allFactionRares = _.filter(
      allCards,
      (card) =>
        card.getFactionId() === Factions.Faction1 &&
...
SisterAchievement.title = i18next.t('achievements.sun_sister_title');
SisterAchievement.description = i18next.t('achievements.sun_sister_desc');
SisterAchievement.rewards = {
  cards: [Cards.Faction1.SunSister, Cards.Faction1.SunSister, Cards.Faction1.SunSister],
};
